...
이 페이지는 C++ 유형 연산 빌더에 대한 참조입니다. 사용자 정의 유형에 대한 튜토리얼 수준의 소개는 [C++에서 사용자 정의 유형 만들기](/ko/mysql-8.4/stable/custom-types)를 참조하세요.
...
사용자 정의 해시 함수를 생략하면 유형의 원시 저장 바이트를 해시하는 방식으로
대체됩니다. 동일한 논리적 값에 대해 인코딩 함수가 둘 이상의 바이너리 표현을
생성할 수 있는 유형에서는 이 방식이 안전하지 않습니다 — 대표적인 경우가 부동
소수점 필드로, `-0.0`과 `+0.0`은 비교 시 같지만 비트 단위로는 다르므로
`GROUP BY`와 `COUNT(DISTINCT)`가 이들을 서로 다른 값으로 취급하게 됩니다.
...
사용자 정의 유형은 값당 최대 **65532바이트**의 저장 공간을 선언할 수 있습니다.
사용자 정의 컬럼은 `VARBINARY` 필드로 뒷받침되며, MySQL의 행당 65535바이트 예산은
해당 필드의 길이 바이트 2개와 행의 null 바이트도 함께 부담합니다. 컬럼의 nullable
여부는 `CREATE TABLE` 시점에 컬럼별로 결정되고 nullable이 기본값이므로, 이 상한값은
null 바이트를 위한 공간을 남겨 두어야 합니다. `persisted_length`와
`max_persisted_length` 모두 이 값에 대해 검사되며, 이 검사는 `CREATE TABLE`이 아니라
`INSTALL EXTENSION` 시점에 실행되므로 너무 넓은 유형은 확장 전체와 함께 거부됩니다:
...
상한값 아래에 머무른다고 해서 컬럼이 반드시 들어맞는 것은 아닙니다. 테이블의 다른
모든 컬럼도 동일한 65535바이트 행 예산을 사용하며, 이는 `CREATE TABLE`만이 가늠할 수
있으므로 상한값에 가깝거나 도달한 유형은 여전히 그 시점에 `ER_TOO_BIG_ROWSIZE`로
실패할 수 있습니다. 동일한 폭의 일반 `VARBINARY`도 같은 방식으로 동작합니다.
...
내장 기본값은 두 가지 방법 중 하나로 제공합니다. 두 방법은 상호 배타적입니다:
`.intrinsic_default_str()`과 `.intrinsic_default_vdf()`를 모두 선언하면
컴파일 타임 오류가 발생합니다.
...
`.params<>()`나 `.int_to_params<>()`가 등록되면 `build()`는 항상
`.resolve_params<>()`를 요구합니다: `int_to_params`는 `MYTYPE(N)` 축약 형식을
매개변수 문자열로 재작성하기만 하는 반면, `resolve_params`는 매개변수를 검증하고
저장 크기를 해석하는 관문입니다. 이를 생략하면 컴파일 타임 오류가 발생합니다.

<Warning>
`.max_persisted_length()`는 VEF 프로토콜 3 이상을 필요로 합니다. 이를 사용하는 유형은
프로토콜 3 이전 서버에서 로드될 수 없습니다. 이는 가변 길이 유형 또는 매개변수화
유형에만 유효합니다. 매개변수화되지 않은 고정 길이 유형이 이를 설정하면 컴파일에
실패합니다.
</Warning>

`resolve_params`가 보고하는 크기는 컬럼의 매개변수가 확정된 뒤 `CREATE TABLE`
시점에도 검증됩니다. 고정 길이 유형의 경우, 해석된 `persisted_length`는 선언된
`max_persisted_length`를 초과해서는 안 됩니다. 계산된 사용량(예: `dimension * 4`
크기 조정)이 그 한도를 넘는 매개변수화는 구문 오류로 거부됩니다. 길이 종류와
무관하게, `resolve_params`는 0보다 큰 `max_decode_buffer_length`를 해석해야 합니다.
이 값이 디코딩 스크래치 버퍼의 크기를 결정하기 때문이며, 0 이하의 결과는 동일한
부류의 오류로 거부됩니다.
...
`SHOW CREATE TABLE`은 작성자가 쓴 내용을 그대로 되풀이하지 않습니다. 서버는 해석된
매개변수 맵을 보관하고 이로부터 표시 형식을 다시 구성하며, `int_to_params(N)`이 맵
전체를 재현할 수 있으면 `MYTYPE(N)`을 우선하고, 그렇지 않으면 키가 정렬된 표준
`MYTYPE('key=value,...')` 형식으로 대체합니다. 따라서 `MYTYPE('dimension=8')`로 작성된
컬럼은 `MYTYPE(8)`로 다시 읽힙니다.
...
상한값이 필요합니다. 가변 길이 유형은 `.persisted_length()`를 함께 호출해서는
안 됩니다: 크기가 값별로 결정되므로 `build()`가 이 조합을 컴파일 타임에 거부합니다.
...
- [C++에서 사용자 정의 유형 만들기](/ko/mysql-8.4/stable/custom-types) — 사용자 정의 유형에 대한 튜토리얼 소개
- [C++ API 참조](/ko/mysql-8.4/stable/extension-api-reference) — VDF 계약, null 처리 및 버퍼 크기 조정
- [C++ 개발](/ko/mysql-8.4/stable/development) — VDF 작성 심화, 인수 및 결과 유형, 집계, 가변 인수
