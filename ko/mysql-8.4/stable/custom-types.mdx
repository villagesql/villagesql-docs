...
title: "C++에서 사용자 정의 유형 만들기"
description: "C++에서 VillageSQL 확장용 새 열 유형 정의 — 유형 연산, ALTER TABLE 규칙, 변환 함수 및 완전한 COMPLEX 유형 예제."
---
...
사용자 정의 유형을 통해 `COMPLEX`, `UUID`, `VECTOR`와 같은 새 열 유형을 정의할 수 있습니다. 이 유형들은 `ORDER BY`, 인덱스, 집계 함수와 함께 작동합니다. 이 페이지는 [C++에서 확장 만들기](/ko/mysql-8.4/stable/create) 튜토리얼의 4단계입니다. 계속하기 전에 1~3단계를 완료하세요.
...
<Note>
`from_string` VDF(사용자 정의 유형을 반환하는 함수)의 경우, 서버는 VDF를 호출하기 전에 출력 버퍼를 유형의 `persisted_length` 값 이상으로 크기 조정합니다. 따라서 `buf.size() >= persisted_length`가 호출 시 보장됩니다. 이는 고정 폭 유형과 매개변수화 유형(해당 유형 컨텍스트에서 `persisted_length`가 호출 시 해석됨) 모두에 적용됩니다. 별도의 버퍼 크기 요청이 필요하지 않습니다.
</Note>

원시 바이너리 접근은 `vsql::Span<T>`를 통해 이루어집니다. 이는 `T`의 연속된 시퀀스를 가리키는 소유하지 않는 뷰로, `in.value()`는 `vsql::Span<const unsigned char>`를 반환하고 `out.buffer()`는 `vsql::Span<unsigned char>`를 반환합니다. C++20 이상에서는 `vsql::Span<T>`가 `std::span<T>`의 별칭이며, C++17에서는 동일한 `data()`, `size()`, `empty()`, 인덱싱, 반복자 인터페이스를 제공하는 최소한의 소스 호환 대체 구현이 C++ SDK에 포함됩니다. `#include <villagesql/vsql.h>`를 통해 사용할 수 있습니다.
...
유형 이름은 비유형 템플릿 매개변수(NTTP)로 전달됩니다. `static constexpr const char[]` 배열로 선언하세요. 포인터의 식별 정보(identity)가 독립된 VDF 이름 버퍼의 키로 사용되므로, 동일한 함수 포인터를 공유하는 두 유형도 별도의 자동 생성 이름을 갖습니다.
...
전체 C++ 서명은 [유형 연산](/ko/mysql-8.4/stable/type-operations)을 참조하세요.
...
| 원본 | 대상 | 결과 |
|------|----|------|
| 비사용자 정의 유형 | 사용자 정의 유형 | 오류: `Cannot convert column 'col' to custom type 'MYTYPE'` |
| 사용자 정의 유형 | 문자열 유형 | 허용 |
| 사용자 정의 유형 | 비문자열 유형 | 오류: `Cannot convert custom type column 'col' to non-string type` |
| 사용자 정의 유형 | 다른 사용자 정의 유형 | 호환되지 않을 경우 오류: `Cannot convert between incompatible custom types 'A' and 'B'` |
...
디코딩에는 `snprintf` 대신 `std::to_chars`를 사용합니다. `"%.6f"`와 같은 고정 정밀도 형식은 `double`에 필요한 자릿수를 버리므로, `0.1234567891`이 `0.123457`로 읽힙니다. 인코딩은 `strtod`를 호출하기 전에 입력을 복사하는데, `std::string_view`에는 종료 널 문자가 없기 때문입니다. `std::from_chars`의 부동소수점 오버로드를 사용하면 이 복사를 피할 수 있지만, libc++는 이 오버로드를 macOS 26부터만 사용할 수 있는 것으로 표시하므로 이를 사용하는 확장은 이전 대상에 대해 빌드에 실패합니다.
...
완전한 구현은 [vsql_complex 예제](/ko/mysql-8.4/stable/examples)를 참조하세요.

## 함수형 인덱스에서의 VDF

VDF는 함수형 인덱스 표현식에서 사용할 수 있습니다. 생성된 컬럼과 동일한 `.deterministic()` 요구 사항이 적용됩니다. MySQL은 함수형 인덱스를 숨겨진 생성된 컬럼으로 구현하기 때문입니다.
...
최적화 도구는 동일한 VDF 표현식이 `WHERE`, `ORDER BY`, `GROUP BY`에 나타날 때 인덱스를 사용합니다. 비교 값을 VDF의 반환 유형으로 캐스팅하여 최적화 도구가 표현식을 일치시킬 수 있도록 해야 합니다:
...
<CardGroup cols={2}>
  <Card
    title="계속: 확장 빌드"
    icon="hammer"
    href="/ko/mysql-8.4/stable/create#step-5-update-build-configuration"
  >
  ...
  <Card
    title="매개변수화 유형"
    icon="sliders"
    href="/ko/mysql-8.4/stable/type-operations#parameterized-types"
  >
  ...
  <Card
    title="확장 API 참조"
    icon="book"
    href="/ko/mysql-8.4/stable/extension-api-reference"
  >
  ...
  <Card
    ...
    href="/ko/mysql-8.4/stable/managing#replication"
  >
