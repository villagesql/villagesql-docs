...
스크립트는 세 가지를 묻습니다. 어떤 코드베이스를 설치할지, 어떤 방법으로 설치할지, 어떤 버전을 설치할지입니다. 첫 번째 질문에서 **MySQL 8.4** 을 선택하면 이 문서가 설명하는 서버가 설치됩니다.
...
개발 또는 맞춤 빌드를 위해, 최신 코드로 직접 컴파일하려면 [소스에서 복제 및 빌드 가이드](/ko/mysql-8.4/stable/source)를 따르세요.

VillageSQL은 업스트림 MySQL 대신 Percona Server 기반으로도 빌드됩니다 — 이에 관한 내용은 [Percona 빌드](/ko/mysql-8.4/stable/percona) 페이지를 참조하세요.
...
- `vsql_complex`와 `vsql_simple`은 확장 프레임워크의 동작 방식을 보여주기 위해 이 문서의 다른 곳([C++ 확장 예제](/ko/mysql-8.4/stable/examples))에서 사용되는 참조용 확장입니다.

VillageSQL을 소스에서 빌드했다면(옵션 C), `vsql_complex`와 `vsql_simple`도 이미 `veb_dir`에 들어 있습니다 — `make install`이 이들을 무조건 빌드하기 때문입니다. 하지만 `vsql_uuid`와 다른 확장들은 그렇지 않습니다: 이들은 별도의 저장소에 있으며 직접 빌드하고 설치해야 합니다. 옵션 C를 사용했다면, 계속하기 전에 [vsql-uuid](https://github.com/villagesql/vsql-uuid)를 클론해서 빌드하거나, 대신 `vsql_complex`를 사용해 건너뛰어도 됩니다 — 자세한 내용은 [확장 설치](/ko/mysql-8.4/stable/install)를 참조하세요.
...
자세한 내용은 [확장 설치](/ko/mysql-8.4/stable/install)를 참조하세요.
...
<CardGroup cols={2}>
  <Card title="확장 관리" icon="puzzle-piece" href="/ko/mysql-8.4/stable/managing">
    다른 확장을 설치하고 관리하는 방법을 배웁니다.
  </Card>
  <Card title="확장 만들기" icon="code" href="/ko/mysql-8.4/stable/create">
