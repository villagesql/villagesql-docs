...
**오류:** `ERROR 3219 (HY000): VEB file not found: my_extension.veb`
...
**오류:** `Failed to load VEF extension 'extension_name'`

**원인:** `.veb` 파일이 진입점 단계에서 로드에 실패했습니다 — 예를 들어
`vef_register` 심볼이 없거나 유효하지 않은 경우, 또는 `.veb` 파일이 손상된
경우입니다.
...
두 이름은 밑줄 사용을 포함하여 정확히 동일해야 합니다(참고: [확장 이름 규칙](/ko/mysql-8.4/stable/install#extension-naming-conventions)):
...
<Note>
  `ALTER EXTENSION`에는 `EXTENSION_ADMIN` 권한(또는 대체 수단으로 `SUPER`)이
  필요하며, 이는 `INSTALL EXTENSION` 및 `UNINSTALL EXTENSION`과 동일한 요건입니다.
  [필수 권한](/ko/mysql-8.4/stable/install#required-privilege)을 참조하세요.
</Note>
...
두 번째 사전 점검은 테이블 열이 여전히 사용 중인 사용자 정의 유형을 삭제하게
되는 버전을 거부합니다:
...
사용자 정의 유형에는 ROW 형식의 binlog 설정이 권장됩니다. 사용자 정의 유형 열에 대한 INSERT, UPDATE, DELETE, ALTER TABLE 작업은 모두 ROW 형식에서 정상적으로 복제됩니다.
...
<CardGroup cols={2}>
  <Card
    title="시스템 참조"
    icon="book"
    href="/ko/mysql-8.4/stable/reference"
  >
    시스템 테이블 및 뷰 쿼리
  </Card>
  <Card
    title="확장 제거"
    icon="trash"
    href="/ko/mysql-8.4/stable/uninstall"
  >
    확장 안전하게 제거
  </Card>
  <Card
