...
<Warning>
  ...
  compare, hash)이 지원되며, `sys_var`, `status_var`, `thread_worker`,
  `keyring` 미리보기 기능도 지원됩니다. 집계, `VarArgs`, 그리고 컬럼 저장
  ABI는 현재 C++ 전용입니다 — 이 중 어느 것이든 필요하다면
  [C++ SDK](/ko/mysql-8.4/stable/create)를 사용하세요.
</Warning>

이 페이지는 `villagesql` 크레이트 API에 대한 참조입니다. 시작하기 가이드는 [Rust로 확장 만들기](/ko/mysql-8.4/stable/rust-sdk)를 참조하세요. 사용자 정의 타입에 대해서는 [Rust에서의 사용자 정의 타입](/ko/mysql-8.4/stable/rust-custom-types)을 참조하세요.
...
| `CustomWithParams { bytes, params }` | 원시 바이트 + `TypeParams` | [`parameterized_type!`](/ko/mysql-8.4/stable/rust-custom-types#parameterized-types)을 통해 등록된 매개변수화된 사용자 정의 타입 |
...
`requires:`는 확장이 사용하는 [Rust에서의 미리보기 기능](/ko/mysql-8.4/stable/rust-preview-capabilities)을 `static` 기능 객체에 대한 참조로 선언합니다. 이는 `funcs:` 섹션 뒤에 마지막으로 와야 합니다 — 확장이 함수를 등록하지 않는다면 `funcs: []`를 포함하세요.
...
상태를 직접 해제해서는 안 됩니다: `func!`가 문장이 끝날 때 상태를 드롭하는 postrun을 생성합니다. 이는 postrun에서 `delete_state<T>()`를 호출해야 하는 C++ SDK와는 반대입니다 — [문장별 상태](/ko/mysql-8.4/stable/development#per-statement-state-prerun-and-postrun)를 참조하세요.
...
[Rust로 확장 만들기](/ko/mysql-8.4/stable/rust-sdk)에 설명된 대로 빌드하고 설치한 다음:
