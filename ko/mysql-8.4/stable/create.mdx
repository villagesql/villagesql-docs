...
시작하기 전에 VillageSQL을 소스에서 빌드해야 합니다. 확장은 서버의 SDK 헤더와 빌드 트리에 연결됩니다. 먼저 [소스에서 빌드하기](/ko/mysql-8.4/stable/source) 가이드를 따르세요.
...
`buffer()`와 `set_length()`로 값을 직접 빌드할 때 버퍼는 `buffer().size()` 바이트로 고정됩니다 — 그 이상으로 쓰면 메모리가 오버플로되므로 넘어 쓰지 않도록 방지하세요:
...
STRING 결과의 경우 `out.set(sv)`는 snprintf 스타일 오버플로 계약을 따릅니다. 즉, 버퍼에 들어가는 만큼의 바이트를 복사하고 `set_length`를 통해 값의 전체 크기를 보고합니다. 보고된 크기가 버퍼를 초과하면 서버는 결과 버퍼를 늘리고 함수를 다시 호출하므로, 요청된 버퍼보다 큰 STRING 값은 더 이상 잘리지 않습니다.
