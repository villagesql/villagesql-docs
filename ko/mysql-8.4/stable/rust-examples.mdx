...
이 페이지에서는 Rust SDK 리포지토리의 참조 확장 네 가지를 살펴봅니다 — 최소한의 함수 전용 예제, 산술, 정렬, 해싱을 갖춘 완전한 사용자 정의 타입, 집계 함수, 그리고 가변 인자 함수입니다. 리포지토리의 `examples/` 디렉터리에는 지원되는 [미리보기 기능](/ko/mysql-8.4/stable/rust-preview-capabilities)별 예제도 하나씩 들어 있습니다.
