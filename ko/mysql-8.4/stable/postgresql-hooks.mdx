...
미리보기 기능을 통해 **사용 가능**으로 표시된 항목은 `vsql_allow_preview_extensions = ON`이 필요합니다 — [미리보기 기능](/ko/mysql-8.4/stable/preview-capabilities)을 참조하세요.
...
| 백그라운드 워커 | 예 | 예 |
| 키링 접근 | 예 | 예 |
| 집계 함수 | 예 | 아직 미지원 — [rust-sdk#37](https://github.com/villagesql/vsql-rust-sdk/issues/37) |
...
| 사용자 정의 데이터 유형 및 연산자 | 자체 저장 및 비교 동작을 가진 새 기본 유형 등록 | **사용 가능** — [C++에서 사용자 정의 유형 만들기](/ko/mysql-8.4/stable/custom-types) 참조 |
| 집계 함수 | 사용자 정의 집계 등록 | **사용 가능** — `make_aggregate_func`, [C++ 개발](/ko/mysql-8.4/stable/development) 참조 |
