...
<Warning>
  ...
  hash)이 지원되며, `sys_var`, `status_var`, `thread_worker`, `keyring`
  미리보기 기능도 지원됩니다. 집계, `VarArgs`, 컬럼 저장 ABI는 현재
  C++ 전용입니다 — 이러한 기능이 필요하다면
  [C++ SDK](/ko/mysql-8.4/stable/create)를 사용하세요.
</Warning>

<Note>
C++를 선호하는 경우 [C++로 확장 만들기](/ko/mysql-8.4/stable/create)에서 C++ SDK 워크스루를 참조하세요.
</Note>
...
시작하기 전에 소스에서 VillageSQL을 빌드하세요 — 확장은 서버의 빌드 트리에 링크됩니다. 먼저 [소스에서 빌드](/ko/mysql-8.4/stable/source) 가이드를 따르세요.
...
<CardGroup cols={2}>
  <Card
    ...
    icon="shapes"
    href="/ko/mysql-8.4/stable/rust-custom-types"
  >
  ...
  <Card
    ...
    icon="flask"
    href="/ko/mysql-8.4/stable/rust-preview-capabilities"
  >
    requires: 목록을 통한 상태 변수, 시스템 변수, 백그라운드 스레드 워커,
    키링 접근.
  </Card>
  <Card
    title="C++ SDK (확장 만들기)"
  ...
  <Card
    title="네트워크 의존 확장 테스트"
    icon="network-wired"
    href="/ko/mysql-8.4/stable/testing-network"
  >
