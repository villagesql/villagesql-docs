...
이 페이지는 `villagesql` 크레이트 API에 대한 참조입니다. 시작하기 가이드는 [Rust로 확장 빌드](/mysql-8.4/0.0.4/rust-sdk)를 참조하세요. 사용자 정의 타입에 대해서는 [Rust에서 사용자 정의 타입](/mysql-8.4/0.0.4/rust-custom-types)을 참조하세요.
