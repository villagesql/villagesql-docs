...
이 가이드에서는 확장을 빌드하고 설치하는 끝에서 끝까지의 단계를 다룹니다. VDF 구현을 심층적으로 작성하려면 [개발 가이드](/ko/mysql-8.4/0.0.4/development)를 참조하세요.

<Note>
Rust를 선호한다면 [Rust로 확장 빌드하기](/ko/mysql-8.4/0.0.4/rust-sdk)를 참조하세요.
</Note>
...
시작하기 전에 VillageSQL을 소스에서 빌드해야 합니다. 확장은 서버의 SDK 헤더와 빌드 트리에 연결됩니다. 먼저 [소스에서 빌드하기](/ko/mysql-8.4/0.0.4/source) 가이드를 따르세요.
...
SQL, 파일 이름, 리포지토리 이름에 대한 전체 명명 규칙은 [확장 명명 규칙](/ko/mysql-8.4/0.0.4/install#extension-naming-conventions)을 참조하세요.
...
자세한 `CustomArg`/`CustomResult` API(파라미터화된 유형을 위한 `CustomArgWith<P>` 및 `CustomResultWith<P>`)는 [개발 가이드](/ko/mysql-8.4/0.0.4/development#typed-wrappers-recommended)를 참조하세요.
...
<Note>
함수 구현 전에 [확장 API 참조](/ko/mysql-8.4/0.0.4/extension-api-reference)를 검토하여 VDF 계약(널 검사, 결과 유형, 버퍼 크기, 오류 처리)을 완전히 이해하세요.
</Note>
...
자세한 구현은 [사용자 정의 유형 만들기](/ko/mysql-8.4/0.0.4/custom-types)를 참조하세요. 유형이 매개변수를 받는 경우(예: `VECTOR(1536)`) [파라미터화된 유형](/ko/mysql-8.4/0.0.4/development#parameterized-types)을 참조하세요.
