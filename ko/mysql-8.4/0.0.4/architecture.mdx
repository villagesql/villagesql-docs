...
확장 작성자 여정은 간단합니다: VEF SDK를 사용하여 C++ 또는 Rust 함수를 작성하고, 공유 라이브러리로 컴파일한 후, 매니페스트와 함께 `.veb` 파일로 패키징합니다. `INSTALL EXTENSION`을 실행하면 VillageSQL이 라이브러리를 로드하고 등록 코드를 호출하여 함수를 즉시 SQL로 사용할 수 있게 합니다. 서버 재시작 없이도 쿼리에서 내장 함수처럼 호출할 수 있습니다. 이 페이지 나머지 부분은 해당 프로세스의 각 단계가 어떻게 작동하는지 설명합니다.
...
VDF는 명시적 및 비명시적 함수 호출을 모두 지원합니다:
...
**성능:** 핫 코드 경로에서, 해석 체인을 건너뛰고 직접 확장 함수를 호출하기 위해 명시적 호출(`extension_name.function_name()`)을 사용하세요.
...
**실패 시나리오:**
- 테이블 누락 → villagesql_schema.sql에서 생성
