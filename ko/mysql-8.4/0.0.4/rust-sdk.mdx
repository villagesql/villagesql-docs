...
<Warning>
Rust SDK는 버전 0.1.0이며 아직 crates.io에 게시되지 않았습니다. 시작하려면 SDK 리포지토리의 로컬 클론과 `Cargo.toml`의 `path` 종속성이 필요합니다.
</Warning>

<Note>
C++를 선호하는 경우 [확장 만들기](/mysql-8.4/0.0.4/create)에서 C++ SDK 워크스테이프를 참조하세요.
</Note>

VillageSQL 확장은 `vsql` 크레이트를 사용하여 Rust로 작성할 수 있습니다. SDK는 모든 FFI 매싱을 처리합니다 — 서버가 로드 시 호출하는 C 엔트리 포인트는 `extension!` 매크로가 생성합니다.
...
`Cargo.toml`을 편집하여 크레이트 유형을 설정하고 `vsql` 종속성을 추가합니다:
...
경로 종속성 대신 Git 종속성을 선호하는 경우 다음과 같이 사용할 수 있습니다:

```toml
vsql = { git = "https://github.com/villagesql/vsql-rust-sdk" }
```

이것은 기본 브랜치의 최신 커밋에 고정합니다. 특정 커밋에 고정하려면 `rev = ".."`를 사용합니다.
...
함수 서명은 `fn(&[InValue]) -> VdfReturn`입니다. `InValue`는 서버가 전달하는 SQL 타입의 열거형입니다. `VdfReturn`은 반환하는 값입니다. 값을 수정하기 전에 `args.first()`를 확인하여 NULL 경우와 잘못된 타입 경우를 처리하세요.
