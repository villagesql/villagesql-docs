...
성공, NULL, 경고, 오류를 전달하려면 결과 래퍼의 단말 메서드 중 하나를 호출하세요: `out.set(...)` / `out.set_length(n)`, `out.set_null()`, `out.warning(msg)`, 또는 `out.error(msg)`.
...
내장 집계 함수 COUNT(DISTINCT), MIN, MAX, GROUP_CONCAT은 사용자 정의 유형과 즉시 호환됩니다. MIN 및 MAX는 유형에 등록된 비교 함수가 필요합니다.
...
**최선의 실천:** 단순성과 안전성을 위해 함수를 상태 없음으로 설계하세요.
...
트리거는 사용자 정의 유형 열이 있는 테이블에서 발생합니다. 트리거 본문은 `NEW` 및 `OLD`에서 사용자 정의 유형이 아닌 열을 참조할 수 있습니다. 트리거 본문 내에서 사용자 정의 유형 열 값을 접근하는 것은 아직 지원되지 않습니다.
