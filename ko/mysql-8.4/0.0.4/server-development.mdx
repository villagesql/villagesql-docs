...
이 가이드는 GDB 디버깅, 서버 테스트 세트 실행, VillageSQL에 기여하는 방법을 다룹니다. 이 가이드를 사용하기 전에 [소스에서 빌드](/ko/mysql-8.4/0.0.4/source)를 통해 작동하는 빌드를 얻으세요.
...
테스트 출력에 테스트 실행기의 임시 디렉터리 경로가 포함된 경우, 기록된 결과에 다른 머신에서 작동하지 않을 수 있는 절대 경로가 포함되지 않도록 `.test` 파일 내부에 다음 지시문을 추가하세요:
...
확장 회귀 테스트 작성에 대한 자세한 내용은 [개발 가이드](/ko/mysql-8.4/0.0.4/development)를 참조하세요.
...
기본적으로 바이너리가 `/usr/local/mysql/`에 설치됩니다. 설치 접두사(사용자 정의 경로)를 변경할 수 있습니다:
