...
이 가이드는 GDB 디버깅, 서버 테스트 세트 실행, VillageSQL에 기여하는 방법을 다룹니다. 이 가이드를 사용하기 전에 [소스에서 빌드](/mysql-8.4/0.0.4/source)를 통해 작동하는 빌드를 얻으세요.

## GDB로 디버깅

개발 및 문제 해결을 위해 VillageSQL을 GDB 하에서 실행할 수 있습니다.
...
**모든 하위 세트를 포함한 VillageSQL 테스트 실행:**
```bash
...
<Note>
  **--suite vs --do-suite:**
  ...
  - `--do-suite=villagesql`은 하위 세트(삽입, 선택, 저장 프로시저 등)를 포함한 모든 테스트를 실행합니다
</Note>
...
<Warning>
  **테스트 기록:** 항상 `.result` 파일 변경 사항을 검토하여 의도된 동작을 반영했는지 확인하세요. `--record` 플래그는 예상 출력을 맹목적으로 덮어씁니다.
</Warning>

### 테스트 이식성

테스트 출력에 테스트 실행기의 임시 디렉터리 경로가 포함된 경우, 기록된 결과가 다른 머신에서 작동하지 않도록 `.test` 파일 내부에 다음 지시문을 추가하세요:
...
VillageSQL 전용 로그 메시지(`LogVSQL()`을 통해 발행)는 기본적으로 억제됩니다. 테스트 실행 중 오류 로그에 표시되도록 하려면:
...
확장 회귀 테스트 작성에 대한 자세한 내용은 [개발 가이드](/mysql-8.4/0.0.4/development)를 참조하세요.
...
기본적으로 바이너리가 `/usr/local/mysql/`에 설치됩니다. 설치 접두사(customize)를 변경할 수 있습니다:
