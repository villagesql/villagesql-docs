...
사용자 정의 유형을 통해 `COMPLEX`, `UUID`, `VECTOR`와 같은 새 열 유형을 정의할 수 있습니다. 이 유형들은 `ORDER BY`, 인덱스, 집계 함수와 함께 작동합니다. 이 페이지는 [확장 만들기](/ko/mysql-8.4/0.0.4/create) 튜토리얼의 4단계입니다. 계속하기 전에 1~3단계를 완료하세요.
...
전체 C++ 서명은 [유형 연산 빌더](/ko/mysql-8.4/0.0.4/development#type-operation-builders)를 참조하세요.
...
완전한 구현은 [vsql_complex 예제](/ko/mysql-8.4/0.0.4/examples)를 참조하세요.
