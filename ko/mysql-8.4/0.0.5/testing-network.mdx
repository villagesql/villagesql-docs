...
**테스트에서**, MTR의 `wait_condition.inc`를 사용하여 상태 변수가 0이 아닐 때까지 폴링한 다음, 포트를 MTR 변수로 캡처하세요. `UNINSTALL EXTENSION` / `INSTALL EXTENSION` 주기가 있을 때마다 다시 읽으세요 — 임시 포트는 바인딩할 때마다 변경됩니다.
...
**`echo_server.py`** — 서빙하기 전에 포트를 작성하는 실제 서버입니다:
...
런처를 시작한 다음(포그라운드 — MTR은 종료될 때까지 블록합니다), 포트 파일을 소스로 가져오세요:
...
**이름이 아니라 PID로 종료하세요.** Linux에서 `pkill -f echo_server.py`는 `/proc/cmdline`을 통해 MTR 프로세스 자체와 일치할 수 있습니다. 런처가 작성한 PID 파일을 사용하세요:
...
패턴 B의 완전하고 테스트된 구현은 `vsql-http/mysql-test/t/vsql_http_requests.test`를 참조하세요 — `--write_file`부터 정리까지 전체 테스트 생명 주기를 다룹니다.
