...
<Warning>
  ...
  상태 변수, 키링 접근, 그리고 열 저장소 ABI는 현재 C++ 전용입니다 —
  이 중 어느 것이든 필요하다면 [C++ SDK](/ko/mysql-8.4/0.0.5/create)를
  사용하세요.
</Warning>

이 페이지는 `villagesql` 크레이트 API에 대한 참조입니다. 시작하기 가이드는 [Rust로 확장 빌드](/ko/mysql-8.4/0.0.5/rust-sdk)를 참조하세요. 사용자 정의 타입에 대해서는 [Rust에서 사용자 정의 타입](/ko/mysql-8.4/0.0.5/rust-custom-types)을 참조하세요.
