...
사용자 정의 유형을 통해 `COMPLEX`, `UUID`, `VECTOR`와 같은 새 열 유형을 정의할 수 있습니다. 이 유형들은 `ORDER BY`, 인덱스, 집계 함수와 함께 작동합니다. 이 페이지는 [C++에서 확장 만들기](/ko/mysql-9.7/dev/create) 튜토리얼의 4단계입니다. 계속하기 전에 1~3단계를 완료하세요.
...
전체 C++ 서명은 [유형 연산](/ko/mysql-9.7/dev/type-operations)을 참조하세요.
...
완전한 구현은 [vsql_complex 예제](/ko/mysql-9.7/dev/examples)를 참조하세요.
...
<CardGroup cols={2}>
  <Card
    title="계속: 확장 빌드"
    icon="hammer"
    href="/ko/mysql-9.7/dev/create#step-5-update-build-configuration"
  >
  ...
  <Card
    title="매개변수화 유형"
    icon="sliders"
    href="/ko/mysql-9.7/dev/type-operations#parameterized-types"
  >
  ...
  <Card
    title="확장 API 참조"
    icon="book"
    href="/ko/mysql-9.7/dev/extension-api-reference"
  >
  ...
  <Card
    ...
    href="/ko/mysql-9.7/dev/managing#replication"
  >
