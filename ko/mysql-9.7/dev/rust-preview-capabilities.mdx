...
미리보기 기능은 API가 최종화되기 전에 확장에 노출되는 서버 기능입니다. Rust SDK는 그중 네 가지를 감쌉니다: 상태 변수, 시스템 변수, 백그라운드 스레드 워커, 키링 접근입니다. 이 페이지는 각각을 Rust에서 선언하고 사용하는 방법을 다룹니다. 개념, 미리보기 계층, 그리고 C++에만 존재하는 기능에 대해서는 [미리보기 기능](/ko/mysql-9.7/dev/preview-capabilities)을 참조하세요.
...
이 변수에 대해 `SET GLOBAL`이 거부되는 이유를 포함한 자세한 내용은 [미리보기 계층 활성화](/ko/mysql-9.7/dev/preview-capabilities#enabling-the-preview-tier)를 참조하세요.

작동하는 Rust 확장 환경도 필요합니다 — [Rust로 확장 만들기](/ko/mysql-9.7/dev/rust-sdk) 안내서가 툴체인, `cargo-vsql`, 그리고 첫 번째 `extension!` 블록을 다룹니다.
...
나머지 미리보기 기능인 `auth`, `mysql_services`, `sql_query`, `statement_event`, 그리고 열 저장소 ABI는 현재 C++ 전용입니다. 이 중 하나가 필요하다면 [C++ SDK](/ko/mysql-9.7/dev/preview-capabilities)를 사용하세요.
...
<CardGroup cols={2}>
  <Card
    title="미리보기 기능(C++)"
    icon="flask"
    href="/ko/mysql-9.7/dev/preview-capabilities"
  >
  ...
  <Card
    title="Rust API 참조"
    icon="book"
    href="/ko/mysql-9.7/dev/rust-api-reference"
  >
