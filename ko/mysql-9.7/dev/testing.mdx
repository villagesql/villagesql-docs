...
이 가이드는 C++ 확장의 테스트-반복 루프를 다룹니다: 로컬 서버 설정, `.veb` 파일 설치, 회귀 테스트 실행, 실패 디버깅. 초기 빌드를 다루는 [C++로 확장 만들기](/ko/mysql-9.7/dev/create), 그리고 VDF 작성 심층 내용을 다루는 [C++ 개발](/ko/mysql-9.7/dev/development)의 동반 자료입니다.

<Note>
  확장을 빌드하는 것이 아니라 VillageSQL 서버 자체에 기여하는 경우, `mysql-test-run.pl`을 직접 사용하여 테스트를 실행하는 것을 포함한 전체 서버 개발자 워크플로를 다루는 [소스에서 빌드하기](/ko/mysql-9.7/dev/source)를 참조하세요.
</Note>

## 환경 설정

확장을 개발하고 테스트하려면 빌드된 VillageSQL 서버가 필요합니다. [클론 후 소스에서 빌드하기](/ko/mysql-9.7/dev/source) 가이드를 따라 서버 바이너리를 컴파일하세요.
...
- [네트워크 의존 확장 테스트](/ko/mysql-9.7/dev/testing-network) — HTTP 서버나 외부 리스너를 생성하는 확장을 위한 안정적인 MTR 패턴
- [C++로 확장 만들기](/ko/mysql-9.7/dev/create) — 종단간 빌드 단계, CMake 설정, 설치
- [C++ 개발](/ko/mysql-9.7/dev/development) — VDF 작성 심층 내용, 인자 및 결과 타입, 집계, 가변 인자
- [C++ API 참조](/ko/mysql-9.7/dev/extension-api-reference) — VDF 계약, null 처리, 버퍼 크기 조정
