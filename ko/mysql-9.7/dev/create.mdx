...
인수 및 결과 유형, 집계, 시스템 변수, 파라미터화된 유형 등 VDF 구현을 심층적으로 다루는 내용은 [개발 가이드](/ko/mysql-9.7/dev/development)를 참조하세요.

<Note>
Rust를 선호한다면 [Rust로 확장 만들기](/ko/mysql-9.7/dev/rust-sdk)를 참조하세요.
</Note>
...
SQL, 파일 이름, 리포지토리 이름에 대한 전체 명명 규칙은 [확장 명명 규칙](/ko/mysql-9.7/dev/install#extension-naming-conventions)을 참조하세요.
...
자세한 `CustomArg`/`CustomResult` API(파라미터화된 유형을 위한 `CustomArgWith<P>` 및 `CustomResultWith<P>`)는 [개발 가이드](/ko/mysql-9.7/dev/development#argument-and-result-types)를 참조하세요.
...
<Note>
함수 구현 전에 [C++ API 참조](/ko/mysql-9.7/dev/extension-api-reference)를 검토하여 VDF 계약(널 검사, 결과 유형, 버퍼 크기, 오류 처리)을 완전히 이해하세요.
</Note>
...
자세한 구현은 [C++로 사용자 정의 유형 만들기](/ko/mysql-9.7/dev/custom-types)를 참조하세요. 유형이 매개변수를 받는 경우(예: `VECTOR(1536)`) [파라미터화된 유형](/ko/mysql-9.7/dev/type-operations#parameterized-types)을 참조하세요.
...
<CardGroup cols={2}>
  ...
  <Card
    title="개발 가이드"
    icon="code"
    href="/ko/mysql-9.7/dev/development"
  >
