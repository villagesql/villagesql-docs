...
이 가이드는 C++ VDF 구현을 작성하기 위한 심층 참조입니다. 종단간 빌드 단계를 다루는 [C++로 확장 만들기](/ko/mysql-9.7/dev/create), 그리고 테스트-반복 루프를 다루는 [C++ 테스트](/ko/mysql-9.7/dev/testing)의 동반 자료입니다.
...
액세서를 추가합니다(참조: [매개변수화된 타입](/ko/mysql-9.7/dev/type-operations#parameterized-types)).
...
[사용자 정의 버퍼 크기](/ko/mysql-9.7/dev/create#custom-buffer-sizes)를
참조하세요.
...
전체 타입 연산 빌더 참조 — 인코딩, 디코딩, 비교, 해시, 내장 기본값, 매개변수화된 타입 — 는 [타입 연산](/ko/mysql-9.7/dev/type-operations)을 참조하세요.

## 미리보기 기능

다음 VEF 기능은 선택적으로 활성화하는 미리보기 헤더로 사용할 수 있습니다. ABI 및 API는 여전히 활발히 개발 중입니다. 전체 참조는 [미리보기 기능](/ko/mysql-9.7/dev/preview-capabilities)을 참조하세요.

- **확장 시스템 변수** — [미리보기 기능 → 시스템 변수](/ko/mysql-9.7/dev/preview-capabilities#system-variables)
- **확장 상태 변수** — [미리보기 기능 → 상태 변수](/ko/mysql-9.7/dev/preview-capabilities#status-variables)
- **키링 액세스** — [미리보기 기능 → 키링 액세스](/ko/mysql-9.7/dev/preview-capabilities#keyring-access)
- **컬럼 저장** — [미리보기 기능 → 컬럼 저장](/ko/mysql-9.7/dev/preview-capabilities#column-storage)
...
- [C++로 확장 만들기](/ko/mysql-9.7/dev/create) — 종단간 빌드 단계, CMake 설정 및 설치
- [C++ 테스트](/ko/mysql-9.7/dev/testing) — 로컬 개발 서버, MTR 및 실패 디버깅
- [타입 연산](/ko/mysql-9.7/dev/type-operations) — 인코딩, 디코딩, 비교, 해시, 매개변수화된 타입
- [C++ API 참조](/ko/mysql-9.7/dev/extension-api-reference) — VDF 계약, null 처리 및 버퍼 크기 조정
- [확장 아키텍처](/ko/mysql-9.7/dev/architecture) — 라이프사이클, Victionary 캐싱, 성능 패턴 및 보안 모델
