...
이 가이드는 디버깅, 서버 테스트 스위트 실행, VillageSQL에 기여하는 방법을 다룹니다. 작동하는 소스 빌드를 전제로 합니다 — 아직 하지 않았다면 [소스에서 빌드](/ko/mysql-9.7/dev/source)를 참조하세요.
...
확장 회귀 테스트 작성에 대한 자세한 내용은 [개발 가이드](/ko/mysql-9.7/dev/development)를 참조하세요.
