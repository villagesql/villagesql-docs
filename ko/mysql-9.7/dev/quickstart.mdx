...
- `vsql_complex`와 `vsql_simple`은 확장 프레임워크의 동작 방식을 보여주기 위해 이 문서의 다른 곳([C++ 확장 예제](/ko/mysql-9.7/dev/examples))에서 사용되는 참조용 확장입니다.

VillageSQL을 소스에서 빌드했다면(옵션 C), `vsql_complex`와 `vsql_simple`도 이미 `veb_dir`에 들어 있습니다 — `make install`이 이들을 무조건 빌드하기 때문입니다. 하지만 `vsql_uuid`와 다른 확장들은 그렇지 않습니다: 이들은 별도의 저장소에 있으며 직접 빌드하고 설치해야 합니다. 옵션 C를 사용했다면, 계속하기 전에 [vsql-uuid](https://github.com/villagesql/vsql-uuid)를 클론해서 빌드하거나, 대신 `vsql_complex`를 사용해 건너뛰어도 됩니다 — 자세한 내용은 [확장 설치](/ko/mysql-9.7/dev/install)를 참조하세요.
