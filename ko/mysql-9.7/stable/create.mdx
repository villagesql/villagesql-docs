...
title: "C++로 확장 만들기"
description: "확장 템플릿과 VEF를 사용하여 C++로 사용자 정의 VillageSQL 확장을 만드는 방법을 배우세요."
---

<Warning>
VEF Protocol 3은 v0.0.4부터 안정적입니다. Protocol 4는 개발 중이며, `-DVSQL_USE_DEV_ABI=ON` 옵션을 사용해야만 개발 ABI 헤더를 통해 사용할 수 있습니다. 이전 Protocol 2로 빌드된 확장은 서버에서 거부되며 재빌드해야 합니다.
</Warning>

## 개요

VillageSQL의 확장 프레임워크(VEF)를 사용하면 데이터베이스 서버에 사용자 정의 기능을 추가할 수 있습니다. 이 가이드에서는 C++ SDK와 확장 템플릿을 사용하여 C++로 확장을 빌드하는 과정을 안내합니다.

인수 및 결과 유형, 집계, 시스템 변수, 파라미터화된 유형 등 VDF 구현을 심층적으로 다루는 내용은 [개발 가이드](/ko/mysql-9.7/stable/development)를 참조하세요.

<Note>
Rust를 선호한다면 [Rust로 확장 만들기](/ko/mysql-9.7/stable/rust-sdk)를 참조하세요.
</Note>
...
C++ 확장은 VEF(VillageSQL Extension Framework)의 C++ 바인딩인 **C++ SDK**를 사용하여 빌드됩니다. 이 SDK는 다음과 같은 기능을 제공합니다:
- 유형과 함수 정의를 위한 C++ API
- SQL 스크립트 없이 자동 등록
- 유형 안전한 인수 및 결과 유형
- 확장 정의를 위한 빌더 패턴
...
<Note>
**VDF vs 기존 UDF:** VEF를 통해 등록된 함수는 VDF(VillageSQL Defined Functions)라고 불립니다. VillageSQL은 `CREATE FUNCTION ... SONAME`을 통해 등록되는 기존 MySQL UDF도 지원하지만, 새로운 확장에는 VDF를 권장합니다.
</Note>
...
시작하기 전에 VillageSQL 확장 SDK가 필요하며, CMake가 SDK를 찾도록 하는 방법은 두 가지입니다. VillageSQL 빌드 트리가 있다면 `-DVillageSQL_BUILD_DIR=/path/to/villagesql/build`를 전달하세요. [소스에서 빌드](/ko/mysql-9.7/stable/source) 가이드가 그 트리를 생성합니다. 압축을 푼 SDK 디렉터리(`include/villagesql/vsql.h`를 포함하는 디렉터리)가 있다면 그것만으로 충분합니다. `-DVillageSQL_SDK_DIR=/path/to/sdk`를 전달하세요. 서버를 빌드하지 않고 SDK를 얻는 방법은 [확장 템플릿 README](https://github.com/villagesql/vsql-extension-template)에서 다룹니다. 빌드 트리 방식만 VEB 설치 디렉터리를 설정하므로, SDK만 사용하는 경우에는 빌드된 VEB 파일을 `SHOW VARIABLES LIKE 'veb_dir'`가 보고하는 디렉터리로 직접 복사하세요.

다음도 필요합니다:

- **Git** - 복제 및 버전 관리
- **CMake** 3.18 이상 - 빌드 시스템
- **C++ 컴파일러** - C++17을 지원해야 합니다. 템플릿은 `CMAKE_CXX_STANDARD 17`을 설정하며, SDK의 CMake 패키지는 그보다 낮은 표준을 거부합니다
- **기본 C++ 지식** - C++ 및 함수 포인터 이해

<Tip>
  **AI 에이전트로 빌드 중이신가요?** [`vsql-extension-builder`](https://github.com/villagesql/villagesql-skills) 스킬은 클로드 코드, 제미나이 등 지원되는 에이전트를 사용하여 전체 워크플로우(스캐폴딩부터 테스트까지)를 자동화합니다. 다음으로 설치하세요:
...
```json
{
  "$schema": "https://raw.githubusercontent.com/villagesql/villagesql-docs/main/mysql-9.7/stable/manifest.json.schema.json",
...
SQL, 파일 이름, 리포지토리 이름에 대한 전체 명명 규칙은 [확장 명명 규칙](/ko/mysql-9.7/stable/install#extension-naming-conventions)을 참조하세요.

## 단계 3: C++ SDK로 확장 구현

C++ SDK는 플루언트 빌더 패턴을 사용하여 확장을 정의하는 C++ API를 제공합니다:
...
- `.max_result_length(size_t)` - STRING 반환 시 결과 열의 크기를 지정하여 구체화된 결과가 인수 폭에서 잘리지 않도록 합니다. `VEF_MAX_RESULT_LENGTH`(16 MiB)를 초과하는 값은 서버에 의해 제한됩니다. STRING에만 해당하며, 먼저 `.returns(STRING)`을 호출해야 합니다. Protocol 4(개발 ABI)가 필요합니다.
...
VDF는 빌더에서 `.param(TYPE_NAME)` 및 `.returns(TYPE_NAME)`을 사용하여 사용자 정의 유형 값을 입력 및 반환할 수 있습니다. 구현은 입력에 `CustomArg`, 출력에 `CustomResult`를 사용합니다 — 유형 연산에 사용되는 동일한 인수 및 결과 유형입니다:
...
자세한 `CustomArg`/`CustomResult` API(파라미터화된 유형을 위한 `CustomArgWith<P>` 및 `CustomResultWith<P>`)는 [개발 가이드](/ko/mysql-9.7/stable/development#argument-and-result-types)를 참조하세요.
...
최적화기는 이 정보를 사용하여 행마다 호출하는 대신 문장당 한 번만 함수를 평가하고 행 간에 결과를 재사용할 수 있습니다. 비결정적 함수를 잘못 결정적으로 표시하면 서버가 실제로 다른 출력을 생성해야 하는 입력에 대해 동일한 결과를 반환할 수 있습니다. 외부 상태, 난수, 시간에 의존하지 않는 경우에만 `.deterministic()`을 추가하세요.
...
### 사용자 정의 버퍼 크기 {#custom-buffer-sizes}

가변 길이 데이터를 반환하는 함수의 경우 특정 버퍼 크기 요청:
...
`buffer()`와 `set_length()`로 값을 직접 빌드할 때 `buffer().size()`를 넘어서 쓰지 마세요 — 메모리가 오버플로됩니다. 하지만 값이 너무 크다고 해서 실패하지도 마세요. 쓰려고 했던 크기를 보고하고, 서버가 버퍼를 늘려 함수를 다시 호출하도록 하세요.
...
STRING 결과의 경우 `out.set(sv)`는 snprintf 스타일 오버플로 계약을 따릅니다. 즉, `out.buffer()`에 들어가는 만큼의 바이트를 복사하되 값의 *전체* 크기를 보고합니다. 서버는 보고된 길이가 버퍼를 초과함을 감지하고 버퍼를 늘린 다음 함수를 두 번째로 호출하므로, `.buffer_size()`보다 큰 값도 잘리지 않고 전체가 반환됩니다. 보고된 길이가 필요한 버퍼 크기를 정확히 알려주기 때문에 재시도는 항상 한 번으로 충분합니다.

버퍼 증가는 `max_allowed_packet`으로 제한됩니다. 그보다 큰 값은 잘리지 않고 `NULL`로 반환됩니다.

`out.buffer()`에 직접 쓰고 `out.set_length()`를 호출하는 경우에도 동일한 계약이 적용됩니다. 들어간 바이트 수가 아니라 쓰려고 했던 전체 크기를 보고하세요. 잘린 길이를 보고하면 재시도가 무효화되고 나머지가 조용히 버려집니다.

이 계약은 행 단위로 평가되는 시점에 적용됩니다. *구체화된* STRING 결과 — GROUP BY/DISTINCT 임시 테이블, `CREATE TABLE ... SELECT`, 또는 UNION 내의 결과 — 는 함수가 `.max_result_length(n)`을 선언하지 않는 한 여전히 인수 폭에서 잘립니다. `.max_result_length(n)`은 결과 열의 크기를 지정합니다(문자 단위, `VEF_MAX_RESULT_LENGTH`인 16 MiB로 제한):
...
<Note>
`.buffer_size()`를 통해 함수의 최대 출력 크기에 기반하여 충분한 버퍼 크기를 요청하세요. 버퍼 크기를 올바르게 지정하면 늘리고 다시 시도하는 왕복 비용을 피할 수 있습니다.
</Note>

<Note>
함수 구현 전에 [C++ API 참조](/ko/mysql-9.7/stable/extension-api-reference)를 검토하여 VDF 계약(널 검사, 결과 유형, 버퍼 크기, 오류 처리)을 완전히 이해하세요.
</Note>
...
자세한 구현은 [C++로 사용자 정의 유형 만들기](/ko/mysql-9.7/stable/custom-types)를 참조하세요. 유형이 매개변수를 받는 경우(예: `VECTOR(1536)`) [파라미터화된 유형](/ko/mysql-9.7/stable/type-operations#parameterized-types)을 참조하세요.

## 단계 5: 빌드 구성 업데이트 {#step-5-update-build-configuration}
...
<CardGroup cols={2}>
  ...
  <Card
    title="개발 가이드"
    icon="code"
    href="/ko/mysql-9.7/stable/development"
  >
  ...
  <Card
    title="소스에서 빌드하기"
    icon="hammer"
    href="/ko/mysql-9.7/stable/source"
  >
...
- [MySQL UDF API 문서](https://dev.mysql.com/doc/extending-mysql/9.7/en/adding-loadable-function.html)
