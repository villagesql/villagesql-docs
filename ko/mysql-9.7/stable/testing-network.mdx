...
`vsql-http/mysql-test/t/vsql_http_requests.test`는 동일한 목표(백그라운드 헬퍼와 경합 없는 깔끔한 정리)를 다른 구체적인 방식으로 보여줍니다. 위에 나온 `launcher.py`/`echo_server.py` 분리 대신, 하나의 Python 파일이 `--serve` 플래그로 자기 자신을 재실행하여 분리된 자식 프로세스가 됩니다. PID 파일은 쓰지 않으며, 자식 프로세스가 테스트의 정리 단계에서 실행할 `stop_echo.sh` 스크립트(`kill <pid>`)를 직접 작성합니다.

## 참고

- [C++ 테스트](/ko/mysql-9.7/stable/testing) — MTR 설정, 스위트 실행, 결과 기록, 실패 디버깅
- [C++로 확장 만들기](/ko/mysql-9.7/stable/create) — 엔드투엔드 빌드 단계 및 CMake 설정
- [C++ 개발](/ko/mysql-9.7/stable/development) — VDF 작성, 인수 유형, 결과 처리
