...
description: "Rust SDK에서 VillageSQL 미리보기 기능을 사용합니다 — extension!의 requires: 목록으로 상태 변수, 시스템 변수, 백그라운드 스레드 워커, SQL 쿼리, 키링 접근을 선언합니다."
---
...
미리보기 기능은 API가 최종화되기 전에 확장에 노출되는 서버 기능입니다. Rust SDK는 그중 다섯 가지를 감쌉니다: 상태 변수, 시스템 변수, 백그라운드 스레드 워커, 워커 스레드에서의 SQL 쿼리, 키링 접근입니다. 이 페이지는 각각을 Rust에서 선언하고 사용하는 방법을 다룹니다. 개념, 미리보기 계층, 그리고 C++에만 존재하는 기능에 대해서는 [미리보기 기능](/ko/mysql-9.7/stable/preview-capabilities)을 참조하세요.
...
이 변수에 대해 `SET GLOBAL`이 거부되는 이유를 포함한 자세한 내용은 [미리보기 계층 활성화](/ko/mysql-9.7/stable/preview-capabilities#enabling-the-preview-tier)를 참조하세요.

작동하는 Rust 확장 환경도 필요합니다 — [Rust로 확장 만들기](/ko/mysql-9.7/stable/rust-sdk) 안내서가 툴체인, `cargo-vsql`, 그리고 첫 번째 `extension!` 블록을 다룹니다.
...
나머지 미리보기 기능인 `auth`, `mysql_services`, `statement_event`, 그리고 열 저장소 ABI는 현재 C++ 전용입니다. 이 중 하나가 필요하다면 [C++ SDK](/ko/mysql-9.7/stable/preview-capabilities)를 사용하세요.
...
`ThreadHandle` 인자는 참조로 전달됩니다. `villagesql::preview::sql_query`의 `SqlQueryCapability::open`에 전달하면 워커 안에서 SQL을 실행할 수 있습니다.
...
<CardGroup cols={2}>
  <Card
    title="미리보기 기능(C++)"
    icon="flask"
    href="/ko/mysql-9.7/stable/preview-capabilities"
  >
    전체 기능 인덱스, 미리보기 계층, 그리고 C++ 전용 기능: auth,
    mysql_services, statement_event, 그리고 열 저장소.
  </Card>
  <Card
    title="Rust API 참조"
    icon="book"
