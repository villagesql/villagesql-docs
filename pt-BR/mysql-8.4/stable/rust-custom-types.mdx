...
<Warning>
  ...
  compare, hash) têm suporte, assim como as capabilities Preview `sys_var`,
  `status_var`, `thread_worker` e `keyring`. Agregações, `VarArgs` e a ABI de
  armazenamento em coluna são
  exclusivos de C++ hoje — use o [Criando Extensões em C++](/pt-BR/mysql-8.4/stable/create) se você
