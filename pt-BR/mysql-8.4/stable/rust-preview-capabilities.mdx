...
description: "Use as capabilities Preview do VillageSQL a partir do SDK Rust — declare variáveis de status, variáveis de sistema, threads de trabalho em segundo plano, consultas SQL e acesso ao keyring com a lista requires: em extension!."
---
...
suas APIs sejam finalizadas. O SDK Rust envolve cinco delas: variáveis de
status, variáveis de sistema, threads de trabalho em segundo plano, consultas
SQL a partir de uma thread de trabalho e acesso ao keyring. Esta página cobre como declarar e usar cada uma a partir do Rust. Para o
...
Consulte [Habilitando o Nível Preview](/pt-BR/mysql-8.4/stable/preview-capabilities#enabling-the-preview-tier)
...
a passo [Criando Extensões em Rust](/pt-BR/mysql-8.4/stable/rust-sdk) cobre a
...
As demais capabilities Preview — `auth`, `mysql_services`,
`statement_event` e a ABI de armazenamento de coluna — são exclusivas do C++
hoje. Use o [SDK C++](/pt-BR/mysql-8.4/stable/preview-capabilities) se você
...
O parâmetro `ThreadHandle` chega por referência; passe-o para
`SqlQueryCapability::open`, em `villagesql::preview::sql_query`, para executar
SQL de dentro do worker.
...
<CardGroup cols={2}>
  <Card
    ...
    icon="flask"
    href="/pt-BR/mysql-8.4/stable/preview-capabilities"
  >
    O índice completo de capabilities, o nível Preview e as capabilities
    exclusivas do C++: auth, mysql_services, statement_event e armazenamento
    de coluna.
  </Card>
  <Card
