...
de agregação. Esta página é o Passo 4 do tutorial [Criando Extensões em C++](/pt-BR/mysql-8.4/stable/create).
...
Consulte [Operações de Tipo](/pt-BR/mysql-8.4/stable/type-operations) para as assinaturas C++ completas.
...
A decodificação usa `std::to_chars` em vez de `snprintf`: um formato de precisão fixa
como `"%.6f"` descarta dígitos de que um `double` precisa, de modo que `0.1234567891` é lido de volta como
`0.123457`. A codificação copia a entrada antes de chamar `strtod`, porque
`std::string_view` não carrega um null terminador. As sobrecargas de ponto flutuante de
`std::from_chars` evitariam a cópia, mas a libc++ as marca como disponíveis apenas
a partir do macOS 26, portanto uma extensão que as utiliza falha ao compilar para alvos
anteriores.
...
Consulte [Exemplo vsql_complex](/pt-BR/mysql-8.4/stable/examples) para a implementação completa.
...
<CardGroup cols={2}>
  <Card
    ...
    icon="hammer"
    href="/pt-BR/mysql-8.4/stable/create#step-5-update-build-configuration"
  >
  ...
  <Card
    ...
    icon="sliders"
    href="/pt-BR/mysql-8.4/stable/type-operations#parameterized-types"
  >
  ...
  <Card
    ...
    icon="book"
    href="/pt-BR/mysql-8.4/stable/extension-api-reference"
  >
  ...
  <Card
    ...
    href="/pt-BR/mysql-8.4/stable/managing#replication"
  >
