...
O arquivo `vsql-http/mysql-test/t/vsql_http_requests.test` demonstra o mesmo objetivo — um processo auxiliar em segundo plano com um encerramento limpo e livre de condições de corrida — por meio de um mecanismo concreto diferente: um único arquivo Python que faz re-exec de si mesmo com uma flag `--serve` para se tornar o processo filho desanexado, em vez da divisão entre `launcher.py` e `echo_server.py` mostrada acima. Ele não escreve nenhum arquivo de PID; o processo filho escreve seu próprio script `stop_echo.sh` (`kill <pid>`) para o teste executar na limpeza.

## Veja também

- [Testes em C++](/pt-BR/mysql-8.4/stable/testing) — configuração do MTR, execução de suítes, registro de resultados e depuração de falhas
- [Criando Extensões em C++](/pt-BR/mysql-8.4/stable/create) — passos completos de compilação e configuração do CMake
- [Desenvolvimento em C++](/pt-BR/mysql-8.4/stable/development) — criação de VDF, tipos de argumento e tratamento de resultados
