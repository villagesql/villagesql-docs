...
<Warning>
O Protocolo 3 do VEF é estável a partir da v0.0.4. O Protocolo 4 está em desenvolvimento e disponível apenas por meio de cabeçalhos opcionais da dev ABI (`-DVSQL_USE_DEV_ABI=ON`). Extensões compiladas com o antigo Protocolo 2 são rejeitadas pelo servidor e precisam ser recompiladas.
</Warning>
...
- **Conhecimento básico de C++** - Compreensão de C++ e ponteiros de função
...
- `make_func<&impl>("name")` - Cria a função com o ponteiro de implementação
...
- `.max_result_length(size_t)` - Dimensiona a coluna de resultado para um retorno STRING para que um resultado materializado não seja truncado na largura do argumento. Valores acima de `VEF_MAX_RESULT_LENGTH` (16 MiB) são limitados pelo servidor. Somente STRING; chame `.returns(STRING)` primeiro. Requer o Protocolo 4 (dev ABI).
- `.deterministic(bool = true)` - Declara que esta função sempre retorna a mesma saída para as mesmas entradas e não tem efeitos colaterais. O padrão é não determinística.
