...
title: "Capacidades de Prévia"
description: "As capacidades de prévia dão às extensões acesso a recursos do servidor que ainda estão se estabilizando. Esta página cobre a habilitação do nível de prévia, o keyring e as capacidades status_var, sys_var, thread_worker, sql_query e statement_event, além de padrões de registro."
---

As capacidades de prévia são recursos fornecidos pelo servidor e expostos às
extensões antes que suas APIs sejam finalizadas. Uma extensão que declara uma
capacidade de prévia requer `vsql_allow_preview_extensions = ON` para instalar
(consulte [Habilitando o Nível de Prévia](#enabling-the-preview-tier)) — as
extensões que não usam capacidades de prévia instalam normalmente,
independentemente dessa configuração.

<Warning>
As APIs das capacidades de prévia não são estáveis. Uma extensão compilada
contra uma capacidade de prévia pode falhar ao carregar após uma atualização do
servidor. Quando uma capacidade se estabiliza, seu cabeçalho é movido para um
caminho versionado do SDK C++ estável.
</Warning>

## Habilitando o Nível de Prévia {#enabling-the-preview-tier}

Defina `vsql_allow_preview_extensions = ON` com `SET PERSIST` antes de instalar
qualquer extensão que use uma capacidade de prévia:
...
capacidades de prévia são carregadas na inicialização, então a variável deve
...
Isso falha se qualquer extensão que usa uma capacidade de prévia estiver
instalada no momento. Desinstale essas extensões primeiro e então desative a
configuração.

## Índice de Capacidades

| Capacidade | Cabeçalho | Status |
|---|---|---|
| `vsql::preview::column_store` | `<villagesql/preview/storage_builder.h>` | Prévia |
| `vsql::preview::keyring` | `<villagesql/preview/keyring.h>` | Prévia |
| `vsql::preview::sql_query` | `<villagesql/preview/sql_query.h>` | Prévia |
| `vsql::preview::statement_event` | `<villagesql/preview/statement_event.h>` | Prévia |
| `vsql::preview::status_var` | `<villagesql/preview/status_var.h>` | Prévia |
| `vsql::preview::storage` | `<villagesql/preview/storage_builder.h>` | Prévia |
| `vsql::preview::sys_var` | `<villagesql/preview/sys_var.h>` | Prévia |
| `vsql::preview::thread_worker` | `<villagesql/preview/thread_worker.h>` | Prévia |

## Padrão de Registro

Para usar uma capacidade de prévia, declare um objeto de capacidade por valor no
escopo do arquivo e passe-o por referência para `.with()` dentro de
`make_extension()`. O servidor popula o ponteiro `abi` do objeto durante o
registro:
...
`.with(capability)` informa ao servidor quais capacidades a extensão requer. Se
`vsql_allow_preview_extensions` estiver em OFF quando a extensão for instalada, o
servidor rejeita a instalação com um erro que nomeia a capacidade.

<Warning>
Todo objeto de capacidade declarado em uma extensão deve ser passado para
`.with()` exatamente uma vez. No momento do carregamento, o servidor verifica
cada instância de capacidade declarada em relação ao que `.with()` recebeu e faz
...
- **Objeto passado para `.with()` não é uma capacidade:**
...
A capacidade keyring (`vsql::preview::keyring`) permite que as extensões leiam e
...
O nome da capacidade `VEF_PREVIEW_KEYRING_NAME` é `"vsql::preview::keyring"`.
...
Inclua o cabeçalho, declare um objeto de capacidade no escopo do arquivo e
passe-o para `.with()`:
...
A capacidade status_var (`vsql::preview::status_var`) permite que uma extensão
...
ponteiros cada vez que a variável de status é consultada.

Construa a capacidade com `vsql::preview_status_var::make_capability()`, passando
...
A capacidade sys_var (`vsql::preview::sys_var`) permite que uma extensão registre
...
Construa a capacidade com `vsql::preview_sys_var::make_capability()` e as funções
de fábrica correspondentes `make_bool`, `make_int` e `make_str`. O objeto de
capacidade também expõe `get()` e `set()` para acesso programático a partir do
...
O objeto de capacidade deve ter duração de armazenamento estática. O MySQL
escreve diretamente nos ponteiros de armazenamento quando o usuário define uma
variável.
...
Para variáveis INT e BOOL, leia o ponteiro de armazenamento global diretamente —
...
A capacidade thread worker (`vsql::preview::thread_worker`) permite que uma
...
O nome da capacidade `VEF_PREVIEW_THREAD_WORKER_NAME` é
...
`VEF_WAKEUP_ENABLE` — veja abaixo). `arg` é o ponteiro opaco registrado no
...
A capacidade sql_query (`vsql::preview::sql_query`) permite que uma extensão
execute instruções SQL a partir de uma thread em segundo plano. As consultas são
executadas dentro do servidor através da vtable da capacidade — as extensões não
são vinculadas a nenhuma biblioteca cliente do MySQL.

O nome da capacidade `VEF_PREVIEW_SQL_QUERY_NAME` é
...
de usá-la; uma `Session` inválida indica que a vtable da capacidade não foi
...
residir em páginas dedicadas. Este é um recurso de capacidade: ele habilita novos
layouts de armazenamento, não é um botão de ajuste para os existentes.

<Warning>
  O column storage é uma ABI de prévia — em desenvolvimento ativo e pode mudar
...
Duas capacidades de prévia trabalham juntas:
...
A capacidade statement event (`vsql::preview::statement_event`) executa um handler
...
O nome da capacidade `VEF_PREVIEW_STATEMENT_EVENT_NAME` é
...
execução excede um limiar, combinando a capacidade statement event com
...
Com o nível de prévia habilitado (consulte
[Habilitando o Nível de Prévia](#enabling-the-preview-tier)), instale a extensão
