...
Algumas capacidades do VEF estão disponíveis como headers opcionais (opt-in) sob
...
O layout de namespace sob `vsql::preview` é por capacidade, não havendo um único
...
Para a documentação completa da API de prévia, consulte [Capacidades de Prévia](/pt-BR/mysql-8.4/0.0.5/preview-capabilities).

<Warning>
Os headers de prévia não são estáveis. Uma extensão compilada com eles pode quebrar
