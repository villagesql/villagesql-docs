...
<Warning>
  Os tipos personalizados usam o VEF Protocolo 3, estável a partir da v0.0.4. O
  Protocolo 4 está em desenvolvimento e está disponível apenas por meio dos
  cabeçalhos opt-in da dev ABI (`-DVSQL_USE_DEV_ABI=ON`). Extensões compiladas
  contra o antigo Protocolo 2 são rejeitadas pelo servidor e precisam ser
  recompiladas.
</Warning>
...
ausente. Cada método de template valida a assinatura do ponteiro de função com
`static_assert`.

O nome do tipo é passado como um parâmetro de template que não é de tipo (NTTP).
Declare-o como um array `static constexpr const char[]` — a identidade do ponteiro
é usada como chave para buffers independentes de nomes de VDF, portanto dois tipos
que compartilham um ponteiro de função ainda obtêm nomes gerados automaticamente
separados.
