...
title: "API do Protocolo 1"
description: "Referência para a API de tipos baseada em ponteiros de função do Protocolo 1 e o estilo de ABI bruta — a interface original do VEF, estável desde a v0.0.1 e que será removida após a 0.0.5."
---

O Protocolo 1 é a interface original do VEF. Ele é estável desde a v0.0.1 e será removido após a 0.0.5. Novas extensões devem usar a API do Protocolo 3 — os construtores de tipos baseados em templates dos guias [Criando Extensões em C++](/pt-BR/mysql-8.4/0.0.5/create) e [Desenvolvimento em C++](/pt-BR/mysql-8.4/0.0.5/development).

Esta página existe como referência ao trabalhar com extensões existentes compiladas com o Protocolo 1.

## API de Tipos Baseada em Ponteiros de Função

Os tipos personalizados do Protocolo 1 são registrados usando ponteiros de função explícitos em vez do template `vsql::make_type<>`. As assinaturas das funções diferem das equivalentes do Protocolo 3 — elas recebem ponteiros brutos e comprimentos em vez de objetos `Arg` e `Result`.
...
`vsql::make_type<kMyTypeName>()` (com um NTTP de string em tempo de compilação) é a forma preferida. `make_type(MYTYPE)` (sem parâmetro de template) é a forma do Protocolo 1 — o suporte a ela será removido após a 0.0.5.

## Estilo de ABI Bruta (Funções)

As implementações de VDF do Protocolo 1 podem passar as structs C brutas diretamente em vez de usar wrappers tipados. Este estilo será removido após a 0.0.5 — use a [API tipada de argumento/resultado](/pt-BR/mysql-8.4/0.0.5/development#argument-and-result-types) para todo código novo.
...
No Protocolo 3, `out.set()`, `out.set_null()`, `out.warning()` e `out.error()` tratam disso automaticamente.

### Registro de Agregados no Protocolo 1

Para agregados, `.clear<>()` e `.accumulate<>()` aceitam assinaturas de ponteiros de função de ABI bruta. `.prerun<>()` e `.postrun<>()` não aceitam — o construtor exige assinaturas tipadas (`void(PrerunArgs, PrerunResult)` e `void(PostrunArgs)`) mesmo para extensões do Protocolo 1. Use a [abordagem tipada de agregados](/pt-BR/mysql-8.4/0.0.5/development#aggregate-vdfs) para todo código novo.
