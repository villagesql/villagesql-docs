...
estiver ausente. Cada método de template verifica a assinatura do ponteiro de função via
`static_assert`.
...
<Warning>
`.max_persisted_length()` requer o VEF Protocolo 3 ou superior. Um tipo que o utiliza
não pode ser carregado por servidores anteriores ao Protocolo 3.
</Warning>
...
<Warning>
`.variable_length_type()` eleva o protocolo exigido pelo tipo para o VEF Protocolo 4.
O servidor lê a flag `variable_length` apenas no protocolo 4 ou superior. Compile
...
`.variable_length_type()` é monotônico: chamá-lo antes ou depois dos setters do Protocolo 3
(`max_persisted_length()`, `params()`, `int_to_params()`) não reduz
a exigência de protocolo de volta para abaixo do Protocolo 4.
