...
**Sobrecarga de memória:** cada entrada confirmada é um nó `std::map<std::string, std::shared_ptr<EntryType>>` mais o payload do descritor — nenhum valor fixo por entrada é registrado no código-fonte.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="lightbulb"
    href="/pt-BR/mysql-9.7/stable/examples"
  >
