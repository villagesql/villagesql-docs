...
Esta página é uma referência da API do crate `villagesql`. Para o tutorial de introdução, consulte [Criando Extensões em Rust](/pt-BR/mysql-9.7/stable/rust-sdk). Para tipos personalizados, consulte [Tipos Personalizados em Rust](/pt-BR/mysql-9.7/stable/rust-custom-types).
...
| `CustomWithParams { bytes, params }` | Bytes brutos + `TypeParams` | Um tipo personalizado parametrizado registrado via [`parameterized_type!`](/pt-BR/mysql-9.7/stable/rust-custom-types#parameterized-types) |
...
`requires:` declara as [capabilities Preview](/pt-BR/mysql-9.7/stable/rust-preview-capabilities) que a extensão usa, como referências a objetos `static` de capability. Ela deve vir por último, depois de uma seção `funcs:` — inclua `funcs: []` se a extensão não registrar nenhuma função.
...
Você não deve liberar o estado por conta própria: `func!` gera a postrun que o descarta quando a instrução termina. Isso é o oposto do SDK C++, onde sua postrun tem que chamar `delete_state<T>()` — consulte [Estado por Instrução](/pt-BR/mysql-9.7/stable/development#per-statement-state-prerun-and-postrun).
...
Compile e instale a extensão conforme descrito em [Criando Extensões em Rust](/pt-BR/mysql-9.7/stable/rust-sdk), e então:
...
<Warning>
  ...
  varargs](/pt-BR/mysql-9.7/stable/development#varargs-vdfs).
</Warning>
