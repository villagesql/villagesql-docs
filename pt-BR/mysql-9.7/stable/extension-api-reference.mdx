...
consulte [Criando Extensões em C++](/pt-BR/mysql-9.7/stable/create). Para tipos de coluna personalizados,
consulte [Tipos Personalizados em C++](/pt-BR/mysql-9.7/stable/custom-types).
...
- **Retornar aviso:** `out.warning(msg)` — retorna NULL para esta linha, adiciona um aviso SQL, continua a execução. No modo estrito, inserir o NULL resultante em uma coluna NOT NULL ainda falha com a rejeição de NULL comum do modo estrito (`ERROR 1048 (23000): Column '<col>' cannot be null`) — o aviso em si não se torna um erro, e uma coluna que aceita NULL recebe o NULL silenciosamente. Chame em vez de `out.set()`, não além dele.
...
[Estado por Instrução](/pt-BR/mysql-9.7/stable/development#per-statement-state-prerun-and-postrun)
...
[Aggregate VDFs](/pt-BR/mysql-9.7/stable/development#aggregate-vdfs) para a
...
Para a documentação completa da API Preview, consulte [Capabilities Preview](/pt-BR/mysql-9.7/stable/preview-capabilities).
