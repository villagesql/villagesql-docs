...
Este guia é a referência aprofundada para escrever implementações de VDF em C++. Ele é o complemento de [Criando Extensões em C++](/pt-BR/mysql-9.7/dev/create), que cobre as etapas de compilação de ponta a ponta, e de [Testes em C++](/pt-BR/mysql-9.7/dev/testing), que cobre o ciclo de teste e iteração.
...
analisada e armazenada em cache (consulte [Tipos Parametrizados](/pt-BR/mysql-9.7/dev/type-operations#parameterized-types)).
...
[Tamanhos de Buffer Personalizados](/pt-BR/mysql-9.7/dev/create#custom-buffer-sizes) para as
...
Para a referência completa do construtor de operações de tipo — codificação, decodificação, comparação, hash, padrões intrínsecos e tipos parametrizados — consulte [Operações de Tipo](/pt-BR/mysql-9.7/dev/type-operations).

## Capabilities Preview

As seguintes capabilities do VEF estão disponíveis como cabeçalhos Preview opcionais. A ABI e a API ainda estão em desenvolvimento ativo; consulte [Capabilities Preview](/pt-BR/mysql-9.7/dev/preview-capabilities) para a referência completa.

- **Variáveis de sistema de extensão** — [Capabilities Preview → Variáveis de Sistema](/pt-BR/mysql-9.7/dev/preview-capabilities#system-variables)
- **Variáveis de status de extensão** — [Capabilities Preview → Variáveis de Status](/pt-BR/mysql-9.7/dev/preview-capabilities#status-variables)
- **Acesso ao keyring** — [Capabilities Preview → Acesso ao Keyring](/pt-BR/mysql-9.7/dev/preview-capabilities#keyring-access)
- **Armazenamento de coluna** — [Capabilities Preview → Armazenamento de Coluna](/pt-BR/mysql-9.7/dev/preview-capabilities#column-storage)
...
- [Criando Extensões em C++](/pt-BR/mysql-9.7/dev/create) — etapas de compilação de ponta a ponta, configuração do CMake e instalação
- [Testes em C++](/pt-BR/mysql-9.7/dev/testing) — servidor de desenvolvimento local, MTR e depuração de falhas
- [Operações de Tipo](/pt-BR/mysql-9.7/dev/type-operations) — codificação, decodificação, comparação, hash, tipos parametrizados
...
- [Arquitetura de Extensões](/pt-BR/mysql-9.7/dev/architecture) — ciclo de vida, cache do Victionary, padrões de desempenho e modelo de segurança
