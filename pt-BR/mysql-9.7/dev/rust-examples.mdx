...
Esta página apresenta duas das extensões de referência do repositório do SDK Rust: um exemplo mínimo que contém apenas uma função e um tipo personalizado completo com aritmética, ordenação e hashing. O diretório `examples/` do repositório também traz um exemplo para cada [capability preview](/pt-BR/mysql-9.7/dev/rust-preview-capabilities) suportada.
...
(observação: o texto de descrição do manifest está desatualizado — a função é registrada como `rot13`, não `vsql_rot13`; a correção upstream é acompanhada separadamente)
