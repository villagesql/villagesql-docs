...
This page assumes you've already worked through [Creating Extensions in Rust](/mysql-9.7/dev/rust-sdk). The setup (Cargo.toml, manifest.json, cargo-vsql) is the same.
...
| `intrinsic_default_fn` | fn | Computes the default instead of stating it: `fn() -> Result<String, String>`. Use it when the default cannot be written as a literal. |

`type_name`, `persisted_length`, `max_decode_buffer_length`, `encode`, `decode`, and `compare` are required. `hash` is optional but recommended, because correct `COUNT(DISTINCT)` and set operations depend on it. Give the type a default one way or the other: `default` for a value you can write as a literal, `intrinsic_default_fn` for one the extension has to compute. Setting both makes `INSTALL EXTENSION` fail. Setting neither lets the extension install, and then fails the first statement that names the type.
...
| `intrinsic_default_fn` | fn | Computes the default instead of stating it: `fn(&P) -> Result<String, String>`. Use it when the default depends on the parameters. |
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="shapes"
    href="/mysql-9.7/dev/custom-types"
  >
