...
- **Real capabilities**, such as `vsql_uuid` (UUID types and generators) and the other extensions in the [bundled extensions list](https://github.com/villagesql/villagesql-server/blob/main/villagesql/dev_server/bundled_extensions.txt).
- **`vsql_complex`** and **`vsql_simple`**, minimal reference extensions used elsewhere in these docs (see [C++ Extension Examples](/mysql-9.7/dev/examples)) to show how the extension framework works. They are not a product feature — treat them as a teaching aid, not something to build on.

If you built VillageSQL from source (Option C), `vsql_complex` and `vsql_simple` are already in `veb_dir` too — `make install` builds them unconditionally. `vsql_uuid` and the other real-capability extensions are not: they live in separate repositories and need to be built and installed on their own. If you used Option C, clone and build [vsql-uuid](https://github.com/villagesql/vsql-uuid) before continuing, or skip ahead using `vsql_complex` in its place — see [Installing Extensions](/mysql-9.7/dev/install).
