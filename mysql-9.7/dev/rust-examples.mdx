...
This page walks through two of the reference extensions in the Rust SDK repository — a minimal function-only example and a complete custom type with arithmetic, ordering, and hashing. The repository's `examples/` directory also carries one example per supported [preview capability](/mysql-9.7/dev/rust-preview-capabilities).
...
(note: the manifest's description text is stale — the function is registered as `rot13`, not `vsql_rot13`; upstream fix tracked separately)
