...
This page walks through four of the reference extensions in the Rust SDK repository — a minimal function-only example, a complete custom type with arithmetic, ordering, and hashing, an aggregate function, and a varargs function. The repository's `examples/` directory also carries one example per supported [preview capability](/mysql-9.7/dev/rust-preview-capabilities).
