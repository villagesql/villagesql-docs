...
Clone the VillageSQL Server repository from GitHub, and ask for the
`mysql-9.7` branch — the default branch builds MySQL 8.4. Clone into your home
directory so the CMake steps below work without modification:

```bash
cd "$HOME"
git clone --depth 1 -b mysql-9.7 https://github.com/villagesql/villagesql-server.git
cd villagesql-server
```

To build the exact code a release shipped, clone the tag that marks it instead
of the branch: `-b publish/mysql-9.7_0.0.6`.
...
- `-DWITH_DEBUG=1` - Builds with the DBUG facility and assertions, unoptimized (recommended for development)
...
This builds the server, the `mysql` client that
[Step 7](#step-7-connect-with-mysql-client) uses, the extension SDK, and the
...
This pair is faster and builds neither the extension SDK nor the example
...
<Tip>
  The `--gdb` flag unblocks `SIGINT`, so Ctrl-C stops the server when you run it from a terminal. It also suppresses the crash backtrace and cancels `--core-file`, so leave it out when you want a crash report. To run in the background, add `--daemonize` to the `mysqld` command.
</Tip>
...
Server version: <version> Source distribution
...
Verify the build with the VillageSQL test suite, from the build directory:
...
`--nounit-tests` skips the upstream MySQL unit tests, which include known failures. Run the VillageSQL unit tests separately:

```bash
make -j10 villagesql-unit-tests && ctest -L villagesql
```

A failing test makes either command exit non-zero.

For GDB debugging, running individual tests, re-recording expected results, and contributing to the server codebase, see the [Server Development Guide](/mysql-9.7/dev/server-development).

## Building Native Packages (RPM/DEB)

`villagesql/bld_tools/build_deb.sh` and `build_rpm.sh` build a Debian or RPM
package straight from the source tree, for installing with your package
manager instead of following the manual steps above. Neither needs a prior
CMake configure — both derive the package version from the `VSQL_VERSION` and
`MYSQL_VERSION` files at the repository root. With Steps 1-2 already done, if
`dpkg-buildpackage` (Debian/Ubuntu, package `dpkg-dev`) or `rpmbuild` (Enterprise Linux,
package `rpm-build`) is not already installed, install it, then run one from
the repository root:

```bash
cd "$HOME/villagesql-server"
villagesql/bld_tools/build_deb.sh   # writes to ./debs
# or
villagesql/bld_tools/build_rpm.sh   # writes to ./rpms
```

According to [villagesql-server#931](https://github.com/villagesql/villagesql-server/pull/931),
which added this tooling, the resulting package takes over an existing MySQL
install on the target host rather than installing alongside one. No CI
workflow in this repository invokes either script.
