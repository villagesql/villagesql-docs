...
- **C++** — [vsql-extension-template](https://github.com/villagesql/vsql-extension-template) is the starting point. Walk through [Creating Extensions in C++](/mysql-9.7/dev/create).
- **Rust** — [vsql-extension-template-rust](https://github.com/villagesql/vsql-extension-template-rust) is the starting point. Walk through [Creating Extensions in Rust](/mysql-9.7/dev/rust-sdk).

The C++ SDK is more complete today (mysql_services, sql_query, statement_event, column storage); the Rust SDK covers functions (including aggregates, varargs, and prerun/postrun state), custom types, and four preview capabilities (status_var, sys_var, thread_worker, keyring) — see [Preview Capabilities in Rust](/mysql-9.7/dev/rust-preview-capabilities).
