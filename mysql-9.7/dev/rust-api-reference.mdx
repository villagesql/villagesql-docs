...
<Warning>
  ...
  supported, as are the `sys_var`, `status_var`, `thread_worker`,
  `keyring`, and `sql_query` preview capabilities. The column storage
  ABI is C++-only today — use the [C++ SDK](/mysql-9.7/dev/create) if you need it.
</Warning>
