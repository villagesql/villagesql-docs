...
<Warning>
  ...
  C++-only today — use the [C++ SDK](/mysql-9.7/dev/create) if you
  need any of those.
</Warning>
...
Both `funcs:` and `types:` are optional on their own. A pure-function extension omits `types:`; a type-only extension omits `funcs:`. An empty `extension!` block (no funcs, no types) is valid but produces an extension that does nothing.
