...
This page is a reference for the `villagesql` crate API. For the getting started tutorial, see [Creating Extensions in Rust](/mysql-9.7/dev/rust-sdk). For custom types, see [Custom Types in Rust](/mysql-9.7/dev/rust-custom-types).
...
| `CustomWithParams { bytes, params }` | Raw bytes + `TypeParams` | A parameterized custom type registered via [`parameterized_type!`](/mysql-9.7/dev/rust-custom-types#parameterized-types) |
...
`requires:` declares the [preview capabilities](/mysql-9.7/dev/rust-preview-capabilities) the extension uses, as references to `static` capability objects. It must come last, after a `funcs:` section — include `funcs: []` if the extension registers no functions.

## func! macro

`func!` declares a SQL-callable function. Eight forms — four without per-statement state (no parameters, `buffer_size` only, `deterministic` only, both), two that attach per-statement state through a `prerun` function, and two more that add an optional `postrun:` on top of those. See [Per-statement state](#per-statement-state) below.
...
villagesql::func!(rust_fn, "sql_name", [param_types] -> return_type, state: StateType, prerun: prerun_fn, postrun: postrun_fn)
villagesql::func!(rust_fn, "sql_name", [param_types] -> return_type, state: StateType, prerun: prerun_fn, postrun: postrun_fn, buffer_size: N, deterministic: true)
```
...
| `buffer_size: N` | Optional. Size in bytes of the result buffer for string/binary returns. Use `0` for the server default (1024 bytes). When a function returns a string or binary value larger than `buffer_size`, the function errors rather than truncating — declare a larger `buffer_size` to handle larger results. |
...
| `postrun: postrun_fn` | Optional, only on the `state:`/`prerun:` forms. `fn(&mut StateType)`, called once after the last row, before the SDK drops the state. |
...
You must not free the state yourself: `func!` always generates the postrun that drops it when the statement ends, so freeing it is not your responsibility even when you supply your own teardown. That drop step is the opposite of the C++ SDK, where your postrun has to call `delete_state<T>()` — see [Per-Statement State](/mysql-9.7/dev/development#per-statement-state-prerun-and-postrun).

Name a `postrun: postrun_fn`, `fn(&mut StateType)`, to run your own teardown first: it runs once after the last row, while the state is still borrowable, and before the SDK's own drop. See `examples/vsql_postrun_order/src/lib.rs` in the [Rust SDK repo](https://github.com/villagesql/vsql-rust-sdk), whose recorded test output includes the line `prerun,postrun,drop|rows_seen=3`, confirming the order.
...
Build and install it as described in [Creating Extensions in Rust](/mysql-9.7/dev/rust-sdk), then:
...
<Warning>
  ...
  types for varargs VDFs](/mysql-9.7/dev/development#varargs-vdfs) either.
</Warning>

Eight forms — three shapes, each with a shorthand and a full form that adds
`buffer_size` and `deterministic` together (never singly). The `state:`/
`prerun:` shape has two more on top of that, adding an optional `postrun:` —
...
// Per-statement state, validating prerun, and author postrun.
villagesql::varargs_func!(impl_fn, "sql_name", [..] -> return_type,
    state: StateType, prerun: prerun_fn, postrun: postrun_fn)
villagesql::varargs_func!(impl_fn, "sql_name", [..] -> return_type,
    state: StateType, prerun: prerun_fn, postrun: postrun_fn,
    buffer_size: N, deterministic: true)
...
| `postrun: postrun_fn` | Optional, only on the `state:`/`prerun:` forms — means the same as it does in `func!`: `fn(&mut StateType)`, run once after the last row and before the SDK drops the state. |
...
`custom_type!` registers a new column type. `type_name`, `persisted_length`, `max_decode_buffer_length`, `encode`, `decode`, and `compare` are required. `hash` is optional but recommended. Give either `default` or `intrinsic_default_fn`, not both.
...
| `intrinsic_default_fn` | `fn() -> Result<String, String>` | Optional. Computes that string instead of stating it, for a default that cannot be written as a literal. |

The `default` field is not a column default value; an omitted column is `NULL`. It supplies the string the server encodes when it initializes the type, and that happens the first time a statement names the type rather than when the extension installs. Give neither `default` nor `intrinsic_default_fn` and the server encodes the empty string instead. Either way, an encode that returns `Err` fails that statement and leaves the type unusable:

```
ERROR 3219 (HY000): Type '<extension>.<type>' failed to initialize: from_string VDF failed to encode intrinsic default input '<default>' (expected persisted_length=<N>)
```

Giving both `default` and `intrinsic_default_fn` is refused when the extension installs, not deferred to first use.
