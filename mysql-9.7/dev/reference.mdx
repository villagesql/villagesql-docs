...
declare the maximum length of its string results. It also adds
`bind_and_check_types`, a hook that lets a function decide its own return-type
parameters at resolution time instead of relying on the built-in
disambiguation rules. It is under active development behind the opt-in dev ABI
and may change before it stabilises. If
