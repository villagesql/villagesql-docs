...
declare the maximum length of its string results. It also adds the `on_init` /
`on_deinit` load and unload hooks, which the server now calls once the
extension's required capabilities are populated and before they are
depopulated (previously the extension ran them itself with no server access).
It is under active development behind the opt-in dev ABI and may change before
it stabilises. If
