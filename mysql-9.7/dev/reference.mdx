...
Protocol V4 adds support for variable-length custom types, lets a function
declare the maximum length of its string results, and adds the `on_init` /
`on_deinit` load and unload hooks, which the server calls once the extension's
required capabilities are populated and before they are depopulated. It is
under active development behind the opt-in dev ABI and may change before it
stabilises. If you are developing extensions, see
