...
Protocol 1 is the original VEF interface. It was stable as of v0.0.1 and will be dropped after 0.0.5. New extensions should use the Protocol 3 API — the template-based type builders in the [Creating Extensions in C++](/mysql-9.7/dev/create) and [C++ Development](/mysql-9.7/dev/development) guides.
...
Protocol 1 VDF implementations can pass the raw C structs directly instead of using typed wrappers. This style will be dropped after 0.0.5 — use the [typed argument/result API](/mysql-9.7/dev/development#argument-and-result-types) for all new code.
