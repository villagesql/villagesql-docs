...
<Note>
  If you are contributing to the VillageSQL server itself (not building an extension), see [Build from Source](/mysql-9.7/dev/source), which covers the full server developer workflow including running tests with `mysql-test-run.pl` directly.
</Note>
