...
declaration (`MYTYPE`, no length or params) calls `resolve_params` with an empty
map, so a type that supplies defaults gives every column explicit params —
`vsql_bitfield_test`'s `BITFIELD` resolves a bare column to
`max_number_of_bits=4096`:
