...
**Error:** `ERROR 3219 (HY000): VEB file not found: my_extension.veb`
...
UNINSTALL EXTENSION my_extension;
-- If blocked: ERROR 3219 (HY000): Cannot drop extension `my_extension` as 1 column(s) depend on it,
...
**Error:** `Failed to load VEF extension 'my_extension': failed to load so: <dlerror text>`
...
**Error:** `Failed to load VEF extension 'extension_name'`

**Cause:** The `.veb` file failed to load at the entry-point stage — for
example a missing or invalid `vef_register` symbol, or a corrupt `.veb` file.
...
Both names must be identical, using underscores (see [Extension Naming Conventions](/mysql-9.7/dev/install#extension-naming-conventions)):
- VEB filename: `my_extension.veb`
...
<Note>
  `ALTER EXTENSION` requires the `EXTENSION_ADMIN` privilege (or `SUPER` as a
  fallback), the same gate as `INSTALL EXTENSION` and `UNINSTALL EXTENSION`. See
  [Required Privilege](/mysql-9.7/dev/install#required-privilege).
</Note>
...
A second precheck rejects a version that would drop a custom type still in
use by a table column:
...
ROW format binlog is the recommended setting for custom types. INSERT, UPDATE,
DELETE, and ALTER TABLE operations on custom type columns all replicate
correctly in ROW format.
...
importing the dump.
<Note>
`CLONE INSTANCE` validates that every VillageSQL extension installed on the
donor is available on the recipient — same name, version, and `.veb` SHA256 —
during the connection handshake, before any data transfers. A missing or
mismatched extension aborts the clone during the handshake instead of letting
it finish and only failing when the recipient restarts. A `CLONE INSTANCE`
from a donor with `vsql_complex` installed, against a recipient missing that
extension's `.veb`, fails with:

```text
ERROR HY000: VillageSQL: VEB file for extension 'vsql_complex' not found
```

Place the same `.veb` build of every extension the donor has in the
recipient's `veb_dir` before cloning.
</Note>

<Warning>
Behavior with XtraBackup and InnoDB Cluster / Group Replication is not yet
tested. Test your restore path before relying on it in production.
</Warning>
