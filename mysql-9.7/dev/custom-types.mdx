...
page is Step 4 of the [Creating Extensions in C++](/mysql-9.7/dev/create)
...
See [Type Operations](/mysql-9.7/dev/type-operations) for the full C++ signatures.
...
An `ALTER TABLE` that adds, drops, or modifies a custom-type column is also
refused outright under `LOCK TABLES`: custom-type operations write to
villagesql's own system tables, which are not in the locked-tables list, so
the server rejects the statement up front rather than failing deep inside
table opening:

```sql
LOCK TABLES my_table WRITE;
ALTER TABLE my_table ADD COLUMN v MYTYPE;
-- ERROR 3219 (HY000): Custom type table operations are not supported under LOCK TABLES.
UNLOCK TABLES;
```

An `ALTER TABLE` that does not touch a custom-type column is not subject to
this restriction.
...
void encode_complex(std::string_view from, CustomResult out) {
    ...
    // strtod needs a null-terminated string; string_view is not one.
    ...
    if (endptr == p || *endptr != ',') return fail();
    p = endptr + 1;
    imag = strtod(p, &endptr);
    if (endptr == p || *endptr != ')') return fail();
...
void decode_complex(CustomArg in, StringResult out) {
    ...
    if (p == end) return;
    *p++ = '(';
    auto re = std::to_chars(p, end, real);
    if (re.ec != std::errc() || re.ptr == end) return;
    p = re.ptr;
    *p++ = ',';
    auto im = std::to_chars(p, end, imag);
    if (im.ec != std::errc() || im.ptr == end) return;
    p = im.ptr;
    *p++ = ')';
...
Decoding uses `std::to_chars` rather than `snprintf`: a fixed-precision format
such as `"%.6f"` drops digits a `double` needs, so `0.1234567891` reads back as
`0.123457`. Encoding copies the input before calling `strtod`, because
`std::string_view` carries no terminating null. The floating-point overloads of
`std::from_chars` would avoid the copy, but libc++ marks them available only
from macOS 26, so an extension that uses them fails to build for earlier
targets.
...
See [vsql_complex Example](/mysql-9.7/dev/examples) for the complete implementation.
...
<CardGroup cols={2}>
  <Card
    ...
    icon="hammer"
    href="/mysql-9.7/dev/create#step-5-update-build-configuration"
  >
  ...
  <Card
    ...
    icon="sliders"
    href="/mysql-9.7/dev/type-operations#parameterized-types"
  >
  ...
  <Card
    ...
    icon="book"
    href="/mysql-9.7/dev/extension-api-reference"
  >
  ...
  <Card
    ...
    href="/mysql-9.7/dev/managing#replication"
  >
