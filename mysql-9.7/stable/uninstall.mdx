...
<Note>
  `UNINSTALL EXTENSION` requires the `EXTENSION_ADMIN` privilege (or `SUPER` as a
  fallback). See [Required Privilege](/mysql-9.7/stable/install#required-privilege).
</Note>
...
- The `EXTENSION_ADMIN` privilege (or `SUPER` as a fallback)
...
<Note>
  ...
  action. See [Managing Extensions](/mysql-9.7/stable/managing) for how
...
| `Extension 'name' is not installed` | Extension not installed | Verify with `SELECT * FROM INFORMATION_SCHEMA.EXTENSIONS` |
...
See [Installing Extensions](/mysql-9.7/stable/install) for details.
