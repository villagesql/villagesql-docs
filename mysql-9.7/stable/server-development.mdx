...
This guide covers debugging, running the server test suite, and contributing to VillageSQL. It assumes a working source build — see [Build from Source](/mysql-9.7/stable/source) if you haven't done that yet.
...
```bash
./mysql-test-run.pl --do-suite=villagesql --nounit-tests --parallel=auto
```
...
<Note>
  **--suite vs --do-suite:**
  ...
  - `--nounit-tests` skips the unit tests MTR would otherwise run alongside the suite. Those are upstream MySQL unit tests and include known failures; run the VillageSQL ones on their own with the [Unit Tests](#unit-tests) commands above.
</Note>
...
The full error log is at `mysql-test/var/log/mysqld.1.err` for a single-worker run. Under `--parallel`, each worker writes to `mysql-test/var/<N>/log/mysqld.1.err` instead.

For writing extension regression tests, see the [Development Guide](/mysql-9.7/stable/development).
