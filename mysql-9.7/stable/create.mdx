...
For writing VDF implementations in depth — argument and result types, aggregates, system variables, parameterized types — see the [Development Guide](/mysql-9.7/stable/development).

<Note>
If you prefer Rust, see [Creating Extensions in Rust](/mysql-9.7/stable/rust-sdk).
</Note>
...
Before you begin, you need the VillageSQL Extension SDK, and there are two ways to point CMake at one. If you have a VillageSQL build tree, pass `-DVillageSQL_BUILD_DIR=/path/to/villagesql/build` — the [Build from Source](/mysql-9.7/stable/source) guide produces that tree. If you have an unpacked SDK directory — one containing `include/villagesql/vsql.h` — that is enough on its own: pass `-DVillageSQL_SDK_DIR=/path/to/sdk`. The [extension template README](https://github.com/villagesql/vsql-extension-template) covers how to get an SDK without building the server. Only the build-tree form sets a VEB install directory, so on the SDK-only path copy the built VEB file yourself into the directory `SHOW VARIABLES LIKE 'veb_dir'` reports.
...
For the full naming convention across SQL, filenames, and repository names, see [Extension Naming Conventions](/mysql-9.7/stable/install#extension-naming-conventions).
...
See the [development guide](/mysql-9.7/stable/development#argument-and-result-types)
...
<Note>
Before implementing your functions, review the [C++ API Reference](/mysql-9.7/stable/extension-api-reference) for the complete VDF contracts: null checking, result types, buffer sizing, and error handling.
</Note>
...
See [Custom Types in C++](/mysql-9.7/stable/custom-types) for the full
implementation walkthrough. If your type takes parameters (e.g., `VECTOR(1536)`),
see [Parameterized Types](/mysql-9.7/stable/type-operations#parameterized-types).
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="code"
    href="/mysql-9.7/stable/development"
  >
  ...
  <Card
    ...
    icon="sitemap"
    href="/mysql-9.7/stable/architecture"
  >
