...
This guide covers the test-and-iterate loop for a C++ extension: setting up a local server, installing `.veb` files, running regression tests, and debugging failures. It is the companion to [Creating Extensions in C++](/mysql-9.7/stable/create), which covers the initial build, and [C++ Development](/mysql-9.7/stable/development), which covers VDF authoring depth.

<Note>
  If you are contributing to the VillageSQL server itself (not building an extension), see the [Server Development Guide](/mysql-9.7/stable/server-development), which covers the full server developer workflow including running tests with `mysql-test-run.pl` directly.
</Note>

## Setting Up Your Environment

To develop and test extensions, you need a built VillageSQL server. Follow the [Clone and Build from Source](/mysql-9.7/stable/source) guide to compile the server binaries.
...
Before installing an extension via SQL, its `.veb` file must be present on the server. The CLI manages the running instance's veb directory (`instances/default/veb/` by default, or `<dir>/veb` when using `--dir`/`--here`):
...
`lib/veb/` at the package root is only the one-time seed source: `.veb` files placed there before `init` are copied into the new instance's veb directory when the instance is created, and `lib/veb/` is not touched afterward. After adding a file, install the extension via SQL:
...
- [Testing Network-Dependent Extensions](/mysql-9.7/stable/testing-network) — reliable MTR patterns for extensions that spawn HTTP servers or external listeners
