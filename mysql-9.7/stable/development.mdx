...
This guide is the deep reference for writing C++ VDF implementations. It is the companion to [Creating Extensions in C++](/mysql-9.7/stable/create), which covers the end-to-end build steps, and [C++ Testing](/mysql-9.7/stable/testing), which covers the test-and-iterate loop.
...
parsed params struct (see [Parameterized Types](/mysql-9.7/stable/type-operations#parameterized-types)).
...
[Custom Buffer Sizes](/mysql-9.7/stable/create#custom-buffer-sizes) for the
...
For the full type operation builder reference — encode, decode, compare, hash, intrinsic defaults, and parameterized types — see [Type Operations](/mysql-9.7/stable/type-operations).

## Preview Capabilities

The following VEF capabilities are available as opt-in preview headers. ABI and API are still under active development; see [Preview Capabilities](/mysql-9.7/stable/preview-capabilities) for the full reference.

- **Extension system variables** — [Preview Capabilities → System Variables](/mysql-9.7/stable/preview-capabilities#system-variables)
- **Extension status variables** — [Preview Capabilities → Status Variables](/mysql-9.7/stable/preview-capabilities#status-variables)
- **Keyring access** — [Preview Capabilities → Keyring Access](/mysql-9.7/stable/preview-capabilities#keyring-access)
- **Column storage** — [Preview Capabilities → Column Storage](/mysql-9.7/stable/preview-capabilities#column-storage)
...
- [Creating Extensions in C++](/mysql-9.7/stable/create) — end-to-end build steps, CMake setup, and installation
- [C++ Testing](/mysql-9.7/stable/testing) — local dev server, MTR, and debugging failures
- [Type Operations](/mysql-9.7/stable/type-operations) — encode, decode, compare, hash, parameterized types
- [C++ API Reference](/mysql-9.7/stable/extension-api-reference) — VDF contracts, null handling, and buffer sizing
- [Extension Architecture](/mysql-9.7/stable/architecture) — lifecycle, Victionary caching, performance patterns, and security model
