...
see [Creating Extensions in C++](/mysql-9.7/stable/create). For custom column types,
see [Custom Types in C++](/mysql-9.7/stable/custom-types).
...
<Note>
  ...
  design philosophy behind the typed argument/result API and lower-level hooks like
  `prerun()` and varargs.
</Note>
...
the result type: `out.set(...)` / `out.set_length(n)`, `out.set_null()`,
`out.warning(msg)`, or `out.error(msg)`.

**2. Check `input.is_null()` before calling `input.value()`.**
...
**4. Pass error messages to `out.error(msg)`. The message is truncated to `VEF_MAX_ERROR_LEN` (512 bytes) if necessary.**
...
- **Return warning:** `out.warning(msg)` — returns NULL for this row, adds a SQL warning, continues execution. In strict mode, inserting the resulting NULL into a NOT NULL column still fails with the ordinary strict-mode NULL rejection (`ERROR 1048 (23000): Column '<col>' cannot be null`) — the warning itself does not become an error, and a nullable column accepts the NULL silently. Call instead of `out.set()`, not in addition to it.
...
[Per-Statement State](/mysql-9.7/stable/development#per-statement-state-prerun-and-postrun)
...
[Aggregate VDFs](/mysql-9.7/stable/development#aggregate-vdfs) for the
...
For the full preview API documentation, see [Preview Capabilities](/mysql-9.7/stable/preview-capabilities).
...
type column values inside a trigger body is supported: `NEW.<column>` and
`OLD.<column>` can be read directly and passed to extension functions.
...
FOR EACH ROW
    INSERT INTO signal_log (id, label, magnitude)
    VALUES (NEW.id, NEW.label, complex_abs(NEW.impedance));

INSERT INTO signals VALUES (1, '(10,5)', 'sensor_a');
SELECT id, label, magnitude FROM signal_log;
-- 1  sensor_a  11.180339887498949
```
