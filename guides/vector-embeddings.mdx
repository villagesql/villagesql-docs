...
On VillageSQL 0.0.6 and earlier, `ai_embedding()` returns binary charset, and MySQL's JSON parser rejects a binary string — the result has to pass through `CONVERT(... USING utf8mb4)` before it reaches a `JSON` column or a JSON function:

```sql
-- On 0.0.6 and earlier, this throws an error — JSON_LENGTH rejects binary charset
-- ERROR 3144 (22032): Cannot create a JSON value from a string with CHARACTER SET 'binary'
SELECT JSON_LENGTH(ai_embedding('openai', 'text-embedding-3-small', @key, 'Machine learning'));

-- This works on every version
SELECT JSON_LENGTH(
  CONVERT(ai_embedding('openai', 'text-embedding-3-small', @key, 'Machine learning')
          USING utf8mb4)
);
```

VillageSQL 0.0.7 and later return the correct charset directly, so `CONVERT` is no longer required there — but it is harmless to keep, so every example below still uses it for compatibility with 0.0.6 and earlier.
...
SET @key = 'your-openai-api-key';
-- max_execution_time only bounds SELECT statements — it has no effect on
-- this INSERT.

INSERT INTO documents (title, content, embedding) VALUES
    ('Intro to ML',
     'Machine learning is a subset of artificial intelligence',
     CONVERT(ai_embedding('openai', 'text-embedding-3-small', @key,
                          'Machine learning is a subset of artificial intelligence')
             USING utf8mb4)),
    ('Neural Networks',
...
-- Confirm the vectors parsed as JSON
...
SET @key = 'your-openai-api-key';
-- max_execution_time only bounds SELECT statements — it has no effect on
-- this UPDATE. To cap how long it runs, get SELECT CONNECTION_ID() beforehand
-- and run KILL QUERY <id> from another session if it runs too long.

UPDATE documents
SET embedding = CONVERT(ai_embedding('openai', 'text-embedding-3-small', @key, content)
                        USING utf8mb4)
...
SET embedding = CONVERT(ai_embedding('openai', 'text-embedding-3-small', @key, product_description)
                        USING utf8mb4);

-- Embed concatenated fields for richer context
SET embedding = CONVERT(ai_embedding('openai', 'text-embedding-3-small', @key,
...
| Problem | Solution |
|:---|:---|
| `FUNCTION <database>.ai_embedding does not exist` | Run `INSTALL EXTENSION vsql_ai` |
| `ERROR 3144: Cannot create a JSON value from a string with CHARACTER SET 'binary'` | Wrap the call in `CONVERT(... USING utf8mb4)` — see [The charset conversion](#the-charset-conversion) |
...
- [Connecting MySQL to AI APIs](/guides/ai-api-setup) — installing and configuring the vsql_ai extension
- [Running AI Models from MySQL Queries](/guides/ai-prompts-in-mysql) — text generation as a complement to embeddings
- [UUIDs in MySQL](/guides/uuids) — stable row identifiers for embedding lookup tables
- [Multi-dimensional range queries](/guides/cube-queries) — geometric containment and distance queries without AI vectors
