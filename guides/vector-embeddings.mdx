...
MySQL doesn't have a native vector distance function, so full similarity search (finding the nearest vectors in a large table) runs fastest in application code. The typical architecture: store embeddings in MySQL, fetch them, compute cosine similarity in your application, and return the top matches.

For smaller tables (under a few thousand rows), you can compute similarity directly in SQL using JSON functions. Cosine similarity between two JSON vectors is computed as the dot product divided by the product of their magnitudes — this is verbose in MySQL but functional:

```sql
-- Example: find documents similar to a query embedding (small tables only)
-- Assumes embedding column holds a JSON array with the same dimensions

SET @query_embedding = create_embed('openai', 'text-embedding-3-small', @key, 'neural networks');

-- Retrieve all embeddings; compute similarity in application code
SELECT id, title, embedding FROM documents;
```

For production-scale similarity search, pass the stored JSON embeddings to a vector similarity library in your application layer (NumPy, scikit-learn, Faiss), or use a dedicated vector database alongside MySQL.
