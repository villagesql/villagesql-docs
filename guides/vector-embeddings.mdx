...
`ai_embedding()` returns binary charset, as every VEF STRING function does. MySQL's JSON parser rejects a binary string, so the result has to pass through `CONVERT(... USING utf8mb4)` before it reaches a `JSON` column or a JSON function:

```sql
-- This throws an error — JSON_LENGTH rejects binary charset
...
The same applies to inserting into a `JSON` column, which is why every example below wraps the call. A `TEXT` column needs no conversion — it is JSON parsing, not storage, that rejects the binary charset.
