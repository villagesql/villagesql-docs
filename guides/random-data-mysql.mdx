...
```sql
-- Insert 1000 rows of test data. WITH RECURSIVE must immediately precede
-- the SELECT it attaches to, so it goes after the INSERT's column list.
...
No. MySQL itself permits non-deterministic functions in DEFAULT expressions — `UUID()`, `RAND()`, and `NOW()` all work there. `gen_random_bytes()` is rejected specifically because it's a VillageSQL extension function registered as non-deterministic, and VEF blocks those in DEFAULT expressions regardless of what MySQL itself allows. Generate the value in each INSERT statement instead.
...
| Problem | Solution |
|:---|:---|
| `FUNCTION <database>.gen_random_bytes does not exist` | Run `INSTALL EXTENSION vsql_crypto` |
