...
Yes, using the `mysql.slow_log` table if you've configured `log_output = TABLE` — there is no `performance_schema` slow-log table:
...
The slow query log and `mysqldumpslow` work identically on [VillageSQL](https://villagesql.com/install), a drop-in replacement for MySQL with extensions.
...
- [Why Is My MySQL Query Slow?](/guides/mysql-query-performance) — what to do with the queries the log surfaces
- [Using MySQL with ORMs: Django, Rails, and Prisma](/guides/mysql-with-orms) — catching what ORMs generate
