...
`SET DEFAULT` is part of the SQL standard. InnoDB accepts the syntax without complaint but does not implement it — at runtime it behaves exactly like `RESTRICT` (ERROR 1451). Don't use it: it silently does the opposite of what its name implies.
...
- [Soft Deletes in MySQL](/guides/soft-deletes) — why cascades ignore soft deletes
