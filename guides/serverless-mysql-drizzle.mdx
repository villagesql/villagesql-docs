...
Sampling the number of active connections for `serverless_app` during the burst, taken every 150ms with `SELECT COUNT(*) FROM information_schema.processlist WHERE user = 'serverless_app'`, showed the count climbing to and past the 15-connection cap (15, 15, 16, 17, 17, 17, 17, 25 across the highest samples). Once past it, the server's own log filled with:
