...
SET @rate_body = JSON_UNQUOTE(JSON_EXTRACT(@rate_resp, '$.content'));
SET @eur_rate = CAST(JSON_VALUE(@rate_body, '$.rates.EUR') AS DECIMAL(10,6));
...
**Each HTTP call is synchronous and blocks the query.** A batch of 50 rows each hitting a 200ms API takes 10 seconds minimum. `max_execution_time` only bounds `SELECT` statements — it has no effect here. To cap how long this `UPDATE` can run, get its connection ID with `SELECT CONNECTION_ID()` beforehand and run `KILL QUERY <id>` from another session if it runs too long:
...
| Problem | Solution |
|:---|:---|
| UPDATE runs too long | Reduce `LIMIT`; `max_execution_time` does not apply to `UPDATE`, use `KILL QUERY <connection_id>` from another session instead |
| Returns NULL for all rows | API unreachable from server host; verify network access |
| JSON extraction returns NULL | `content` is a JSON-encoded string, not a nested object, so a single-step `'$.content.field'` path never resolves. Unquote it first — `JSON_UNQUOTE(JSON_EXTRACT(resp, '$.content'))` — then extract fields from that. This is independent of response size |
...
- [Serving Tables as a REST API](/guides/rest-api-from-tables) — publishing your own tables over HTTP
