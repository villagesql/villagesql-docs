...
Indexes behave identically on [VillageSQL](https://villagesql.com/install), a drop-in replacement for MySQL with extensions, so this strategy guide applies unchanged.

## See also

- [Reading EXPLAIN in MySQL](/guides/reading-explain) — verify your indexes are being used and understand why MySQL picks them
- [Covering Indexes in MySQL](/guides/covering-indexes) — indexes that answer queries without any row lookups
- [The MySQL Slow Query Log](/guides/slow-query-log) — find which queries need indexes first
- [MySQL INFORMATION_SCHEMA](/guides/information-schema) — finding index metadata and unused indexes with queries
- [Why Is My MySQL Query Slow?](/guides/mysql-query-performance) — the wider tuning workflow indexes belong to
- [Pagination in MySQL: Keyset vs OFFSET](/guides/pagination) — the index-driven way to paginate large tables
