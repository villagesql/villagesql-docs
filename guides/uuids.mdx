...
These introspection functions are deterministic, so — unlike the generators (`UUID_V4()`, `UUID_V7()`, and friends, whose output changes on every call) — they can appear in generated columns and CHECK constraints. That lets you derive or validate stored values from a UUID at the schema level:

```sql
CREATE TABLE audit_log (
  id uuid NOT NULL,
  ver INT AS (UUID_VERSION(id)) STORED,  -- derive the version into a column
  CHECK (UUID_VERSION(id) IN (4, 7))     -- accept only v4 or v7 IDs
);
```
