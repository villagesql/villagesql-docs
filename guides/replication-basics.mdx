...
- **Applier parallelism**: MySQL 8.4 already applies events in parallel by default — `replica_parallel_workers` is 4 and `replica_parallel_type` is `LOGICAL_CLOCK`. Raise the worker count if the replica still falls behind:
...
| Replication lag growing | Raise `replica_parallel_workers` above its default of 4; check for long-running queries on the replica blocking the applier |
...
- [Read Replicas in MySQL](/guides/read-replicas) — using the replicas this guide sets up
