...
```sql
SELECT ai_prompt('anthropic', 'claude-haiku-4-5-20251001', @api_key, 'Say hello') AS test;
```

(Set `@api_key` first — see below.)

## Setting Your API Key

The `ai_prompt()` and `create_embed()` functions take the API key as a parameter. The recommended pattern is a session variable set at connection time — never embed a key literal in a query that could end up in a log file.
...
Anthropic does not support `create_embed()`.
...
SELECT create_embed('google', 'gemini-embedding-001', @google_key, @text);
```
...
SELECT create_embed('openai', 'text-embedding-3-small', @openai_key, @text);

-- Embeddings (3072 dimensions — higher accuracy)
SELECT create_embed('openai', 'text-embedding-3-large', @openai_key, @text);
```
...
For batch operations over many rows, this matters — a 1000-row UPDATE calling `ai_prompt()` on each row can take 30+ minutes. See [Running AI Models from MySQL Queries](/guides/ai-prompts-in-mysql) for batch patterns.
