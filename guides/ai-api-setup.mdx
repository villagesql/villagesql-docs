...
SELECT ai_prompt('anthropic', 'claude-haiku-4-5', @anthropic_key, 'Say hello') AS test;
```
...
SELECT ai_prompt('anthropic', 'claude-haiku-4-5', @anthropic_key, 'Hello') AS response;
```
...
SELECT ai_prompt('anthropic', 'claude-haiku-4-5', @anthropic_key, @prompt);

-- Balanced — speed and intelligence
SELECT ai_prompt('anthropic', 'claude-sonnet-5', @anthropic_key, @prompt);

-- Recommended default — complex agentic and enterprise work
SELECT ai_prompt('anthropic', 'claude-opus-5', @anthropic_key, @prompt);
```

**Claude models:**
- `claude-fable-5` — most capable, for demanding reasoning and long-horizon work
- `claude-opus-5` — recommended default; complex agentic coding and enterprise work
- `claude-sonnet-5` — best combination of speed and intelligence
- `claude-haiku-4-5` — fastest and most cost-effective

Previous generations remain supported: `claude-opus-4-8`, `claude-opus-4-7`, `claude-opus-4-6`, `claude-opus-4-5-20251101`, `claude-sonnet-4-6`, `claude-sonnet-4-5-20250929`.
...
- [Running AI Models from MySQL Queries](/guides/ai-prompts-in-mysql) — using ai_prompt() once the extension is configured
- [Generating Vector Embeddings in MySQL](/guides/vector-embeddings) — using ai_embedding() for semantic search
