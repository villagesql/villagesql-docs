...
VillageSQL's `vsql_network_address` extension adds a native `INET` type that stores both IPv4 and IPv6 addresses in a compact binary format — a fixed 19 bytes per row for both families, with IPv4 values padded to that width — and validates the input on the way in.
...
An invalid string does not abort the statement: `inet_from_string()` returns NULL with Warning 3200, so the row inserts with a NULL value. Declare the column `NOT NULL` if you want bad input to fail the insert.
...
| `INET` (VillageSQL) | Yes | Yes | Yes | 19 bytes |
...
| Problem | Solution |
|:---|:---|
| `FUNCTION <database>.inet_from_string does not exist` | Run `INSTALL EXTENSION vsql_network_address` |
...
| `Incorrect CIDR value` for `192.168.1.5/24` | CIDR requires host bits to be zero — use `192.168.1.0/24`, or use INET instead. Inserting the bad value directly into a CIDR column aborts the statement; calling `cidr_from_string('192.168.1.5/24')` as a function instead returns NULL with a warning rather than erroring. |
...
- [Storing MAC Addresses in MySQL](/guides/storing-mac-addresses) — the same approach for hardware addresses
