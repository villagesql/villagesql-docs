...
The functions work unqualified. They can also be called as `vsql_fuzzystrmatch.soundex(...)`, qualifying by the owning extension's name — that is VEF name resolution, not a real schema, and `vsql_fuzzystrmatch` does not appear in `SHOW DATABASES`. All return NULL when any argument is NULL.
