...
Yes. Leave `vsql_mcp.schema` empty to expose every non-system schema the `db_url` account can see, instead of scoping to one.
...
| Problem | Solution |
|:---|:---|
| `INSTALL EXTENSION vsql_mcp` fails with a preview capabilities error | Start the server with `--vsql_allow_preview_extensions=ON`, or run `SET PERSIST vsql_allow_preview_extensions = ON` and restart. |
