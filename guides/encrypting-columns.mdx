...
A key shorter than the cipher requires raises `ERROR 3200: VDF error in function 'encrypt': key too short for <cipher>: need N bytes, got M` — it is not zero-padded. A longer key is silently truncated. Use exactly 16, 24, or 32 bytes.

### What to store

The `VARBINARY` column holds the IV (16 bytes) prepended to the ciphertext. PKCS7 padding always adds a full extra AES block (16 bytes) when the plaintext length is already an exact multiple of 16 — it's never zero bytes of padding. So the safe formula is plaintext length + 32 bytes total (16 for the IV, up to 16 for padding). Size the column with headroom:
