...
keywords: "mysql json, mysql json column, mysql json_extract, mysql json path, mysql json functions, mysql json index, mysql json generated column"
---
...
`->` returns a JSON value (strings include surrounding quotes). `->>` returns the unquoted string. For display, concatenation and anything that consumes the text, `->>` is what you want. Equality still works through `->`, though: MySQL coerces the other operand to JSON first, so `attributes -> '$.brand' = 'Acme'` does match.
...
| `WHERE attributes -> '$.key' = 'value'` returns no rows | The quoting is not the cause — that comparison is JSON-aware and does match. Check the path itself, and that the value is a JSON string rather than a number |
