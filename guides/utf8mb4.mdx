...
`\xF0\x9F\x9A\x80` is the rocket emoji's four bytes. With strict SQL mode off, this arrives as a warning instead, and the offending character is silently replaced with `?` while the rest of the string is kept — which is worse: you find out weeks later, from the data.
