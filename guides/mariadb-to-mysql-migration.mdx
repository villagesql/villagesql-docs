...
A schema that uses `CREATE SEQUENCE` fails immediately when loaded into MySQL, before any table is created, because `mysqldump` writes sequence definitions ahead of tables. Loading an unmodified dump containing a sequence produces:
...
A column typed `INET6` fails the same way, later in the same load, once the loader reaches the table that declares it:
...
Change the column type to `VARCHAR(45)` (long enough for an IPv6 address in text form) before loading, or to an INET type provided by an extension if one is installed on the target.
...
| `ERROR 1064 ... Expected a type or a custom type instead of "inet6"` | Change the column to `VARCHAR(45)` or a supported INET type before loading |
