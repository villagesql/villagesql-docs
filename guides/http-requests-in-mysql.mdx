...
| Field | Description |
|:---|:---|
| `status` | HTTP status code as a JSON number (`200`, `404`, etc.) |
...
On VillageSQL 0.0.6 and earlier, VEF STRING functions return binary charset. Before passing the result to `JSON_VALUE` or `JSON_EXTRACT`, wrap it with `CONVERT(... USING utf8mb4)`:

```sql
-- On 0.0.6 and earlier, this throws an error — JSON_VALUE rejects binary charset
...
VillageSQL 0.0.7 and later return the correct charset directly, so `CONVERT` is no longer required there — but it is harmless to keep, and every example below still uses it for compatibility with 0.0.6 and earlier. Assign to a variable with `CONVERT` once and reuse the variable to avoid repeating the cast.
...
**256KB response cap** — Responses larger than 256KB are truncated. This covers typical API payloads used in SQL queries; it's not suited for large file downloads. A response cut off by this cap is no longer valid JSON: `JSON_EXTRACT`/`JSON_UNQUOTE` raise `ERROR 3141` on it, while `JSON_VALUE` returns NULL with a `Warning 3141` instead of erroring, under its default `NULL ON ERROR` behavior.
...
**Synchronous** — Every HTTP call blocks until it completes or times out. `max_execution_time` does not bound or interrupt a blocking `vsql_http` call — a slow or unreachable endpoint holds the statement for its own connect/read timeout regardless. `http_get`/`http_post`/`http_put`/`http_delete`/`http_patch` take no options; only `http_request(method, url, headers_json, body, content_type, options_json)` accepts a timeout, as `{"timeout": N}` in seconds in its `options_json` argument (default 30). Use `http_request` instead of the shorthand functions when you need to shorten it (see [HTTP Webhooks](/guides/http-webhooks)).

## Troubleshooting

| Problem | Solution |
|:---|:---|
| `FUNCTION <database>.http_get does not exist` | Run `INSTALL EXTENSION vsql_http` |
...
| Query times out | Set a `{"timeout": N}` option on the request itself — `max_execution_time` has no effect on a blocking `vsql_http` call |
...
- [Serving Tables as a REST API](/guides/rest-api-from-tables) — the same server answering HTTP instead of making requests
