...
**256KB response cap** — Responses larger than 256KB are truncated. This covers typical API payloads used in SQL queries; it's not suited for large file downloads. A response cut off by this cap is no longer valid JSON, so both `JSON_VALUE` and `JSON_EXTRACT`/`JSON_UNQUOTE` raise `ERROR 3141` on it — this is a different failure from the `JSON_VALUE` size limit below, which returns NULL instead of erroring.
...
**Synchronous** — Every HTTP call blocks until it completes or times out. `max_execution_time` does not bound or interrupt a blocking `vsql_http` call — a slow or unreachable endpoint holds the statement for its own connect/read timeout regardless. For bulk UPDATE operations, set the timeout in the request itself with `{"timeout": N}` (see [HTTP Webhooks](/guides/http-webhooks)).
