...
CREATE TABLE events (
    ...
    ym          VARCHAR(7) AS (DATE_FORMAT(occurred_at, '%Y-%m')) VIRTUAL
);

-- Index the computed ym. (Note: YEAR_MONTH is a reserved word — the
-- INTERVAL unit in INTERVAL '1-2' YEAR_MONTH — so an unquoted column
-- can't be named year_month.)
ALTER TABLE events ADD INDEX idx_ym (ym);

-- This query now uses the index
SELECT * FROM events WHERE ym = '2024-06';
```
...
Change a generated column's expression. The `VIRTUAL`/`STORED` storage class has to stay the same — switching it raises `ERROR 3106: 'Changing the STORED status' is not supported for generated columns`, so changing storage means dropping and re-adding the column:

```sql
ALTER TABLE orders
MODIFY COLUMN tax_amount DECIMAL(10,2)
    AS (ROUND(subtotal * tax_rate, 2)) VIRTUAL;
```
...
Yes, for both `STORED` and `VIRTUAL` generated columns.
...
| `ERROR 3763: Expression of generated column '<col>' contains a disallowed function: <name>` | The expression uses `NOW()`, `RAND()`, `UUID()`, or a subquery — replace with deterministic alternatives |
...
- [Soft Deletes in MySQL](/guides/soft-deletes) — the unique-constraint fix for soft-deleted rows
