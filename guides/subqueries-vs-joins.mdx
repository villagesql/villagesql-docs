...
| Derived table query slow | Check if MySQL materialized the derived table — try rewriting as a CTE or a JOIN. Don't reach for `SQL_NO_CACHE`: the query cache it controlled was removed in MySQL 8.0, so the hint only raises a deprecation warning |
...
- [MySQL Views](/guides/views) — naming a query once instead of repeating it
