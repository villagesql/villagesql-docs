...
| `REPLACE` didn't match what `LIKE` matched | `REPLACE` compares byte-for-byte and ignores the connection collation, so it is case-*sensitive* even on a `_ci` column — unlike `=`, `LIKE` or `LOCATE` |
