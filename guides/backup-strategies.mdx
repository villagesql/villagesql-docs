...
| `mysqlpump` | Large (plain SQL) | Faster (parallel dump) | Slow (re-executes SQL) | Per-table consistency only | Removed in 8.4 — the binary does not ship |
...
`mysqldump` is still the go-to for portability and simplicity. MySQL Shell dump has effectively replaced `mysqlpump` for large databases — it's faster, produces smaller files, and supports MySQL 8.0+. `mysqlpump` was removed in MySQL 8.4 — the binary is not in the build at all, so any script calling it fails outright.
...
| `mysqlpump` | (Removed in MySQL 8.4 — the binary does not ship. Use MySQL Shell dump instead.) | — |
...
The backup user needs only `SELECT`, `LOCK TABLES`, `SHOW VIEW`, `TRIGGER`, `EVENT`, `RELOAD`, and `PROCESS` privileges — not `ALL PRIVILEGES`. Without `PROCESS`, `mysqldump` still completes but prints an "Access denied ... PROCESS privilege" error to stderr for every run while it tries to dump tablespace metadata.
...
| Restore fails with `ERROR 1824: Failed to open the referenced table` (or `ERROR 1452` on a row) | `mysqldump` already wraps its output in `SET FOREIGN_KEY_CHECKS=0` by default, so a full dump restored as-is won't hit this. It shows up when restoring a hand-edited or partial dump (only some tables extracted, or the FK-check lines removed) — add `SET FOREIGN_KEY_CHECKS=0;` at the top in that case |
...
Your backup scripts survive a switch to [VillageSQL](https://villagesql.com/install) untouched. It is a drop-in replacement for MySQL, so the mysqldump commands, binary log settings, and restore runbooks in this guide work as-is.
...
- [MySQL on Docker](/guides/mysql-on-docker) — backups and volumes for containerized MySQL
- [Upgrading from MySQL](/guides/upgrade) — taking the backup an upgrade to VillageSQL starts with
- [Upgrading MySQL 8.0 to 8.4 Before End of Life](/guides/mysql-80-eol-upgrade) — the 8.0 to 8.4 upgrade these backups protect
