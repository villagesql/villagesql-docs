...
| Restore fails with `ERROR 1824: Failed to open the referenced table` (or `ERROR 1452` on a row) | The dump restores a child table before its parent — add `SET FOREIGN_KEY_CHECKS=0;` at the top of the dump. `--disable-keys` does not help here; it only defers secondary-index rebuilds |
