...
ON DUPLICATE KEY UPDATE
    page_views.views   = page_views.views + new.views,
    page_views.last_at = new.last_at;
```
...
The problem with `INSERT IGNORE` is that it suppresses *all* errors, not just duplicates — and the non-duplicate cases are worse than a skipped row. `INSERT IGNORE INTO page_views (page, views) VALUES ('/new', 'not_a_number')` inserts `('/new', 0)` with only a warning: a silent, corrupted write rather than a no-op. Use it only when you genuinely don't care about the outcome.
...
| Foreign key cascades | No | Yes — the internal DELETE fires them | No |
...
`ON DUPLICATE KEY UPDATE` increments the `AUTO_INCREMENT` counter even when it takes the update path, not the insert path. Over time this creates gaps in the sequence. `page_views` uses `page` as its primary key and has no `AUTO_INCREMENT` column, so to see the effect, use a table with a surrogate key:

```sql
CREATE TABLE page_views_by_id (
    id    INT AUTO_INCREMENT PRIMARY KEY,
    page  VARCHAR(255) UNIQUE,
    views INT DEFAULT 0
);
INSERT INTO page_views_by_id (page, views) VALUES ('/home', 1);
INSERT INTO page_views_by_id (page, views) VALUES ('/home', 2) AS new
    ON DUPLICATE KEY UPDATE views = new.views;  -- updates the existing row

SHOW CREATE TABLE page_views_by_id;  -- AUTO_INCREMENT has advanced anyway
```
...
- [Bulk Inserts in MySQL](/guides/bulk-inserts) — loading many rows fast, with or without conflict handling
- [Common MySQL Errors and How to Fix Them](/guides/common-mysql-errors) — duplicate-key errors and what they mean
