...
**No index acceleration.** A preview capability for custom index types ships in the extension SDK, but the DDL that would attach one is refused on every build (`ERROR 3219: Extended Index feature not yet implemented`), so `WHERE trgm_similarity(col, 'term') > 0.3` requires a full table scan. For large tables, pre-filter with `LIKE` or full-text search to reduce the candidate set, then re-score with `trgm_similarity`.
