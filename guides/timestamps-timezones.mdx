...
Named zones like `'America/New_York'` require the timezone tables to be loaded (see [Named Time Zones](#named-time-zones) below) — a freshly initialized server doesn't have them. Check with `SELECT COUNT(*) FROM mysql.time_zone_name;` before running the example below; if it's `0`, load them first or substitute a numeric offset like `'-05:00'`.
