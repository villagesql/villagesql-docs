...
| Problem | Solution |
|:---|:---|
| `FUNCTION <database>.digest does not exist` | Run `INSTALL EXTENSION vsql_crypto` |
| `digest()` returns NULL | Check for NULL input or a genuinely unrecognized algorithm name. Case is not the problem — the name is matched case-insensitively, so `'SHA256'` and `'sha256'` return identical output |
