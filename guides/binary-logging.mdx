...
sidebarTitle: "Binary Logging"
description: "How MySQL binary logging works: enabling and configuring the binary log, log formats (ROW, STATEMENT, MIXED), using mysqlbinlog, point-in-time recovery, and replication basics."
keywords: "mysql binary log, mysql binlog, mysql enable binary logging, mysql mysqlbinlog, mysql point in time recovery, mysql binlog format, mysql replication log, mysql binlog row format"
---
...
| Format | What's logged | Use for |
|:-------|:-------------|:--------|
| `ROW` | The actual before/after row data | Safest for replication; required for some replication features |
| `STATEMENT` | The SQL statement that caused the change | Smaller logs; may be unsafe for non-deterministic statements |
| `MIXED` | STATEMENT by default; ROW for unsafe statements | Balance of size and safety |

`ROW` format is the recommended default. It replicates correctly even when non-deterministic functions like `RAND()` or `UUID()` are involved. The downside is larger log files.
...
1. Take a full backup with `mysqldump --single-transaction --master-data`:
...
`--master-data=2` writes the binlog filename and position as a comment in the dump. This is your recovery starting point.
...
```bash
head -50 backup.sql | grep "MASTER_LOG_FILE"
