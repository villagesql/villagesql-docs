...
| `STATEMENT` | The SQL statement that caused the change | Unsafe for per-call non-deterministic functions (`RAND()`, `UUID()`); `NOW()` is safe, since the statement's timestamp travels in the binlog event | Smallest | Lowest overhead | Read-heavy workloads where log size matters and all statements are deterministic |
...
Auto-purge with `binlog_expire_logs_seconds` (MySQL 8.0+; `expire_logs_days` was removed):
...
This is useful for loading data on a replica that doesn't need to propagate further, or for bulk operations you want to exclude from point-in-time recovery scope. Requires `SUPER`, `SYSTEM_VARIABLES_ADMIN` or `SESSION_VARIABLES_ADMIN` — `BINLOG_ADMIN` alone is not enough and the attempt fails with ERROR 1227.
...
- [Read Replicas in MySQL](/guides/read-replicas) — scaling reads with the log stream replicas replay
