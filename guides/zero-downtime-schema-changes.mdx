...
`SHOW PROCESSLIST` during this window showed the `ALTER` in the `copy to tmp table` state, and the `INSERT` was not visible in `SHOW PROCESSLIST` at all until the `ALTER` released its lock; it was simply waiting to acquire the table. Trying to fail fast with `LOCK=NONE` on this same statement is exactly how you find out, before you run it against a production table, that it is going to block writers for however long the rewrite takes.
