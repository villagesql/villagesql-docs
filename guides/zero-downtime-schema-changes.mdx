...
`INSTANT` is not available for every operation. Adding a column with a `BLOB`/`TEXT` default, or changing an existing column's type, falls back to `INPLACE` or `COPY`. Adding a column at any position, including the middle of a table, uses `INSTANT` too (MySQL 8.0.29+).
...
# Linux (Debian/Ubuntu)
curl -LO https://github.com/github/gh-ost/releases/latest/download/gh-ost.amd64.deb
sudo dpkg -i gh-ost.amd64.deb
```
...
With a real password, pass it through a `-conf` file instead of `--password`
on the command line — a command-line password is visible to any other user
on the host via `ps` and lands in shell history.
...
| `gh-ost` refuses to start | Confirm `binlog_format=ROW` and that the target table has a primary key or a unique key; both are required. |
