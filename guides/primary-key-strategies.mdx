...
sidebarTitle: "Choosing Primary Keys"
...
-- UUID_NS_DNS() returns the RFC 9562 DNS namespace, so no literal is needed
SELECT UUID_V5(UUID_NS_DNS(), 'user@example.com');
```
...
Additional functions: `UUID_IS_VALID(string)` returns 1 or 0 for whether a non-NULL string is a well-formed UUID, and NULL when the argument itself is NULL. It takes text and never raises, which is what you want for IDs arriving from external systems. `UUID_VERSION(uuid)` returns the version number, for routing an ID once it is already stored as a `UUID`. `UUID_COMPARE(uuid1, uuid2)` returns -1, 0, or 1 for explicit lexicographic comparison.
...
MySQL itself allows non-deterministic functions like `UUID()`, `RAND()`, and `NOW()` in `DEFAULT` expressions. The real restriction is VillageSQL-specific: extension functions registered as non-deterministic — like `UUID_V7()` — are blocked from `DEFAULT` expressions, while deterministic ones like `UUID_V3()`/`UUID_V5()` are allowed. That permission isn't useful for a primary key here, though: `UUID_V3()`/`UUID_V5()` take fixed arguments in a `DEFAULT` expression (a `DEFAULT` can't read other columns), so every row gets the identical UUID — the second `INSERT` collides with the first on the primary key. Generate `UUID_V7()` in each `INSERT` statement instead.
...
| Problem | Solution |
|:---|:---|
| `FUNCTION <database>.UUID_V7 does not exist` | Run `INSTALL EXTENSION vsql_uuid` |
...
| Join on a UUID FK fails with `ERROR 3219: Unable to implicitly cast a non-custom type during compare with a custom type in =` | Both sides must be the native `UUID` type. Joining a `UUID` column against a `CHAR(36)`/`VARCHAR` UUID string raises this error rather than silently mismatching |
...
- [Pagination in MySQL: Keyset vs OFFSET](/guides/pagination) — keys double as pagination cursors
