...
Additional functions: `UUID_IS_VALID(string)` returns 1 or 0 for whether a string is a well-formed UUID, which is what you want for IDs arriving from external systems — it takes text and never raises. `UUID_VERSION(uuid)` returns the version number, for routing an ID once it is already stored as a `UUID`. `UUID_COMPARE(uuid1, uuid2)` returns -1, 0, or 1 for explicit lexicographic comparison.
