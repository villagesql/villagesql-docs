...
sidebarTitle: "Running AI Prompts"
...
-- Enrich a single row
SELECT review_text,
       ai_prompt('anthropic', 'claude-haiku-4-5', @key,
...
WHERE ai_prompt('anthropic', 'claude-haiku-4-5', @key,
...
- **Use LIMIT in UPDATE** — AI calls take 5–30 seconds each, so a batch `UPDATE` over 100 rows takes minutes; process in batches of 50–100 rows to stay within provider rate limits. `max_execution_time` only bounds `SELECT` and does nothing here — to cap a running `UPDATE`, use `KILL QUERY <connection_id>` from another session.
...
UPDATE reviews
SET ai_label = ai_prompt('anthropic', 'claude-haiku-4-5', @key,
...
|:---|:---|:---|:---|
| `claude-haiku-4-5` | Fast | Low | Bulk classification, simple labeling |
| `claude-sonnet-5` | Medium | Medium | Complex reasoning, nuanced output |
| `claude-opus-5` | Slow | High | Recommended default — agentic and enterprise work |
| `claude-fable-5` | Slow | High | Most capable — demanding reasoning, long-horizon work |
...
No. If the API call fails, it returns NULL. Run `SHOW WARNINGS` to see why — `vsql_ai` raises a `Warning 3200` with the real failure reason — before implementing retry logic in your application's batch loop.

## Troubleshooting

| Problem | Solution |
|:---|:---|
| `FUNCTION <database>.ai_prompt does not exist` | Run `INSTALL EXTENSION vsql_ai` |
| Returns NULL | Bad API key, rate limit hit, or timeout — check `max_execution_time` |
| UPDATE runs too long | Use smaller `LIMIT` batches; `max_execution_time` does not apply to `UPDATE`, use `KILL QUERY <connection_id>` from another session instead |
...
- [Connecting MySQL to AI APIs](/guides/ai-api-setup) — installing and configuring the vsql_ai extension first
- [AI Summarization in MySQL](/guides/ai-summarization) — summarizing long text with ai_prompt()
- [Classifying Text in MySQL with AI](/guides/text-classification) — labeling rows by category using ai_prompt()
