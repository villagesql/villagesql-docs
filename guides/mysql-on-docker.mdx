...
| | Official `mysql:8.4` | `villagesql/server:latest` |
|:---|:---|:---|
...
| Image availability | Docker Hub | Docker Hub (`villagesql/server`) |
...
| VillageSQL extensions needed | Use `villagesql/server` image | Build from source | Not available on managed services |
...
Everything on this page also works with the `villagesql/server` image. VillageSQL is a drop-in replacement for MySQL with extensions, so the compose files, volumes, and health checks above carry over with one image-name change. [Try it in Docker](https://villagesql.com/install).
...
- [Upgrading from MySQL](/guides/upgrade) — moving a container from MySQL to VillageSQL
- [Running MySQL on Kubernetes](/guides/mysql-on-kubernetes) — the next step up in container orchestration
