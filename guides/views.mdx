...
- Every column you write belongs to a single underlying table. A view over a JOIN is still updatable under that rule, but an `INSERT` through it silently touches only one table and leaves the other's columns unset
- No `DISTINCT`
