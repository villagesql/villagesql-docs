...
- Every column you write belongs to a single underlying table. A view over a JOIN is still updatable under that rule, but an `INSERT` through it silently touches only one table and inserts no row into the other at all. Writing columns from both underlying tables in one `INSERT` isn't silent, though — it raises `ERROR 1393: Can not modify more than one base table through a join view`
- No `DISTINCT`
...
If MySQL can't satisfy the requested algorithm it raises Warning 1354 and stores the view as `ALGORITHM=UNDEFINED` — that is what `SHOW CREATE VIEW` reports, not `TEMPTABLE`. The optimizer then materializes such views at execution time.
...
- [Subqueries vs JOINs in MySQL](/guides/subqueries-vs-joins) — the query shapes views wrap
- [Soft Deletes in MySQL](/guides/soft-deletes) — hiding deleted rows behind a live-rows view
