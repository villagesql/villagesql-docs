...
SELECT NULL = '';      -- NULL
SELECT 1 + NULL;       -- NULL
SELECT 'hello' || NULL; -- NULL (logical OR with NULL is unknown, not concatenation — see below)
```

By default MySQL's `sql_mode` does not include `PIPES_AS_CONCAT`, so `||` is logical OR, not string concatenation — `'hello' || 'world'` evaluates to `0`, not `'helloworld'`. This differs from PostgreSQL and the ANSI standard, where `||` always concatenates. Use `CONCAT()` for string concatenation in MySQL.
...
-- This returns only 1 row — both the 'google' row and the NULL row are excluded
...
- [Real Booleans in MySQL](/guides/boolean-columns) — how the boolean aggregates treat NULL
