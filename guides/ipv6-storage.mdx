...
sidebarTitle: "Storing IPv6 Addresses"
...
-- Under MySQL 8.4's default strict mode this raises ERROR 1411 and the row is
-- rejected. A bare SELECT INET6_ATON('not-an-ip') returns NULL with a warning.
...
No — it's the correct MySQL-only approach. It's more compact than INET (4 or 16 bytes vs a fixed 19), but you lose built-in family detection, prefix handling, and validation. For schemas that need to work across both IPv4 and IPv6 with prefix-aware operations, `INET` is more ergonomic.
...
| `INET6_ATON` returns different values than `inet_from_string` | The underlying binary differs in size (4 or 16 bytes vs a fixed 19); they're not interchangeable |
...
- [Storing MAC Addresses in MySQL](/guides/storing-mac-addresses) — MACADDR types from the same extension
