...
description: "How to use Common Table Expressions in MySQL: WITH clause syntax, multiple CTEs, recursive CTEs for hierarchical data, and CTEs vs subqueries."
...
The `RECURSIVE` keyword is required — without it, MySQL doesn't recognize the self-reference and returns `ERROR 1146: Table '<database>.org_chart' doesn't exist`. A recursive CTE has two parts joined by `UNION ALL`:
...
Usually the same. The MySQL optimizer often inlines a CTE the same way it would a subquery. CTEs referenced multiple times may be materialized into a temporary table, which can be faster (avoids re-executing the subquery) or slower (extra I/O). If you suspect unwanted materialization, use `EXPLAIN FORMAT=TREE` and look for "Materialize CTE ... if needed" — the default `EXPLAIN`'s Extra column never shows the word "MATERIALIZED".
...
| `ERROR 1146: Table '<database>.cte_name' doesn't exist` | Missing `RECURSIVE` keyword — add `WITH RECURSIVE` when the CTE references itself |
| `ERROR 3636: Recursive query aborted after N iterations. Try increasing @@cte_max_recursion_depth to a larger value.` | Add a depth counter and `WHERE depth < N` guard condition, or raise `@@cte_max_recursion_depth` |
...
| CTE performing worse than subquery | Check `EXPLAIN FORMAT=TREE` for "Materialize CTE ... if needed" — try inlining the CTE as a subquery to compare |
...
- [MySQL Views](/guides/views) — persisting a CTE-style abstraction as a schema object
