...
sidebarTitle: "Next.js with Prisma"
...
`@prisma/adapter-mariadb` is the adapter for both MariaDB and MySQL: the wire protocol is shared, so it works against VillageSQL the same way it works against stock MySQL. Set up a singleton so Next.js's dev-mode hot reload doesn't open a new connection pool on every file save. `new PrismaMariaDb(...)` calls `mariadb.createPool()` in its own constructor, so the adapter has to be built inside the factory rather than at module scope, or a reload opens a pool even when it reuses the cached client:
