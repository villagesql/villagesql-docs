...
- Adding a column, at any position (MySQL 8.0.29+)
...
Adding a nullable column, at any position, uses `ALGORITHM=INSTANT` and completes in milliseconds regardless of table size (MySQL 8.0.29+). If it doesn't, use `ALGORITHM=INPLACE, LOCK=NONE`, or pt-osc or gh-ost.
