...
`HAVING COUNT(*) >= 5` works because `COUNT(*)` is available after aggregation. `WHERE COUNT(*) >= 5` raises `ERROR 1111 (HY000): Invalid use of group function` — `COUNT()` doesn't exist at the WHERE stage. That is a semantic error, not a parser one.
...
SELECT
    ...
    SUM(amount) AS revenue
FROM (
    SELECT category, DATE_FORMAT(created_at, '%Y-%m') AS month, amount
    FROM orders
) t
GROUP BY category, month WITH ROLLUP;
```

MySQL adds NULL for the grouped columns in subtotal and grand total rows. `COALESCE` replaces those NULLs with readable labels. The month is computed in a
derived table so the `GROUP BY` can name a plain column: MySQL's
functional-dependency check does not see through a `COALESCE()` wrapped around
the grouped expression, and grouping by the raw `DATE_FORMAT(...)` while
selecting the `COALESCE()` of it fails under the default `ONLY_FULL_GROUP_BY`
with `ERROR 1055`. `WITH ROLLUP` is useful for summary reports where you want group subtotals without running multiple queries.
...
- [Common MySQL Errors and How to Fix Them](/guides/common-mysql-errors) — the ONLY_FULL_GROUP_BY error explained
