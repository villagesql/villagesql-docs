...
Summarization is generally well-handled by faster, cheaper models. Use `claude-haiku-4-5` or `gpt-4o-mini` for bulk summarization. Step up to `claude-sonnet-5` when:
...
| Returns NULL | Run `SHOW WARNINGS` for the real cause, then check API key, `max_execution_time`, and model name |
...
- [Running AI Models from MySQL Queries](/guides/ai-prompts-in-mysql) — the ai_prompt() function this guide builds on
- [Sentiment Analysis on MySQL Data](/guides/sentiment-analysis) — another text analysis pattern with ai_prompt()
- [Classifying Text in MySQL with AI](/guides/text-classification) — categorizing text using the same approach
