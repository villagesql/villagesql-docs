...
`REPLACE` executes as a DELETE + INSERT when there's a conflict. On conflict all four fire on the target table, in this order: `BEFORE INSERT` (the duplicate-key check), then `BEFORE DELETE` and `AFTER DELETE` for the conflicting row, then `AFTER INSERT`. Because the delete is a real row delete, `ON DELETE CASCADE` rules on referencing tables DO fire.
