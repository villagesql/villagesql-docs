...
Key length is enforced by the byte count of the key string. A key shorter than the cipher requires raises `ERROR 3200: VDF error in function "encrypt": key too short for <cipher>: need N bytes, got M` rather than being zero-padded; a longer key is silently truncated to the required length. Check `LENGTH(@key)` if you're getting unexpected behavior.
...
| Problem | Solution |
|:---|:---|
| `FUNCTION <database>.encrypt does not exist` | Run `INSTALL EXTENSION vsql_crypto` |
...
| `key too short for <cipher>` | Check `LENGTH(@key)` — it must be at least the cipher's size (16, 24, or 32 bytes). A longer key is truncated silently, so use the exact size to avoid surprises |

## See also

- [Encrypting Columns in MySQL](/guides/encrypting-columns) — applying encryption at the column level with vsql_crypto
