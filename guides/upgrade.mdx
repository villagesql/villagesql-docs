...
sidebarTitle: "Upgrading to VillageSQL"
description: "Step-by-step guide to upgrading from upstream MySQL to VillageSQL."
---
...
VillageSQL Server is a drop-in replacement for MySQL. For a plain single-instance MySQL 8.4 install with no third-party plugins, migrating to VillageSQL is functionally identical to a minor version upgrade (e.g., 8.4.5 to 8.4.6), preserving your existing data. Anything relying on non-standard plugins should be tested on a copy first.
...
- [MySQL on Docker](/guides/mysql-on-docker) — running MySQL or VillageSQL in a container instead
- [Upgrading MySQL 8.0 to 8.4 Before End of Life](/guides/mysql-80-eol-upgrade) — getting to 8.4 first if you are still on 8.0
