...
More secure than MySQL 5.7 defaults, but still not hardened for production. MySQL 8.0 introduced random root passwords and SSL by default; 8.4 keeps those improvements. You still need to: verify the root password is strong, remove anonymous users, restrict host access, and configure the `validate_password` component.
