...
The ~8KB figure, roughly half the default 16KB page, is where InnoDB moves variable-length column data off-page — it isn't a row-size ceiling. MySQL's actual row-size limit is 65,535 bytes, not counting BLOB/TEXT overflow pages, and applies regardless of `ROW_FORMAT`: `ERROR 1118: Row size too large. The maximum row size for the used table type, not counting BLOBs, is 65535.`
