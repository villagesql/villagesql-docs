...
| Format | Variable-length columns | Inline storage before overflow | Notes |
|:-------|:------------------------|:--------------------------------|:------|
...
The ~8KB figure, roughly half the default 16KB page, is where InnoDB moves variable-length column data off-page, so a row with few large `VARCHAR`/`TEXT`/`BLOB` columns can grow well past it without hitting a ceiling. But columns that store inline instead of moving off-page — many fixed-width `CHAR` columns, for example — still count fully against InnoDB's own row-size limit, and hitting it raises `ERROR 1118 (42000): Row size too large (> 8126). Changing some columns to TEXT or BLOB may help.` MySQL's separate, higher row-size limit is 65,535 bytes not counting BLOB/TEXT overflow pages, and applies regardless of `ROW_FORMAT`; both limits are real, and either can be the one you hit first depending on which columns are large and whether they can move off-page.
...
VillageSQL ships the same InnoDB engine, so every setting on this page applies unchanged. [Install VillageSQL](https://villagesql.com/install) to keep InnoDB and gain an extension framework for custom types, functions, and indexes.
