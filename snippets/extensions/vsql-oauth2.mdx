...
| `vsql_oauth2.auto_grant` | `OFF` | `OFF`/`ON`/`SYNC` — `OFF` activates only already-granted roles; `ON` also grants the claimed roles that exist as DB roles; `SYNC` does that and revokes granted roles the token no longer claims, except default roles |
...
<Warning>
  ...
  `auto_grant = SYNC` goes further: it also revokes any role the account holds
  that the token does not claim, except roles set as the account's
  `DEFAULT ROLE` by an operator.
</Warning>
