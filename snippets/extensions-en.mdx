...
The following extensions are maintained in separate repositories. Most of them already ship pre-built with the standard install methods (installer script, Docker image, release tarball) — `INSTALL EXTENSION` is all that's needed. `vsql_currency`, `vsql_mcp`, and `vsql_stat_ch` are the exceptions and still need a separate build. Each extension's own repository has its full installation instructions and documentation.

### vsql_ai

**Repository:** [villagesql/vsql-ai](https://github.com/villagesql/vsql-ai)

AI-powered SQL functions. AI prompting and embedding functions for SQL queries. `ai_prompt()` supports Anthropic (`'anthropic'`), Google (`'google'`), OpenAI (`'openai'`), and local Ollama (`'local'`); `ai_embedding()` supports Google (`'google'`), OpenAI (`'openai'`), and local Ollama (`'local'`).
...
ISO 4217 currency code type. A `currency` column type holding any of 164 currency codes in a single byte, with case-insensitive input, canonical uppercase output, alphabetical ordering, and index support. Also ships `supported_currencies()`, which returns the ISO 4217 code list — optionally narrowed by a prefix — as a JSON array. Rust port of `adjust/pg-currency`.

### vsql_oauth2

**Repository:** [villagesql/vsql-oauth2](https://github.com/villagesql/vsql-oauth2)

OAuth2/OIDC JWT authentication. An account created with `IDENTIFIED WITH vsql_oauth2` authenticates by presenting a JWT instead of a password. The extension verifies the RSA (RS256/RS384/RS512) or ECDSA (ES256/ES384/ES512) signature against signing keys fetched from a JWKS endpoint (`jwks_url`, which takes precedence and is refreshed on an interval) or a static public key, checks `exp` — and `iss`/`aud` when configured — then maps a claim to a VillageSQL account. It can also map a role claim onto database roles and, off by default, create the account or grant those roles at login. Preview capability: requires a server started with `--vsql_allow_preview_extensions=ON`.
...
Model Context Protocol server. Exposes the database as a tool surface for AI agents over MCP Streamable HTTP, served by a listener inside the server process. An agent can list schemas, describe tables, and run read-only queries; writes stay off until you enable them. Schema and table browsing runs in-process; the `query`, `write`, and `explain` tools and the table-DDL resource run through a dedicated database account configured by `vsql_mcp.db_url`, with an optional table allowlist, row caps, and per-call timeouts. Preview capability: requires a server started with `--vsql_allow_preview_extensions=ON`.

### vsql_duckdb

**Repository:** [villagesql/vsql-duckdb](https://github.com/villagesql/vsql-duckdb)

DuckDB embedded in the server process. Query Parquet, CSV, and JSON files in object storage or on disk with `duckdb_query()`, which returns a JSON array of rows, and `duckdb_scalar()`, which returns a single value; `duckdb_status()` reports the linked readers and current settings. Requires a VillageSQL server newer than 0.0.6. Preview capability: requires a server started with `--vsql_allow_preview_extensions=ON`.
...
Statistical aggregate functions for data analysis. Quartiles and Tukey fence outlier detection, two-sample t-tests, one-sample z-tests, one-way ANOVA, chi-squared, covariance, skewness, kurtosis, mode, and trimmed, winsorized, geometric and harmonic means. Each function returns its figures as a JSON string: cast it with `CAST(... AS CHAR)` and read the fields with `JSON_EXTRACT()`. Requires VillageSQL 0.0.4.
