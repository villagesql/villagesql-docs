...
OAuth2/OIDC JWT authentication. An account created with `IDENTIFIED WITH vsql_oauth2` authenticates by presenting a JWT instead of a password. The extension verifies the signature against a configured RSA (RS256/RS384/RS512) or ECDSA (ES256/ES384/ES512) public key, checks `exp` — and `iss`/`aud` when configured — then maps a claim to a VillageSQL account. Preview capability: requires a server started with `--vsql_allow_preview_extensions=ON`.
