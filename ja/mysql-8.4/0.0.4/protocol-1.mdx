...
プロトコル 1 は元の VEF インターフェースです。v0.0.1 で安定しており、0.0.5 以降は廃止されます。新しい拡張機能では、[拡張機能の作成](/ja/mysql-8.4/0.0.4/create) および [開発](/ja/mysql-8.4/0.0.4/development) ガイドにあるテンプレートベースの型ビルダーを使用して、プロトコル 3 API を使用する必要があります。
...
プロトコル 1 の VDF 実装では、型付きラッパーを使用する代わりに、生の C 構造体を直接渡すことができます。このスタイルは 0.0.5 以降は廃止されます。すべての新しいコードでは、[型付きラッパー](/ja/mysql-8.4/0.0.4/development#typed-wrappers-recommended) を使用してください。
...
集計関数の場合、`.clear<>()` および `.accumulate<>()` は生の ABI 関数ポインタシグネチャを受け入れます。`.prerun<>()` および `.postrun<>()` は受け入れません。ビルダーは、プロトコル 1 の拡張機能であっても、型付きシグネチャ (`void(PrerunArgs, PrerunResult)` および `void(PostrunArgs)`) を必要とします。すべての新しいコードでは、[型付き集計関数のアプローチ](/ja/mysql-8.4/0.0.4/development#aggregate-vdfs) を使用してください。
