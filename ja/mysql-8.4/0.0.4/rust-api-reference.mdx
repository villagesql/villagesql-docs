...
このページは、`villagesql` クレート API のリファレンスです。 入門チュートリアルについては、[Rust で拡張機能を構築する](/mysql-8.4/0.0.4/rust-sdk) を参照してください。 カスタム型については、[Rust のカスタム型](/mysql-8.4/0.0.4/rust-custom-types) を参照してください。
