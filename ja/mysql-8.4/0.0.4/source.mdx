...
<Tip>
  並列処理の度合い（`-j10`）は、CPUコア数に基づいて調整します。システムの応答性を維持するために、合計コア数から2〜4を引いてください。たとえば、12コアのマシンでは、`-j10`を使用します。
</Tip>
...
システムデータベースが作成されたことを確認して、初期化が成功したことを確認します。
...
GDBデバッグ、テストの実行、サーバーコードベースへの貢献については、[サーバー開発ガイド](/ja/mysql-8.4/0.0.4/server-development)を参照してください。
