...
カスタムタイプを使用すると、`ORDER BY`、インデックス、および集計関数で機能する`COMPLEX`、`UUID`、または`VECTOR`のような新しいカラムタイプを定義できます。このページは、[拡張機能の作成](/mysql-8.4/0.0.4/create)チュートリアルのステップ4です。ここで続行する前に、ステップ1〜3を完了してください。
...
完全なC++シグネチャについては、[タイプ操作ビルダー](/mysql-8.4/0.0.4/development#type-operation-builders)を参照してください。
...
完全な実装については、[vsql_complexの例](/mysql-8.4/0.0.4/examples)を参照してください。
