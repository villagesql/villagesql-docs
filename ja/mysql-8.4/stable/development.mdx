...
このガイドは、C++ VDF 実装を作成するための詳細なリファレンスです。エンドツーエンドのビルド手順をカバーする [Creating Extensions in C++](/ja/mysql-8.4/stable/create) と、テストと反復のループをカバーする [C++ Testing](/ja/mysql-8.4/stable/testing) の補完資料です。
...
**引数型:** `IntArg`, `RealArg`, `StringArg`, `CustomArg` — 各型は `is_null()` と `value()` を提供します。パラメータ化されたカスタム型の場合、`CustomArgWith<P>` はキャッシュされた解析済みの params 構造体を返す `params()` アクセサを追加します（[Parameterized Types](/ja/mysql-8.4/stable/type-operations#parameterized-types) を参照）。
...
型演算ビルダーの完全なリファレンス — エンコード、デコード、比較、ハッシュ、組み込みデフォルト、およびパラメータ化された型 — については、[Type Operations](/ja/mysql-8.4/stable/type-operations) を参照してください。

## プレビュー機能

以下の VEF 機能は、オプトインのプレビューヘッダーとして利用可能です。ABI と API はまだ活発に開発中です。完全なリファレンスについては、[Preview Capabilities](/ja/mysql-8.4/stable/preview-capabilities) を参照してください。

- **拡張機能システム変数** — [Preview Capabilities → System Variables](/ja/mysql-8.4/stable/preview-capabilities#system-variables)
- **拡張機能ステータス変数** — [Preview Capabilities → Status Variables](/ja/mysql-8.4/stable/preview-capabilities#status-variables)
- **キーリングアクセス** — [Preview Capabilities → Keyring Access](/ja/mysql-8.4/stable/preview-capabilities#keyring-access)
- **列ストレージ** — [Preview Capabilities → Column Storage](/ja/mysql-8.4/stable/preview-capabilities#column-storage)
...
- [Creating Extensions in C++](/ja/mysql-8.4/stable/create) — エンドツーエンドのビルド手順、CMake セットアップ、およびインストール
- [C++ Testing](/ja/mysql-8.4/stable/testing) — ローカル開発サーバー、MTR、および失敗のデバッグ
- [Type Operations](/ja/mysql-8.4/stable/type-operations) — エンコード、デコード、比較、ハッシュ、パラメータ化された型
- [C++ API Reference](/ja/mysql-8.4/stable/extension-api-reference) — VDF 契約、null 処理、およびバッファサイズ設定
- [Extension Architecture](/ja/mysql-8.4/stable/architecture) — ライフサイクル、Victionary キャッシュ、パフォーマンスパターン、およびセキュリティモデル
