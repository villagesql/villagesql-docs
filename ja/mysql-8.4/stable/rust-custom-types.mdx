...
<Warning>
  Rust SDKはアルファ版です。リリース間で破壊的なAPI変更が発生する可能性が
  あります。関数のみの拡張機能とカスタム型（エンコード、デコード、比較、
  ハッシュ）はサポートされています。集計、`prerun()`、`VarArgs`、システム
  変数とステータス変数、キーリングアクセス、およびカラムストレージABIは
  現在C++でのみ利用可能です。これらのいずれかが必要な場合は、
  [C++ SDK](/ja/mysql-8.4/stable/create)を使用してください。
</Warning>
...
このページでは、まず[Rustでの拡張機能の作成](/ja/mysql-8.4/stable/rust-sdk)を完了していることを前提としています。セットアップ（Cargo.toml、manifest.json、cargo-vsql）は同じです。
...
いずれのセクションも、空の場合は省略できます。型のみの拡張機能は`funcs:`を省略します。関数のみの拡張機能は`types:`を省略します。
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="wrench"
    href="/ja/mysql-8.4/stable/rust-sdk"
  >
  ...
  <Card
    title="C++カスタム型"
    icon="shapes"
    href="/ja/mysql-8.4/stable/custom-types"
  >
