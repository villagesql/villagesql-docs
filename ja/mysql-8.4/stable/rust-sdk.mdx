...
<Warning>
  Rust SDK はアルファ版です。リリース間で破壊的な API 変更が発生する可能性があります。関数のみの拡張機能とカスタム型（encode、decode、compare、hash）に加えて、`sys_var`、`status_var`、`thread_worker`、`keyring` のプレビュー機能もサポートされています。集約関数、`VarArgs`、および列ストレージ ABI は現在 C++ のみで利用可能です。これらのいずれかが必要な場合は [C++ SDK](/ja/mysql-8.4/stable/create) を使用してください。
</Warning>
