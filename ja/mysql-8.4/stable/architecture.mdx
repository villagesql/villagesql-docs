...
**メモリオーバーヘッド：** コミット済みの各エントリは、`std::map<std::string, std::shared_ptr<EntryType>>` のノードとディスクリプタのペイロードで構成されます。エントリあたりの固定値はソース上で管理されていません。
...
<CardGroup cols={2}>
  ...
  <Card
    title="実装例"
    icon="lightbulb"
    href="/ja/mysql-8.4/stable/examples"
  >
    vsql_complex の実装を学ぶ
  </Card>
  <Card
