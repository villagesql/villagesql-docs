...
このページは、C++タイプ操作ビルダーのリファレンスです。カスタムタイプのチュートリアルレベルの導入については、[C++でのカスタム型](/ja/mysql-8.4/stable/custom-types)を参照してください。
...
カスタムハッシュ関数を省略すると、タイプの生の格納バイトをハッシュする方式にフォールバックします。これは、エンコード関数が、論理的に等しい値に対して複数のバイナリ表現を生成しうるタイプでは安全ではありません。よくあるケースは浮動小数点フィールドで、`-0.0`と`+0.0`は等しいと比較されるにもかかわらずビット単位では異なるため、`GROUP BY`と`COUNT(DISTINCT)`がそれらを異なる値として扱ってしまいます。
...
カスタムタイプが宣言できるストレージは、値ごとに最大**65532バイト**です。カスタムカラムは`VARBINARY`フィールドによって支えられており、MySQLの1行あたり65535バイトの予算からは、そのフィールドの長さを表す2バイトと行のNULLバイトも支払われます。カラムがNULL可能かどうかは`CREATE TABLE`時にカラムごとに決まり、NULL可能がデフォルトであるため、この上限はNULLバイトの分を残しておく必要があります。`persisted_length`と`max_persisted_length`の両方がこの上限に対してチェックされ、そのチェックは`CREATE TABLE`時ではなく`INSTALL EXTENSION`時に実行されます。そのため、幅が大きすぎるタイプは拡張機能全体とともに拒否されます。
...
上限を下回っていても、カラムが収まることが保証されるわけではありません。テーブル内の他のすべてのカラムも同じ65535バイトの行予算から引き出され、それを判断できるのは`CREATE TABLE`だけです。そのため、上限またはその付近のタイプは、そこで`ER_TOO_BIG_ROWSIZE`により失敗する可能性があります。同じ幅の通常の`VARBINARY`も同じ動作をします。
...
組み込みデフォルトは、次の2つの方法のいずれかで指定します。この2つは相互に排他的です。`.intrinsic_default_str()`と`.intrinsic_default_vdf()`の両方を宣言すると、コンパイルエラーになります。
...
`.params<>()`または`.int_to_params<>()`が登録されている場合、`build()`は常に
`.resolve_params<>()`を必要とします。`int_to_params`は`MYTYPE(N)`という短縮形を
パラメータ文字列に書き換えるだけであり、パラメータを検証してストレージサイズを
解決するゲートは`resolve_params`です。これを省略すると、コンパイルエラーになります。

<Warning>
`.max_persisted_length()`にはVEFプロトコル3以上が必要です。これを使用するタイプは、プロトコル3より前のサーバーではロードできません。これは、可変長タイプまたはパラメータ化されたタイプでのみ有効です。パラメータ化されていない固定長タイプがこれを設定すると、コンパイルに失敗します。
</Warning>

`resolve_params`が報告するサイズは、カラムのパラメータが判明した`CREATE TABLE`時にも
検証されます。固定長タイプの場合、解決された`persisted_length`は、宣言された
`max_persisted_length`を超えてはなりません。計算されたフットプリント（例えば
`dimension * 4`によるサイズ決定）がその上限を超えるパラメータ化は、構文エラーで
拒否されます。長さの種類とは無関係に、`resolve_params`はゼロより大きい
`max_decode_buffer_length`を解決する必要があります。この値はデコード用の
スクラッチバッファーのサイズを決定するためであり、正でない結果は同じ種類のエラーで
拒否されます。
...
`SHOW CREATE TABLE`は、作者が書いた内容をそのまま繰り返すわけではありません。サーバーは
解決済みのパラメータマップを保持し、そこから表示形式を再構築します。`int_to_params(N)`が
マップ全体を再現できる場合は`MYTYPE(N)`が優先され、そうでない場合はキーをソートした
正規形式の`MYTYPE('key=value,...')`にフォールバックします。したがって、
`MYTYPE('dimension=8')`と書かれたカラムは、`MYTYPE(8)`として読み戻されます。
...
割り当てるために上限を必要とします。可変長タイプは、`.persisted_length()`を併せて
呼び出してはなりません。そのサイズは値ごとに決まるため、`build()`はこの組み合わせを
コンパイル時に拒否します。
...
- [C++でのカスタム型](/ja/mysql-8.4/stable/custom-types) — カスタムタイプのチュートリアル導入
- [C++ APIリファレンス](/ja/mysql-8.4/stable/extension-api-reference) — VDFコントラクト、null処理、およびバッファーサイズ
- [C++開発](/ja/mysql-8.4/stable/development) — VDFの作成の詳細、引数と結果のタイプ、集計、可変長引数
