...
<Note>
`UNINSTALL EXTENSION`には`EXTENSION_ADMIN`権限（または代替として`SUPER`権限）が必要です。[必要な権限](/ja/mysql-8.4/stable/install#required-privilege)を参照してください。
</Note>
...
<Note>
保留中のバージョン変更（キューに入れられた`ALTER EXTENSION name VERSION 'x' AT RESTART`）がある拡張機能をアンインストールすると、その保留中のアクションは拡張機能の行とともに破棄されます。クリーンアップすべき孤立した保留状態は残らず、エラーも発生しません。その後の`INSTALL EXTENSION`は、保留中のアクションなしで新規に開始されます。保留中のバージョン変更がどのように追跡され、クリアされるかについては、[拡張機能の管理](/ja/mysql-8.4/stable/managing)を参照してください。
</Note>
...
| エラー | 原因 | 解決策 |
|-------|-------|----------|
| `Cannot drop extension \`name\` as N column(s) depend on it, e.g. db.table.col has type TYPE` | テーブルに、拡張機能のカスタムタイプを使用する列があります | 特定された列を削除または変更してから、再試行します |
| `Cannot uninstall extension 'name': stored procedure db.sp uses custom type TYPE` | ストアドプロシージャに、拡張機能のカスタムタイプを使用するパラメータまたはDECLARE変数があります | ストアドプロシージャを削除または変更して、カスタムタイプの使用を削除してから、再試行します |
| `Cannot uninstall extension 'name': installed version is 'X' but VERSION 'Y' was specified` | `VERSION`句が指定されましたが、インストールされているバージョンと一致しません | `VERSION`句なしで再実行するか、`SELECT EXTENSION_VERSION FROM INFORMATION_SCHEMA.EXTENSIONS`に表示されているバージョンを使用します |
| `Extension 'name' is not installed` | 拡張機能がインストールされていません | `SELECT * FROM INFORMATION_SCHEMA.EXTENSIONS`で確認します |
...
<CardGroup cols={2}>
  ...
  <Card
    title="システムリファレンス"
    icon="book"
    href="/ja/mysql-8.4/stable/reference"
  >
    システムテーブルをクエリします
  </Card>
</CardGroup>
