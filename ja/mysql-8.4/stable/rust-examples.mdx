...
このページでは、Rust SDK リポジトリにある参照拡張のうち 4 つを取り上げます。関数のみの最小限の例、算術演算、順序付け、ハッシュ化を備えた完全なカスタム型、集約関数、そして可変長引数関数です。リポジトリの `examples/` ディレクトリには、[Rustにおけるプレビュー機能](/ja/mysql-8.4/stable/rust-preview-capabilities)ごとの例が 1 つずつ含まれているほか、プレビュー機能ではなくステートメントごとの状態ライフサイクルを示す `vsql_call_index` と `vsql_postrun_order` も含まれています。
