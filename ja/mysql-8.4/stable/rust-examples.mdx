...
このページでは、Rust SDK リポジトリにある参照拡張のうち 2 つを取り上げます。関数のみの最小限の例と、算術演算、順序付け、ハッシュ化を備えた完全なカスタム型です。リポジトリの `examples/` ディレクトリには、サポートされている[Rustにおけるプレビュー機能](/ja/mysql-8.4/stable/rust-preview-capabilities)ごとの例も 1 つずつ含まれています。
...
（注: マニフェストの説明文は古くなっています。関数は `vsql_rot13` ではなく `rot13` として登録されます。アップストリームでの修正は別途追跡されています）
