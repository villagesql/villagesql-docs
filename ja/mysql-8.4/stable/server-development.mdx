...
このガイドでは、デバッグ、サーバーテストスイートの実行、およびVillageSQLへの貢献について説明します。動作するソースビルドがあることを前提としています。まだビルドしていない場合は、[ソースからのビルド](/ja/mysql-8.4/stable/source)を参照してください。
...
拡張回帰テストの作成については、[開発ガイド](/ja/mysql-8.4/stable/development)を参照してください。
