...
VDFの実装の詳細（引数と結果の型、集約、システム変数、パラメータ化された型）については、[開発ガイド](/ja/mysql-8.4/stable/development)を参照してください。

<Note>
Rustを使用する場合は、[Rustでの拡張機能の作成](/ja/mysql-8.4/stable/rust-sdk)を参照してください。
</Note>
...
SQL、ファイル名、リポジトリ名全体での完全な命名規則については、[拡張機能の命名規則](/ja/mysql-8.4/stable/install#extension-naming-conventions)を参照してください。
...
完全な`CustomArg`/`CustomResult`APIについては、[開発ガイド](/ja/mysql-8.4/stable/development#argument-and-result-types)を参照してください。これには、パラメータ化された型用の`CustomArgWith<P>`および`CustomResultWith<P>`が含まれます。
...
<Note>
関数を実装する前に、[C++ APIリファレンス](/ja/mysql-8.4/stable/extension-api-reference)を確認して、完全なVDF契約を確認してください。これには、nullチェック、結果の型、バッファのサイズ設定、およびエラー処理が含まれます。
</Note>
...
完全な実装のウォークスルーについては、[C++でのカスタム型](/ja/mysql-8.4/stable/custom-types)を参照してください。型にパラメータがある場合（例：`VECTOR(1536)`）、[パラメータ化された型](/ja/mysql-8.4/stable/type-operations#parameterized-types)を参照してください。
...
<CardGroup cols={2}>
  ...
  <Card
    title="開発ガイド"
    icon="code"
    href="/ja/mysql-8.4/stable/development"
  >
  ...
  <Card
    title="ソースからのビルド"
    icon="hammer"
    href="/ja/mysql-8.4/stable/source"
  >
