...
- **Git** - リポジトリのクローン用
- **CMake** 3.14.6以降 - ビルドシステムジェネレーター（macOSでは3.19以降が必要）
- **C++コンパイラー** - GCC 10以降、またはClang 14以降
...
**Ubuntu/Debian:**
```bash
sudo apt install cmake libssl-dev libncurses5-dev libaio-dev pkg-config \
                 bison libtirpc-dev build-essential zlib1g-dev
```
...
次に、依存関係をインストールします。
```bash
brew install cmake openssl@3 pkgconf bison
```
