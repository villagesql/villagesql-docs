...
<Warning>
  Rust SDK はアルファ版です。リリース間で破壊的な API 変更が発生する可能性があります。 関数のみの拡張機能とカスタム型 (encode、decode、compare、hash) がサポートされています。 集約、`prerun()`、`VarArgs`、システム変数とステータス変数、キーリングアクセス、および列ストレージ ABI は、現在 C++ のみです。 これらのいずれかが必要な場合は、[C++ SDK](/ja/mysql-8.4/stable/create) を使用してください。
</Warning>
...
`func!` は、SQL で呼び出すことができる関数を宣言します。 4 つの形式 (パラメータなし、`buffer_size` のみ、`deterministic` のみ、両方):
...
| フィールド | 必須 | 形式 | 説明 |
|---|---|---|---|
| `name` | はい | 小文字の文字、数字、`_`、`-` | 拡張機能の識別子。 `INSTALL EXTENSION` 名と一致する必要があります。 アンダースコアを使用してください。ハイフンは SQL でバッククォートで囲む必要があります。 |
