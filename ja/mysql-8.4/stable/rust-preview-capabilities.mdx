...
プレビュー機能は、API が最終化される前に拡張機能に公開されるサーバー機能です。Rust SDK はそのうちの 5 つをラップしています：ステータス変数、システム変数、バックグラウンドスレッドワーカー、ワーカースレッドからの SQL クエリ、キーリングアクセスです。このページでは、それぞれを Rust から宣言して使用する方法を説明します。概念、プレビュー層、および C++ にのみ存在する機能については、[プレビュー機能](/ja/mysql-8.4/stable/preview-capabilities)を参照してください。
...
残りのプレビュー機能 — `auth`、`mysql_services`、`statement_event`、および列ストレージ ABI — は現時点では C++ のみです。これらのいずれかが必要な場合は、[C++ SDK](/ja/mysql-8.4/stable/preview-capabilities) を使用してください。
