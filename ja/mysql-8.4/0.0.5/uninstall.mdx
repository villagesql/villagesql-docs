...
<Note>
保留中のバージョン変更（キューに入れられた`ALTER EXTENSION name VERSION 'x' AT RESTART`）がある拡張機能をアンインストールすると、その保留中のアクションは拡張機能の行とともに破棄されます。クリーンアップすべき孤立した保留状態は残らず、エラーも発生しません。その後の`INSTALL EXTENSION`は、保留中のアクションなしで新規に開始されます。保留中のバージョン変更がどのように追跡され、クリアされるかについては、[拡張機能の管理](/ja/mysql-8.4/0.0.5/managing)を参照してください。
</Note>
