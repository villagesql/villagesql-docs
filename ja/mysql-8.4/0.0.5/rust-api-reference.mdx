...
<Warning>
  Rust SDK はアルファ版です。リリース間で破壊的な API 変更が発生する可能性があります。 関数のみの拡張機能とカスタム型 (encode、decode、compare、hash) がサポートされています。 集約、`prerun()`、`VarArgs`、システム変数とステータス変数、キーリングアクセス、および列ストレージ ABI は、現在 C++ のみです。 これらのいずれかが必要な場合は、[C++ SDK](/ja/mysql-8.4/0.0.5/create) を使用してください。
</Warning>

このページは、`villagesql` クレート API のリファレンスです。 入門チュートリアルについては、[Rust で拡張機能を作成する](/ja/mysql-8.4/0.0.5/rust-sdk) を参照してください。 カスタム型については、[Rust のカスタム型](/ja/mysql-8.4/0.0.5/rust-custom-types) を参照してください。
