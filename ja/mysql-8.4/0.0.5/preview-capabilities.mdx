...
sys_var 機能（`vsql::sys_var`）は、拡張機能所有のストレージに紐付けられた MySQL システム変数を登録できるようにします。3つの型がサポートされます：`BOOL`（`bool *`）、`INT`（`long long *`）、`STR`（`char **`）。`INT` 記述子には `min_val` および `max_val` の境界が含まれます。すべての記述子にはデフォルト値とコメントが含まれます。
