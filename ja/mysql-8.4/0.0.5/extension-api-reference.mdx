...
このページは、C++ 拡張機能の作成者向けのリファレンスです。ステップバイステップのチュートリアルについては、[C++ での拡張機能の作成](/ja/mysql-8.4/0.0.5/create) を参照してください。カスタム列タイプについては、[C++ でのカスタムタイプ](/ja/mysql-8.4/0.0.5/custom-types) を参照してください。
...
`out.error(msg)` は `std::string_view` を受け入れます。メッセージをサーバーが管理するバッファーにコピーし、結果の状態を 1 回の呼び出しでエラーに設定します。
...
一部の VEF 機能は、C++ SDK インクルードツリーの `villagesql/preview/` の下にあるオプトインヘッダーとして利用できます。ABI と API はまだ積極的に開発されており、予告なしに変更される可能性があります。
