...
### 拡張機能の読み込みエラー {#extension-load-errors}

**Error:** `Failed to load VEF extension 'extension_name'`

**原因:** `.veb` ファイルがエントリーポイントの段階で読み込みに失敗しました。たとえば、
`vef_register` シンボルが存在しないか無効である、または `.veb` ファイルが破損している場合です。
...
両方の名前は一致している必要があり、アンダースコアを使用します（[拡張機能の命名規則](/ja/mysql-9.7/stable/install#extension-naming-conventions) を参照）:
...
<Note>
  `ALTER EXTENSION` には `EXTENSION_ADMIN` 権限（またはフォールバックとして
  `SUPER`）が必要です。これは `INSTALL EXTENSION` および `UNINSTALL EXTENSION`
  と同じゲートです。[必要な権限](/ja/mysql-9.7/stable/install#required-privilege)
  を参照してください。
</Note>
...
2 つ目の事前チェックは、テーブルの列でまだ使用されているカスタム型を削除する
ことになるバージョンを拒否します：
...
ROW 形式のバイナリログは、カスタム型に推奨される設定です。カスタム型カラムに対する
...
<CardGroup cols={2}>
  ...
  <Card
    title="拡張機能のアンインストール"
    icon="trash"
    href="/ja/mysql-9.7/stable/uninstall"
  >
    拡張機能を安全に削除
  </Card>
  <Card
