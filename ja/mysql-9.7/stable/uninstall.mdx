...
<Note>
`UNINSTALL EXTENSION`には`EXTENSION_ADMIN`権限（または代替として`SUPER`権限）が必要です。[必要な権限](/ja/mysql-9.7/stable/install#required-privilege)を参照してください。
</Note>
...
<Note>
保留中のバージョン変更（キューに入れられた`ALTER EXTENSION name VERSION 'x' AT RESTART`）がある拡張機能をアンインストールすると、その保留中のアクションは拡張機能の行とともに破棄されます。クリーンアップすべき孤立した保留状態は残らず、エラーも発生しません。その後の`INSTALL EXTENSION`は、保留中のアクションなしで新規に開始されます。保留中のバージョン変更がどのように追跡され、クリアされるかについては、[拡張機能の管理](/ja/mysql-9.7/stable/managing)を参照してください。
</Note>
...
<CardGroup cols={2}>
  ...
  <Card
    title="拡張機能の管理"
    icon="sliders"
    href="/ja/mysql-9.7/stable/managing"
  >
  ...
  <Card
    title="システムリファレンス"
    icon="book"
    href="/ja/mysql-9.7/stable/reference"
  >
    システムテーブルをクエリします
  </Card>
</CardGroup>
