...
title: "C++ 開発"
description: "C++ 拡張機能向けの VDF 作成の詳細 — 引数と結果の型、例外安全性、集計、prerun/postrun、可変長引数、および登録。"
---

このガイドは、C++ VDF 実装を作成するための詳細なリファレンスです。エンドツーエンドのビルド手順をカバーする [Creating Extensions in C++](/ja/mysql-9.7/stable/create) と、テストと反復のループをカバーする [C++ Testing](/ja/mysql-9.7/stable/testing) の補完資料です。
...
## 例外安全性 {#exception-safety}

例外の封じ込めは拡張機能の責任です。SDK は実装を `try`/`catch` でラップせず、サーバーもラップしません。そのため、エントリポイントから抜け出した例外は、上位のどこにもハンドラーがないまま C ABI 境界を越えてスタックを巻き戻し、サーバープロセスが終了します。通常の SQL 入力でもこれに到達します: `std::stoi()` は数値以外の文字列でスローし、`nlohmann::json::dump()` は有効な UTF-8 ではないテキストでスローします。

登録するすべてのエントリポイントの本体をラップしてください: VDF、集計結果関数、およびカスタム型の `from_string`/`to_string` 演算です。
...
`const std::exception &` に加えて `...` もキャッチしてください: 依存ライブラリが `std::exception` を継承しない型をスローする場合があり、それもサーバーを終了させます。`warning()` と `error()` の選択は、他の失敗の場合と同じ基準で行ってください — ステートメントが継続すべき不正な入力には `warning()`、続行が安全でない場合には `error()` です。
...
**引数型:** `IntArg`, `RealArg`, `StringArg`, `CustomArg` — 各型は `is_null()` と `value()` を提供します。パラメータ化されたカスタム型の場合、`CustomArgWith<P>` はキャッシュされた解析済みの params 構造体を返す `params()` アクセサを追加します（[Parameterized Types](/ja/mysql-9.7/stable/type-operations#parameterized-types) を参照）。
...
`StringResult` の集計 VDF はテキストを返します。結果は `utf8mb4_bin` の文字セットと照合順序を報告するため、クライアントは 16 進数ではなく文字として表示します — スカラー VDF の STRING パスと同じです。また、`.max_result_length(n)`（オプトイン形式の dev ABI ヘッダー（`-DVSQL_USE_DEV_ABI=ON`）と Protocol 4 が必要）も同様に適用し、マテリアライズされた集計結果（`GROUP BY`/`DISTINCT` の一時テーブル、`CREATE TABLE ... SELECT`、または UNION）のサイズを設定して引数幅で切り捨てられないようにします。サイズ設定のルールと上限については [Custom Buffer Sizes](/ja/mysql-9.7/stable/create#custom-buffer-sizes) を参照してください。
...
`PrerunArgType::custom_type()` は、引数のカスタム型名を `std::string_view` として返します。カスタム型でない引数に対しては空になります。`is_custom()` だけでは、サーバーに登録されているすべてのカスタム型を受け入れます。`custom_type()` の比較と組み合わせることで、可変長引数の呼び出しを特定の 1 つの型に絞り込めます。
...
## 拡張機能のロードフックとアンロードフック {#extension-load-and-unload-hooks}

拡張機能には、呼び出しごとではなくロードごとに 1 回だけ実行する作業が必要になることがあります — CPU 固有の実装の選択や、VDF が後で読み取るルックアップテーブルの構築などです。その作業は拡張機能ビルダーの `.on_init<&Fn>()` で登録し、対応する後処理は `.on_deinit<&Fn>()` で登録します。どちらもシグネチャが `void()` の関数を取ります。

<Warning>
`.on_init<&Fn>()` と `.on_deinit<&Fn>()` は、オプトイン形式の dev ABI ヘッダー（`-DVSQL_USE_DEV_ABI=ON`）でのみ宣言されています。デフォルトの安定版 ABI ビルドにはどちらのメソッドもないため、フックを登録するとコンパイルに失敗します。
</Warning>

`on_init` は拡張機能のロードのたびに実行されます: `INSTALL EXTENSION` 時と、サーバー起動のたびに、サーバーが拡張機能を検証して受け入れた後に実行されます。サーバーが拒否した拡張機能に対しては決して実行されません。`on_deinit` はアンロード時 — `UNINSTALL EXTENSION` またはサーバーシャットダウン時 — に実行されます。

どちらのフックも拡張機能プロセス内で **サーバーへのアクセスなし** で実行されます: SQL の実行、システム変数の読み取り、サーバー状態への到達はできません。そのため、サーバーとやり取りする必要があるセットアップには適していません — その場合はプレビュー機能の populate ステップを使用してください。
...
`INSTALL EXTENSION my_extension` の後、`SELECT fast_sqrt(16)` は `4` を返します — これは `build_table` がすでに実行されている場合にのみ得られる結果です。

<Note>
`on_deinit` は `VEF_GENERATE_ENTRY_POINTS` が生成する `vef_unregister` エントリポイントから呼び出されます。`VEF_GENERATE_REGISTRATION` はそのエントリポイントを定義しないため、それを使用する拡張機能は、自身が記述する `vef_unregister` から後処理を呼び出す必要があります。
</Note>
...
型演算ビルダーの完全なリファレンス — エンコード、デコード、比較、ハッシュ、組み込みデフォルト、およびパラメータ化された型 — については、[Type Operations](/ja/mysql-9.7/stable/type-operations) を参照してください。

## プレビュー機能

以下の VEF 機能は、オプトインのプレビューヘッダーとして利用可能です。ABI と API はまだ活発に開発中です。完全なリファレンスについては、[Preview Capabilities](/ja/mysql-9.7/stable/preview-capabilities) を参照してください。

- **拡張機能システム変数** — [Preview Capabilities → System Variables](/ja/mysql-9.7/stable/preview-capabilities#system-variables)
- **拡張機能ステータス変数** — [Preview Capabilities → Status Variables](/ja/mysql-9.7/stable/preview-capabilities#status-variables)
- **キーリングアクセス** — [Preview Capabilities → Keyring Access](/ja/mysql-9.7/stable/preview-capabilities#keyring-access)
- **列ストレージ** — [Preview Capabilities → Column Storage](/ja/mysql-9.7/stable/preview-capabilities#column-storage)
...
| `REGISTRATION_JSON` | `VARCHAR(65535)` | `funcs` および `types` 配列を含む `vef_registration_t` 構造体の JSON シリアライゼーション。 |

## 関連資料

- [Creating Extensions in C++](/ja/mysql-9.7/stable/create) — エンドツーエンドのビルド手順、CMake セットアップ、およびインストール
- [C++ Testing](/ja/mysql-9.7/stable/testing) — ローカル開発サーバー、MTR、および失敗のデバッグ
- [Type Operations](/ja/mysql-9.7/stable/type-operations) — エンコード、デコード、比較、ハッシュ、パラメータ化された型
- [C++ API Reference](/ja/mysql-9.7/stable/extension-api-reference) — VDF 契約、null 処理、およびバッファサイズ設定
- [Extension Architecture](/ja/mysql-9.7/stable/architecture) — ライフサイクル、Victionary キャッシュ、パフォーマンスパターン、およびセキュリティモデル
