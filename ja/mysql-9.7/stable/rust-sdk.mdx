...
<Note>
C++ を使用する場合は、[C++ で拡張機能を作成する](/ja/mysql-9.7/stable/create) を参照して、C++ SDK の手順を確認してください。
</Note>
...
始める前に、VillageSQL をソースからビルドしてください。拡張機能はサーバーのビルドツリーに対してリンクします。まず [ソースからビルドする](/ja/mysql-9.7/stable/source) ガイドに従ってください。
...
新しい Rust ライブラリクレートを作成し、SDK を追加します。
...
`Cargo.toml` を編集して、クレートのタイプを設定します。
...
<CardGroup cols={2}>
  <Card
    title="Rust でのカスタム型"
    icon="shapes"
    href="/ja/mysql-9.7/stable/rust-custom-types"
  >
  ...
  <Card
    ...
    icon="flask"
    href="/ja/mysql-9.7/stable/rust-preview-capabilities"
  >
    ステータス変数、システム変数、バックグラウンドスレッドワーカー、および requires: リストによるキーリングアクセス。
  </Card>
  <Card
    title="C++ SDK（拡張機能の作成）"
    icon="code"
  ...
  <Card
    ...
    icon="network-wired"
    href="/ja/mysql-9.7/stable/testing-network"
  >
