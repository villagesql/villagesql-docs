...
プレビュー機能は、API が最終化される前に拡張機能に公開されるサーバー機能です。Rust SDK はそのうちの 4 つをラップしています：ステータス変数、システム変数、バックグラウンドスレッドワーカー、キーリングアクセスです。このページでは、それぞれを Rust から宣言して使用する方法を説明します。概念、プレビュー層、および C++ にのみ存在する機能については、[プレビュー機能](/ja/mysql-9.7/stable/preview-capabilities)を参照してください。
...
残りのプレビュー機能 — `auth`、`mysql_services`、`sql_query`、`statement_event`、および列ストレージ ABI — は現時点では C++ のみです。これらのいずれかが必要な場合は、[C++ SDK](/ja/mysql-9.7/stable/preview-capabilities) を使用してください。
