...
title: "C++でのカスタム型"
description: "C++でVillageSQL拡張の新しいカラムタイプを定義します。タイプ操作、ALTER TABLEルール、変換関数、および完全なCOMPLEXタイプの例。"
---
...
カスタムタイプを使用すると、`ORDER BY`、インデックス、および集計関数で機能する`COMPLEX`、`UUID`、または`VECTOR`のような新しいカラムタイプを定義できます。このページは、[C++での拡張機能の作成](/ja/mysql-9.7/stable/create)チュートリアルのステップ4です。ここで続行する前に、ステップ1〜3を完了してください。
...
完全なC++シグネチャについては、[タイプ操作](/ja/mysql-9.7/stable/type-operations)を参照してください。
...
デコードには`snprintf`ではなく`std::to_chars`を使用します。`"%.6f"`のような固定精度の書式では`double`に必要な桁が失われ、`0.1234567891`が`0.123457`として読み戻されるためです。エンコードでは`strtod`を呼び出す前に入力をコピーします。`std::string_view`は終端のヌル文字を持たないためです。`std::from_chars`の浮動小数点オーバーロードを使えばこのコピーを回避できますが、libc++ではmacOS 26以降でのみ利用可能とマークされているため、これらを使用する拡張機能はそれより前のターゲット向けにビルドできません。
...
完全な実装については、[vsql_complexの例](/ja/mysql-9.7/stable/examples)を参照してください。
...
オプティマイザは、同じVDF式が`WHERE`、`ORDER BY`、または`GROUP BY`に含まれる場合に、インデックスを使用します。オプティマイザが式を一致させられるように、比較値をVDFの戻り値のタイプにキャストします。
...
<CardGroup cols={2}>
  <Card
    ...
    icon="hammer"
    href="/ja/mysql-9.7/stable/create#step-5-update-build-configuration"
  >
  ...
  <Card
    title="パラメータ化された型"
    icon="sliders"
    href="/ja/mysql-9.7/stable/type-operations#parameterized-types"
  >
  ...
  <Card
    title="拡張機能APIリファレンス"
    icon="book"
    href="/ja/mysql-9.7/stable/extension-api-reference"
  >
  ...
  <Card
    ...
    href="/ja/mysql-9.7/stable/managing#replication"
  >
