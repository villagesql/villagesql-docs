...
title: "C++での拡張機能の作成"
description: "拡張テンプレートとVEFを使用して、C++でカスタムのVillageSQL拡張機能を作成する方法を学びます。"
---
...
VDFの実装の詳細（引数と結果の型、集約、システム変数、パラメータ化された型）については、[開発ガイド](/ja/mysql-9.7/stable/development)を参照してください。

<Note>
Rustを使用する場合は、[Rustでの拡張機能の作成](/ja/mysql-9.7/stable/rust-sdk)を参照してください。
</Note>
...
開始する前に、VillageSQL Extension SDKが必要です。CMakeにSDKを指定する方法は2つあります。VillageSQLのビルドツリーがある場合は、`-DVillageSQL_BUILD_DIR=/path/to/villagesql/build`を渡します。[ソースからのビルド](/ja/mysql-9.7/stable/source)ガイドに従うと、このツリーが生成されます。展開済みのSDKディレクトリ（`include/villagesql/vsql.h`を含むもの）がある場合は、それだけで十分です。`-DVillageSQL_SDK_DIR=/path/to/sdk`を渡してください。サーバーをビルドせずにSDKを入手する方法については、[拡張機能テンプレートのREADME](https://github.com/villagesql/vsql-extension-template)を参照してください。VEBのインストール先ディレクトリを設定するのはビルドツリー形式だけなので、SDKのみのパスでは、ビルドしたVEBファイルを`SHOW VARIABLES LIKE 'veb_dir'`が示すディレクトリに自分でコピーしてください。

また、次のものも必要です。

- **Git** - クローニングとバージョン管理用
- **CMake** 3.18以降 - ビルドシステム
- **C++コンパイラ** - C++17のサポートが必要です。テンプレートは`CMAKE_CXX_STANDARD 17`を設定しており、SDKのCMakeパッケージはこれより低い規格を拒否します。
...
SQL、ファイル名、リポジトリ名全体での完全な命名規則については、[拡張機能の命名規則](/ja/mysql-9.7/stable/install#extension-naming-conventions)を参照してください。
...
完全な`CustomArg`/`CustomResult`APIについては、[開発ガイド](/ja/mysql-9.7/stable/development#argument-and-result-types)を参照してください。これには、パラメータ化された型用の`CustomArgWith<P>`および`CustomResultWith<P>`が含まれます。
...
### カスタムバッファサイズ {#custom-buffer-sizes}
...
`buffer()`と`set_length()`を使用して値を自分で構築する場合、`buffer().size()`を超えて書き込まないでください。メモリオーバーフローが発生します。ただし、大きすぎる値でも失敗させないでください。書き込もうとしたサイズを報告して戻り、サーバーにバッファを拡張させて再度呼び出させてください。
...
`STRING`結果の場合、`out.set(sv)`はsnprintfスタイルのオーバーフロー契約に従います。`out.buffer()`に収まる分だけバイトをコピーしますが、値の*完全な*サイズを報告します。サーバーは報告された長さがバッファを超えていることを検出し、バッファを拡張して関数を 2 回目に呼び出します。そのため、`.buffer_size()`より大きい値は切り捨てられずに全体が返されます。報告された長さがバッファに必要な大きさを正確に示すため、再試行は常に 1 回で十分です。

拡張は`max_allowed_packet`によって制限されます。それより大きい値は切り捨てられるのではなく、`NULL`として返されます。

`out.buffer()`に自分で書き込んで`out.set_length()`を呼び出す場合も、同じ契約に従います。収まったバイト数ではなく、書き込もうとした完全なサイズを報告してください。切り捨てられた長さを報告すると再試行が行われず、残りが黙って失われます。
...
<Note>
関数を実装する前に、[C++ APIリファレンス](/ja/mysql-9.7/stable/extension-api-reference)を確認して、完全なVDF契約を確認してください。これには、nullチェック、結果の型、バッファのサイズ設定、およびエラー処理が含まれます。
</Note>

## ステップ4：カスタム型を作成する

カスタム型を使用すると、`ORDER BY`、インデックス、および集約関数で機能する`COMPLEX`、`UUID`、または`VECTOR`などの新しい列タイプを定義できます。作成する拡張機能が関数のみを登録する場合は、ステップ5に進みます。

完全な実装のウォークスルーについては、[C++でのカスタム型](/ja/mysql-9.7/stable/custom-types)を参照してください。型にパラメータがある場合（例：`VECTOR(1536)`）、[パラメータ化された型](/ja/mysql-9.7/stable/type-operations#parameterized-types)を参照してください。
...
<CardGroup cols={2}>
  ...
  <Card
    title="開発ガイド"
    icon="code"
    href="/ja/mysql-9.7/stable/development"
  >
  ...
  <Card
    title="拡張機能のアーキテクチャ"
    icon="sitemap"
    href="/ja/mysql-9.7/stable/architecture"
  >
  ...
  <Card
    title="ソースからのビルド"
    icon="hammer"
    href="/ja/mysql-9.7/stable/source"
  >
...
- [MySQL UDF APIドキュメント](https://dev.mysql.com/doc/extending-mysql/9.7/en/adding-loadable-function.html)
