...
プレビュー機能を通じて **利用可能** と記されているものには、`vsql_allow_preview_extensions = ON` が必要です — [プレビュー機能](/ja/mysql-9.7/dev/preview-capabilities)を参照してください。
...
| カスタムデータ型と演算子 | 独自のストレージと比較動作を持つ新しい基本型を登録する | **利用可能** — [C++でのカスタム型](/ja/mysql-9.7/dev/custom-types)を参照 |
| 集約関数 | ユーザー定義の集約を登録する | **利用可能** — `make_aggregate_func`、[C++ 開発](/ja/mysql-9.7/dev/development)を参照 |
