...
カスタムタイプを使用すると、`ORDER BY`、インデックス、および集計関数で機能する`COMPLEX`、`UUID`、または`VECTOR`のような新しいカラムタイプを定義できます。このページは、[C++での拡張機能の作成](/ja/mysql-9.7/dev/create)チュートリアルのステップ4です。ここで続行する前に、ステップ1〜3を完了してください。
...
完全なC++シグネチャについては、[タイプ操作](/ja/mysql-9.7/dev/type-operations)を参照してください。
...
完全な実装については、[vsql_complexの例](/ja/mysql-9.7/dev/examples)を参照してください。
...
<CardGroup cols={2}>
  <Card
    ...
    icon="hammer"
    href="/ja/mysql-9.7/dev/create#step-5-update-build-configuration"
  >
  ...
  <Card
    title="パラメータ化された型"
    icon="sliders"
    href="/ja/mysql-9.7/dev/type-operations#parameterized-types"
  >
  ...
  <Card
    title="拡張機能APIリファレンス"
    icon="book"
    href="/ja/mysql-9.7/dev/extension-api-reference"
  >
  ...
  <Card
    ...
    href="/ja/mysql-9.7/dev/managing#replication"
  >
