...
<Warning>
  Rust SDK はアルファ版です。リリース間で破壊的な API 変更が発生する可能性があります。関数のみの拡張機能とカスタム型（encode、decode、compare、hash）に加えて、`sys_var`、`status_var`、`thread_worker`、`keyring` のプレビュー機能もサポートされています。集約関数、`VarArgs`、および列ストレージ ABI は現在 C++ のみで利用可能です。これらのいずれかが必要な場合は [C++ SDK](/ja/mysql-9.7/dev/create) を使用してください。
</Warning>

<Note>
C++ を使用する場合は、[C++ で拡張機能を作成する](/ja/mysql-9.7/dev/create) を参照して、C++ SDK の手順を確認してください。
</Note>
...
始める前に、VillageSQL をソースからビルドしてください。拡張機能はサーバーのビルドツリーに対してリンクします。まず [ソースからビルドする](/ja/mysql-9.7/dev/source) ガイドに従ってください。
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="flask"
    href="/ja/mysql-9.7/dev/rust-preview-capabilities"
  >
  ...
  <Card
    ...
    icon="network-wired"
    href="/ja/mysql-9.7/dev/testing-network"
  >
