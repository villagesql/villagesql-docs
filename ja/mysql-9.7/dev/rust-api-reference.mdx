...
<Warning>
  Rust SDK はアルファ版です。リリース間で破壊的な API 変更が発生する可能性があります。 関数のみの拡張機能とカスタム型 (encode、decode、compare、hash) がサポートされており、`sys_var`、`status_var`、`thread_worker`、`keyring` の各プレビュー機能もサポートされています。 集約、`VarArgs`、および列ストレージ ABI は、現在 C++ のみです。 これらのいずれかが必要な場合は、[C++ SDK](/ja/mysql-9.7/dev/create) を使用してください。
</Warning>
