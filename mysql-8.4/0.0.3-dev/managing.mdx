...
```bash
# Replace the .veb file in veb_dir, then:
INSTALL EXTENSION my_extension;
```

Run `UNINSTALL` and `INSTALL` as **separate** client invocations. Combining them in
a single `-e "UNINSTALL ...; INSTALL ...;"` call may silently drop the second
statement.

If the extension still shows old behavior, remove the expansion cache manually
before reinstalling:
...
Then run `INSTALL EXTENSION my_extension;` again.
