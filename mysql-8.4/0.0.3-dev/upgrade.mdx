...
VillageSQL Server is a drop-in replacement for MySQL. Migrating from MySQL to VillageSQL is functionally identical to a minor version upgrade (e.g., 8.4.7 to 8.4.8), preserving your existing data.

## Prerequisites

- **VillageSQL Binaries**: Ensure you have the VillageSQL binaries ready (either [compiled from source](/mysql-8.4/0.0.3-dev/source) or downloaded).
...
+--------------------------+---------------------+
| 8.4.8-villagesql-0.0.3   | Source distribution |
+--------------------------+---------------------+
```
