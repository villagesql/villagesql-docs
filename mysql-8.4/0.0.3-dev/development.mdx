---
title: "Development"
description: "Writing extension functions and running tests for your VillageSQL extension."
---

This guide covers writing VDF implementations and running regression tests for VillageSQL extensions.

<Note>
  If you are contributing to the VillageSQL server itself (not building an extension), see [Clone and Build from Source](/mysql-8.4/0.0.3-dev/source), which covers the full server developer workflow including running tests with `mysql-test-run.pl` directly.
</Note>

## Setting Up Your Environment

To run extension tests, you need a built VillageSQL server. Follow the [Clone and Build from Source](/mysql-8.4/0.0.3-dev/source) guide to compile the server binaries.

Once you have a build directory (e.g., `build/`), you are ready to run regression tests for your extension.

## Writing Extension Functions

Extension functions are written in C++ and registered with VEF. Include a
single header to access the full SDK:

```cpp
#include <villagesql/extension.h>
```

### Typed Wrappers (Recommended)

Typed wrappers provide a type-safe interface for VDF parameters and results.
The framework detects wrapper types in your function signature and adapts
automatically — the `make_func` registration syntax is unchanged.

**Input wrappers:** `IntArg`, `RealArg`, `StringArg`, `BinaryArg` — each
provides `is_null()` and `value()`.

**Result wrappers:** `IntResult`, `RealResult`, `StringResult`, `BinaryResult`
— each provides `set_null()` and `error(msg)`. Scalar results also provide
`set(value)`. Buffer results provide `buffer()` and `set_length(len)`.

**Scalar example** — add two integers:

```cpp
using namespace villagesql;

void add_impl(IntArg a, IntArg b, IntResult out) {
  if (a.is_null() || b.is_null()) { out.set_null(); return; }
  out.set(a.value() + b.value());
}

// Registration is unchanged:
make_func<&add_impl>("add").returns(INT).param(INT).param(INT).build();
```

**Binary example** — transform a custom type buffer in place:

```cpp
using namespace villagesql;

void rot13_impl(BinaryArg in, BinaryResult out) {
  if (in.is_null()) { out.set_null(); return; }
  auto src = in.value();   // villagesql::Span<const unsigned char>
  auto dst = out.buffer(); // villagesql::Span<unsigned char>
  for (size_t i = 0; i < src.size(); i++) { dst[i] = transform(src[i]); }
  out.set_length(src.size());
}
```

For `StringResult` and `BinaryResult`, write into `buffer()`, then call
`set_length()` with the number of bytes written. `buffer().size()` is the
maximum capacity.

You can use different styles across functions in the same extension — each
function's style is determined by its own signature.

### Raw ABI Style (Deprecated)

Prefer [typed wrappers](#typed-wrappers-recommended) for new code.

The raw ABI style passes pointers to the underlying C structs directly:

```cpp
void add_impl(vef_context_t* ctx,
              vef_invalue_t* a, vef_invalue_t* b,
              vef_vdf_result_t* result) {
  result->int_value = a->int_value + b->int_value;
  result->type = VEF_RESULT_VALUE;
}
```

Registration is the same as with typed wrappers.

### Type Operation Builders

Beyond standard VDFs, custom types require four type-level operations:
encode (string → binary), decode (binary → string), compare, and hash.
Implement them against these ergonomic C++ signatures (all defined in
`func_builder.h`):

```cpp
// Encode (fixed-size types): string -> binary. false = success, true = error.
// Set *length = SIZE_MAX to produce SQL NULL.
using TypeEncodeFunc = bool (*)(std::string_view from,
                                villagesql::Span<unsigned char> buf,
                                size_t *length);

// Encode (variable-size types): TypeEncodeWithParamsFunc — same signature
// but adds the column's declared type parameters as first argument.
// See func_builder.h for the full typedef. make_type_encode accepts both.

// Decode (fixed-size types): binary -> string. false = success, true = error.
using TypeDecodeFunc = bool (*)(villagesql::Span<const unsigned char> data,
                                villagesql::Span<char> out,
                                size_t *out_len);

// Decode (variable-size types): TypeDecodeWithParamsFunc — same signature
// but adds the column's declared type parameters as first argument.
// See func_builder.h for the full typedef. make_type_decode accepts both.

// Compare: returns -1, 0, or 1 (used for ORDER BY and indexes).
using TypeCompareFunc = int (*)(villagesql::Span<const unsigned char> a,
                                villagesql::Span<const unsigned char> b);

// Compare (variable-size types): TypeCompareWithParamsFunc — same signature
// but adds the column's declared type parameters as first argument.
// See func_builder.h for the full typedef. make_type_compare accepts both.

// Hash: returns hash code (used for hash joins).
using TypeHashFunc = size_t (*)(villagesql::Span<const unsigned char> data);

// Hash (variable-size types): TypeHashWithParamsFunc — same signature
// but adds the column's declared type parameters as first argument.
// See func_builder.h for the full typedef. make_type_hash accepts both.
```

Register each operation with `make_type_encode`, `make_type_decode`,
`make_type_compare`, or `make_type_hash`, passing the function pointer and
the type name constant. The type descriptor references them by name:

```cpp
constexpr const char *MYTYPE = "mytype";

VEF_GENERATE_ENTRY_POINTS(
    make_extension("my_ext", "1.0.0")
        .type(make_type(MYTYPE)
                  .persisted_length(8)
                  .max_decode_buffer_length(64)
                  .encode("mytype_encode")    // Name of the encode VDF
                  .decode("mytype_decode")    // Name of the decode VDF
                  .compare("mytype_compare")  // Name of the compare VDF
                  .hash("mytype_hash")        // Optional: name of the hash VDF
                  .build())
        .func(make_type_encode<&my_encode>("mytype_encode", MYTYPE))
        .func(make_type_decode<&my_decode>("mytype_decode", MYTYPE))
        .func(make_type_compare<&my_compare>("mytype_compare", MYTYPE))
        .func(make_type_hash<&my_hash>("mytype_hash", MYTYPE)))
```

Fixed-size types (like `complex`) use `TypeEncodeFunc`. Variable-size types
that need the column's declared type parameters at encode time use
`TypeEncodeWithParamsFunc` — it adds a string key-value params map as the
first argument. `make_type_encode` detects which form you're using
automatically based on your function's signature:

```cpp
// TypeEncodeWithParamsFunc example: params contains TYPE('k=v,...') pairs.
bool mytype_encode(const std::map<std::string, std::string> &params,
                   std::string_view from,
                   villagesql::Span<unsigned char> buf, size_t *length) {
  int64_t dim = strtoll(params.at("dimension").c_str(), nullptr, 10);
  size_t bytes = (size_t)dim * 4;
  if (buf.size() < bytes) return true;
  // ... parse from, write to buf ...
  *length = bytes;
  return false;  // success
}

// Registration is the same for both forms:
make_type_encode<&mytype_encode>("mytype_encode", MYTYPE)
```

The full `TypeEncodeWithParamsFunc` typedef is in `func_builder.h`.

Decode follows the same pattern — `TypeDecodeWithParamsFunc` for variable-size
types, `TypeDecodeFunc` for fixed-size:

```cpp
bool mytype_decode(const std::map<std::string, std::string> &params,
                   villagesql::Span<const unsigned char> data,
                   villagesql::Span<char> out, size_t *out_len) {
  auto it = params.find("dimension");
  if (it == params.end()) return true;  // error
  int64_t dimension = strtoll(it->second.c_str(), nullptr, 10);
  // ... read dimension floats from data, format into out ...
  *out_len = bytes_written;
  return false;  // success
}

// Registration is the same for both forms:
make_type_decode<&mytype_decode>("mytype_decode", MYTYPE)
```

The full `TypeDecodeWithParamsFunc` typedef is in `func_builder.h`.

Compare and hash follow the same pattern:

```cpp
// TypeCompareWithParamsFunc — variable-size types:
int mytype_compare(const std::map<std::string, std::string> &params,
                   villagesql::Span<const unsigned char> a,
                   villagesql::Span<const unsigned char> b) {
  // params["dimension"] etc. available if needed
  // Returns -1, 0, or 1.
}

// TypeHashWithParamsFunc — variable-size types:
size_t mytype_hash(const std::map<std::string, std::string> &params,
                   villagesql::Span<const unsigned char> data) {
  // params["dimension"] etc. available if needed
}

// Registration is the same for both forms:
make_type_compare<&mytype_compare>("mytype_compare", MYTYPE)
make_type_hash<&mytype_hash>("mytype_hash", MYTYPE)
```

The full `TypeCompareWithParamsFunc` and `TypeHashWithParamsFunc` typedefs are
in `func_builder.h`.

These four operations are everything most custom types need. For types that
require direct control over how column data is laid out in InnoDB pages —
such as a dense vector type that stores packed floats outside the main row —
VEF provides an optional storage interface.

### Column Storage Interface

Custom types store their column data in InnoDB's row format by default. If a
type needs to manage its own column-level storage (for example, to store
vector data in a separate page structure for more efficient storage and indexing), register a column storage
interface with `make_storage()` and attach it to the type via `column_storage()`.

The storage interface requires the following seven callbacks:

`create`, `drop`, `load`, `insert`, `select`, `mark_delete`, and `purge`.

All callbacks must be provided — omitting any results in a compile-time error.
Each callback returns `false` on success and `true` on error. On failure, the
implementation should populate the `error_msg` buffer with an appropriate
message.

```cpp
#include <villagesql/extension.h>

// Implement the 7 storage callbacks (see abi/storage.h for full signatures).
bool my_create(vef_storage_space_ref_t space_ref,
               vef_storage_trx_ref_t trx_ref, uint32_t col_len,
               vef_storage_arena_t *arena_ctx,
               vef_storage_arena_func_t arena_alloc,
               vef_storage_ctx_t **storage, char *error_msg,
               uint32_t error_msg_len) {
  // Allocate storage context using the arena allocator, set *storage.
  return false;  // success
}

bool my_drop(vef_storage_ctx_t *storage, vef_storage_trx_ref_t trx_ref,
             char *error_msg, uint32_t error_msg_len) {
  return false;
}

bool my_load(vef_storage_ref_t storage_ref, vef_storage_arena_t *arena_ctx,
             vef_storage_arena_func_t arena_alloc,
             vef_storage_ctx_t **storage, char *error_msg,
             uint32_t error_msg_len) {
  return false;
}

bool my_insert(vef_storage_ctx_t *storage, vef_storage_mtr_ref_t mctx,
               vef_storage_trx_ref_t trx_ref, vef_storage_col_data_t col_data,
               vef_storage_col_data_t rowid_prefix,
               vef_storage_col_ref_t *col_ref, char *error_msg,
               uint32_t error_msg_len) {
  return false;
}

bool my_select(vef_storage_ctx_t *storage, vef_storage_mtr_ref_t mctx,
               vef_storage_col_ref_t col_ref,
               vef_storage_col_data_t *col_data,
               vef_storage_col_data_t *rowid_prefix,
               vef_storage_trx_ref_t *trx_ref, bool *delete_marked,
               char *error_msg, uint32_t error_msg_len) {
  return false;
}

bool my_mark_delete(vef_storage_ctx_t *storage, vef_storage_mtr_ref_t mctx,
                    vef_storage_trx_ref_t trx_ref,
                    vef_storage_col_ref_t col_ref, bool delete_mark,
                    char *error_msg, uint32_t error_msg_len) {
  return false;
}

bool my_purge(vef_storage_ctx_t *storage, vef_storage_mtr_ref_t mctx,
              vef_storage_trx_ref_t trx_ref, vef_storage_col_ref_t col_ref,
              char *error_msg, uint32_t error_msg_len) {
  return false;
}

using namespace villagesql;

constexpr const char *MYTYPE = "mytype";

VEF_GENERATE_ENTRY_POINTS(
    make_extension("my_ext", "1.0.0")
        .type(make_type(MYTYPE)
                  .persisted_length(8)
                  .max_decode_buffer_length(64)
                  .encode("mytype_encode")
                  .decode("mytype_decode")
                  .compare("mytype_compare")
                  .column_storage(make_storage()
                                      .create(&my_create)
                                      .drop(&my_drop)
                                      .load(&my_load)
                                      .insert(&my_insert)
                                      .select(&my_select)
                                      .mark_delete(&my_mark_delete)
                                      .purge(&my_purge)
                                      .build())
                  .build())
        .func(make_type_encode<&my_encode>("mytype_encode", MYTYPE))
        .func(make_type_decode<&my_decode>("mytype_decode", MYTYPE))
        .func(make_type_compare<&my_compare>("mytype_compare", MYTYPE)))
```

`make_storage()` returns a `StorageBuilder`. Chain the 7 callbacks, then call
`.build()` to produce a `vef_type_storage_intf_t`. The builder sets the
`version` field automatically. Pass the result to `column_storage()` on the
`TypeBuilder` before calling `.build()`.

Types without a `column_storage()` call use InnoDB's default row-level
storage, which is sufficient for most fixed-size types.

## Running Regression Tests

VillageSQL uses the `./villagesql mysql-test` wrapper for regression testing. This command passes arguments through to the MySQL Test Framework. Run all commands below from your VillageSQL dev server directory.

### Running the Full Suite

To run all tests for your extension:

```bash
./villagesql mysql-test --suite=/path/to/your/extensions/tests --parallel=auto
```

### Running Individual Tests

To run a single test case, specify the suite path and test name:

```bash
# Run a specific insert test
./villagesql mysql-test villagesql/insert.insert_values_complex
```

## Creating New Tests

When adding new features or fixing bugs, you should add corresponding regression tests.

### Test Location

Tests are located in `mysql-test/suite/villagesql/`. Choose the appropriate sub-directory based on the SQL statement or feature being tested.
- Test files end with `.test` and go in the `t/` directory.
- Expected result files end with `.result` and go in the `r/` directory.

### Test File Conventions

VillageSQL tests often involve the custom `COMPLEX` type. We follow a specific pattern to ensure tests are skipped gracefully if the feature is not yet fully implemented:

```sql
# Description of the test
# Grammar: (Optional) SQL syntax reference

--source include/villagesql/not_implemented_complex_type.inc

--source include/villagesql/install_complex_extension.inc

# ... Your Test Code Here ...
CREATE TABLE t1 (val COMPLEX);
INSERT INTO t1 VALUES ('(1.0,2.0)');
SELECT * FROM t1;
DROP TABLE t1;

--source include/villagesql/uninstall_complex_extension.inc
```

### Steps to Add a Test

1.  **Create the `.test` file** in the appropriate `t/` directory (e.g., `mysql-test/suite/villagesql/select/t/my_new_test.test`).
2.  **Create an empty `.result` file** in the corresponding `r/` directory (e.g., `mysql-test/suite/villagesql/select/r/my_new_test.result`).
3.  **Run the test with `--record`** to generate the expected output:
    ```bash
    ./villagesql mysql-test --suite=villagesql/select --record my_new_test
    ```
4.  **Verify the output** in the generated `.result` file to ensure it matches your expectations.

## Debugging Tests

If a test fails, the test framework provides detailed logs.

-   **Test output:** Check `mysql-test/var/log/my_new_test.log`.
-   **Server error log:** Check `mysql-test/var/log/mysqld.1.err`.
-   **Diff:** The framework outputs a diff between the actual output and the expected `.result` file.

To run a test with extra debug information:

```bash
./villagesql mysql-test --verbose villagesql/select.my_new_test
```
