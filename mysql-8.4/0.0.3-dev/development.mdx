...
```cpp
using vef_intrinsic_default_func_t = bool (*)(
    int64_t buffer_size,   // resolved persisted_length in bytes
    unsigned char *buffer,
    size_t *length,
    char *error_msg);  // false = success, true = error
```

`buffer_size` is the resolved `persisted_length`, so variable-size types
(e.g., `TVECTOR(N)`) produce the correct number of bytes.

Register with `make_intrinsic_default`, bind via
`TypeBuilder::intrinsic_default()`.

**Complete example** — COMPLEX defaults to `(0,0)`:

```cpp
constexpr int64_t kComplexSize = 16;  // two doubles

bool complex_default(int64_t buffer_size, unsigned char *buffer,
