...
variable-length data in a separate page structure), register a column storage
interface with `make_storage()` and attach it to the type via `column_storage()`.

The storage interface requires 7 callbacks: `create`, `drop`, `load`, `insert`,
`select`, `mark_delete`, and `purge`. All 7 must be provided — omitting any
causes a compile-time error. Each callback returns `false` on success and `true`
on error (writing to the `error_msg` buffer on failure).
