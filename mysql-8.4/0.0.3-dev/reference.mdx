...
`EXTENSION_NAME` values in the illustrative output above are lowercase,
matching the name passed to `make_extension()` during extension registration.

The view reflects the state after the most recently completed
`INSTALL EXTENSION` or `UNINSTALL EXTENSION` statement.
