...
VillageSQL exposes server metadata through read-only global system variables, queryable with standard MySQL syntax.

### villagesql_server_version

A read-only global system variable that exposes the build-time VillageSQL version string (for example, `1.0.0` for a release build or `1.0.0-dev` for a development build). This is the VillageSQL-specific version, not the full MySQL version string returned by `SELECT VERSION()`. This is distinct from `villagesql_schema_version`, which tracks the internal metadata catalog version.

**Querying the variable:**

You can retrieve the value using either `SHOW VARIABLES` or a `SELECT` on the global variable:

```sql
-- Returns both villagesql_server_version and villagesql_schema_version
SHOW VARIABLES LIKE 'villagesql_%';
```

```sql
SELECT @@global.villagesql_server_version;
```

**Properties:**

- **Scope:** Global
- **Dynamic:** No (set at build time)
- **Type:** String
