...
VDFs can be used in generated column expressions and `CHECK` constraints, letting you derive computed columns or enforce data validity using your extension's functions directly in the schema:
...
<Note>
Only VDFs (functions registered through the VEF SDK) are allowed in generated column
expressions and `CHECK` constraints. Traditional MySQL UDFs are not permitted here.
</Note>
