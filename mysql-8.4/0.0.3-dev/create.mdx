...
<Note>
  Parameterized types require the development SDK headers (`include-dev/`), not
  the default stable `include/` path. When building against the staged
  0.0.3-dev SDK, add `include-dev/` to your include path alongside `include/`.
  Building with `VillageSQL_BUILD_DIR` pointing to the VillageSQL source tree
  also provides these headers.
</Note>
