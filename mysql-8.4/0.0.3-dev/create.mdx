...
Every custom type needs encode, decode, and compare operations, and optionally a hash operation. There are two ways to provide these: **function pointers** or **VDF names** (VEF_PROTOCOL_2 or later). You must use exactly one of the two for each operation.
...
The named VDFs must also be registered with `.func()` in the same extension. They are registered as callable SQL functions.
