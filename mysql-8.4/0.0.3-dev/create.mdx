...
bool encode_mytype(unsigned char* buffer, size_t buffer_size,
    ...
    // Set *length to bytes written (need not equal persisted_length; server persists exactly *length bytes)
    // Set *length = SIZE_MAX to return SQL NULL
...
VEF_GENERATE_ENTRY_POINTS(
  make_extension("my_extension", "1.0.0")
    .type(make_type(MYTYPE)
      .persisted_length(16)              // Max buffer size (bytes) passed to encode/decode
...
<Note>
**`persisted_length` is a buffer ceiling, not a fixed record size.** The value you
pass to `.persisted_length(N)` sets the maximum buffer size your `encode()` function
receives. Your encode function controls the *actual* stored size by writing the byte
count into the `*length` output parameter — the server persists exactly that many
bytes. For fixed-size types like COMPLEX, always write exactly N bytes. For
variable-length types (strings, maps), write only what you need — up to N bytes.
</Note>

--- 8a6f0d2 (Clarify persisted_length semantics and add reinstall guide)
...
### VDFs in Generated Columns and CHECK Constraints

VDFs can be used in generated column expressions and `CHECK` constraints. The VDF must be declared `.deterministic()` in the extension builder — the server blocks non-deterministic functions in these contexts.
...
CREATE TABLE signals (
    ...
    magnitude DOUBLE GENERATED ALWAYS AS (complex_magnitude(impedance)) STORED,
    -- CHECK constraint enforced by a VDF
    CHECK (complex_is_finite(impedance))
);
```

<Note>
Both `complex_magnitude` and `complex_is_finite` must be registered with `.deterministic()`. Traditional MySQL UDFs are not permitted in generated columns or `CHECK` constraints.
</Note>
