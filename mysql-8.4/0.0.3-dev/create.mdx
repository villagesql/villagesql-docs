...
The types referenced below are defined in
`#include <villagesql/extension.h>` (which transitively includes the ABI
header `villagesql/abi/types.h`).
...
Do not attempt to return `true`, `false`, or any other value.
...
terminator is `VEF_MAX_ERROR_LEN` (512), giving 511 usable characters.
...
expect. `false` means success, `true` means error. This follows the same
pattern as MySQL UDF functions, which conventionally return 0 (false) for
success. Double-check every `return` statement in your encode and decode
implementations.
</Warning>
