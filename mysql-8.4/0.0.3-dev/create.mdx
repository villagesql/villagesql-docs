...
Every custom type needs encode, decode, and compare operations, and optionally a hash operation. There are two ways to provide these: **function pointers** or **VDF names** (VEF_PROTOCOL_2 or later). For each operation, provide one or the other — not both.

**Function pointers** (`false` = success, `true` = error — see [Part B](#part-b-encodedecode-return-convention)):
...
```cpp
// encode VDF: STRING -> CUSTOM(MYTYPE)
void mytype_from_string(vef_context_t *ctx, vef_invalue_t *arg0,
                        vef_vdf_result_t *result) { /* ... */ }

// decode VDF: CUSTOM(MYTYPE) -> STRING
void mytype_to_string(vef_context_t *ctx, vef_invalue_t *arg0,
                      vef_vdf_result_t *result) { /* ... */ }

// compare VDF: (CUSTOM(MYTYPE), CUSTOM(MYTYPE)) -> INT
void mytype_compare(vef_context_t *ctx, vef_invalue_t *arg0,
                    vef_invalue_t *arg1, vef_vdf_result_t *result) { /* ... */ }

// hash VDF: CUSTOM(MYTYPE) -> INT (optional)
void mytype_hash(vef_context_t *ctx, vef_invalue_t *arg0,
                 vef_vdf_result_t *result) { /* ... */ }
```
...
**Using VDF names** (VEF_PROTOCOL_2 required; see [VDF-Name Type Operations](#vdf-name-type-operations)):
...
VEF_GENERATE_ENTRY_POINTS(
  make_extension("my_extension", "1.0.0")
    .type(make_type(MYTYPE)
      ...
      .build())
    .func(make_func<&mytype_from_string>("mytype_from_string")
      .returns(MYTYPE).param(STRING).build())
    .func(make_func<&mytype_to_string>("mytype_to_string")
      .returns(STRING).param(MYTYPE).build())
    .func(make_func<&mytype_compare>("mytype_compare")
      .returns(INT).param(MYTYPE).param(MYTYPE).build())
    .func(make_func<&mytype_hash>("mytype_hash")
      .returns(INT).param(MYTYPE).build())
);
```

The named VDFs must also be registered with `.func()` in the same extension.

### VDF-Name Type Operations

Each VDF named via `.encode()`, `.decode()`, `.compare()`, or `.hash()` must be registered with `make_func<>` in the same extension with the matching signature:

| Operation | VDF SQL Signature |
|-----------|-------------------|
| `encode`  | `(STRING) -> CUSTOM(this type)` |
| `decode`  | `(CUSTOM(this type)) -> STRING` |
| `compare` | `(CUSTOM(this type), CUSTOM(this type)) -> INT` |
| `hash`    | `(CUSTOM(this type)) -> INT` |

VDF-name operations use the standard VDF `void` convention (communicate results via `result->type`), not the `bool` return convention of function-pointer encode/decode.
...
Type compatibility requires matching type name, extension name, extension version, and parameters. For example, `TVECTOR(3)` and `TVECTOR(4)` are incompatible because their parameters differ.
...
(or `.int_to_params()`, or any VDF-name field) automatically selects VEF_PROTOCOL_2, which requires
...
<Note>
Expected size for fixed-length binary storage. While the encode function may return
a smaller length (e.g., 0 to indicate an error), this field defines the standard
persisted footprint for the type.
</Note>
...
bool decode_complex(const unsigned char* buffer, size_t buffer_size,
    ...
    int len = snprintf(to, to_size, "(%.6f,%.6f)", real, imag);
    if (len < 0 || len >= (int)to_size) return true;
...
int compare_complex(const unsigned char* data1, size_t len1,
                    const unsigned char* data2, size_t len2) {
    if (len1 < 16 || len2 < 16) return 0;

    double data1_real, data1_imag, data2_real, data2_imag;
    memcpy(&data1_real, data1, 8);
    memcpy(&data1_imag, data1 + 8, 8);
    memcpy(&data2_real, data2, 8);
    memcpy(&data2_imag, data2 + 8, 8);

    if (data1_real < data2_real) return -1;
    if (data1_real > data2_real) return 1;
    if (data1_imag < data2_imag) return -1;
    if (data1_imag > data2_imag) return 1;
    return 0;
}
```
...
VDFs can be used in functional index expressions. The VDF must be declared `.deterministic()` — the same rule that applies to generated columns.
