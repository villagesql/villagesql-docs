...
`result->max_decode_buffer_length` (maximum decoded string length). Required
...
```cpp
// Called for MYTYPE(N). Writes up to VEF_MAX_TYPE_PARAMS (16) key-value
// pairs into params and sets *param_count.
...
// Validates params and computes per-column storage sizes.
// Set result->persisted_length and result->max_decode_buffer_length.
// Returns false on success, true on error (write message to error_msg).
...
Include `<villagesql/extension.h>` to pull in the full SDK.
...
To also support `MYTYPE(N)`, add `.int_to_params()`. Key/value strings must
outlive the extension (not just the call), so they must have static or
permanent heap lifetime. Static mutable buffers are not safe under concurrent
DDL; the example below uses a lookup table of string literals:
