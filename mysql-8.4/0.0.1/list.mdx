...
Extensions built-in to your VillageSQL installation are ready to install:

```sql
INSTALL EXTENSION 'extension_name';
```

**For source builds:** Only `vsql_complex` and `vsql_simple` are included by default. To use other extensions, build them from their GitHub repositories (links above) and copy the `.veb` files to your VEF directory (see [Installing External Extensions](/mysql-8.4/0.0.1/install#installing-external-extensions)).
