...
Now that the extension is active, you can use the `UUID` type in your tables just like native types. UUID v7 values are time-ordered, so they sort by creation time and carry an embedded timestamp — a sequential-friendly key without hand-rolled `BINARY(16)` generation.
