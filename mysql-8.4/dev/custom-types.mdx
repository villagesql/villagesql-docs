...
void encode_complex(std::string_view from, CustomResult out) {
    ...
    auto r = std::from_chars(p, end, real);
    if (r.ec != std::errc() || r.ptr == end || *r.ptr != ',') return fail();
    auto i = std::from_chars(r.ptr + 1, end, imag);
    if (i.ec != std::errc() || i.ptr == end || *i.ptr != ')') return fail();
...
Both operations use `<charconv>` rather than `snprintf` and `sscanf`: a
fixed-precision format such as `"%.6f"` drops digits a `double` needs, so
`0.1234567891` reads back as `0.123457`, and `std::from_chars` takes an end
pointer where `sscanf(from.data(), ...)` would read past a `std::string_view`,
which carries no terminating null.
