...
Versions with a pre-release suffix (e.g., `0.0.7-dev`) are development builds. The server blocks upgrades from a database initialized with a development build by default. Dev builds are not tested for upgrade compatibility and may include breaking schema or protocol changes.
...
```
Upgrading from a development version (0.0.7-dev) is not allowed. Use --villagesql-allow-unsafe-dev-upgrade to permit this.
```

With the flag, the server logs a warning and proceeds:

```
Upgrading from a development version (0.0.7-dev); --villagesql-allow-unsafe-dev-upgrade was specified.
```
