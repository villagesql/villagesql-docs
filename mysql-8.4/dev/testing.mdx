...
This guide covers the test-and-iterate loop for a C++ extension: setting up a local server, installing `.veb` files, running regression tests, and debugging failures. It is the companion to [Creating Extensions in C++](/mysql-8.4/dev/create), which covers the initial build, and [C++ Development](/mysql-8.4/dev/development), which covers VDF authoring depth.

<Note>
  If you are contributing to the VillageSQL server itself (not building an extension), see the [Server Development Guide](/mysql-8.4/dev/server-development), which covers running the server suites with `mysql-test-run.pl` directly, plus debugging and the contribution workflow.
</Note>

## Setting Up Your Environment

To develop and test extensions, you need a built VillageSQL server. Follow the [Clone and Build from Source](/mysql-8.4/dev/source) guide to compile the server binaries.
...
- [Testing Network-Dependent Extensions](/mysql-8.4/dev/testing-network) — reliable MTR patterns for extensions that spawn HTTP servers or external listeners
