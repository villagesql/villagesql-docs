...
This page is the reference for the C++ type operation builders. For the tutorial-level introduction to custom types, see [Custom Types in C++](/mysql-8.4/dev/custom-types).
...
Omitting a custom hash function falls back to hashing the type's raw stored
bytes. This is unsafe for a type whose encode function can produce more than
one binary representation of an equal logical value — the common case is a
floating-point field, where `-0.0` and `+0.0` compare equal but differ
bit-for-bit, so `GROUP BY` and `COUNT(DISTINCT)` would treat them as different.
...
A custom type may declare at most **65532 bytes** of storage per value. A custom
column is backed by a `VARBINARY` field, and MySQL's 65535-byte per-row budget
also pays for that field's two length bytes and the row's null byte; because
whether a column is nullable is decided per column at `CREATE TABLE`, and
nullable is the default, the cap has to leave room for the null byte. Both
`persisted_length` and `max_persisted_length` are checked against it, and the
check runs at `INSTALL EXTENSION` rather than at `CREATE TABLE`, so a type that
is too wide is rejected with the whole extension:
...
Staying under the cap does not guarantee a column fits. Every other column in
the table draws on the same 65535-byte row budget, which only `CREATE TABLE` can
weigh, so a type at or near the cap can still fail there with
`ER_TOO_BIG_ROWSIZE`. A plain `VARBINARY` of the same width behaves the same way.
...
Provide the intrinsic default one of two ways. The two are mutually exclusive:
declaring both `.intrinsic_default_str()` and `.intrinsic_default_vdf()` is a
compile-time error.
...
The parse cache is keyed on the params struct itself: `TypeParamsCache<P>` is
one instance per `P`, shared by every custom type declared with that `P`. If
more than one of your types uses the same params struct, each of them must
register the same parse function; a type that needs to parse differently needs
its own params struct.
...
`build()` requires `.resolve_params<>()` whenever `.params<>()` or
`.int_to_params<>()` is registered: `int_to_params` only rewrites the
`MYTYPE(N)` shorthand into a parameter string, while `resolve_params` is the
gate that validates the parameters and resolves storage size. Omitting it is a
compile-time error.

<Warning>
`.max_persisted_length()` requires VEF Protocol 3 or higher. A type using it
cannot be loaded by pre-Protocol-3 servers. It is valid only for variable-length
or parameterized types; a fixed non-parameterized type that sets it fails to
compile.
</Warning>

The sizes `resolve_params` reports are also validated at `CREATE TABLE` time,
once a column's parameters are known. For a fixed-length type, the resolved
`persisted_length` must not exceed the declared `max_persisted_length`: a
parameterization whose computed footprint (for example a `dimension * 4` sizing)
overruns that bound is rejected with a syntax error. Independently of length
kind, `resolve_params` must resolve a `max_decode_buffer_length` greater than
zero, since that value sizes the decode scratch buffer; a non-positive result is
rejected with the same class of error.
...
`SHOW CREATE TABLE` does not repeat what the author wrote. The server keeps the
resolved parameter map and rebuilds a display form from it, preferring
`MYTYPE(N)` when `int_to_params(N)` reproduces the whole map and falling back to
the canonical `MYTYPE('key=value,...')` with the keys sorted. A column written as
`MYTYPE('dimension=8')` therefore reads back as `MYTYPE(8)`.
...
Whichever syntax supplies the parameters, the server validates the resulting
map before calling `resolve_params`: every key must be non-empty, appear only
once, and have a non-empty value. A violation is a syntax error naming the
type. Except for an empty parameter name, it also names the offending key:

```sql
INSTALL EXTENSION vsql_tvector;
CREATE TABLE t3 (v vsql_tvector.TVECTOR('dimension=3,dimension=4'));
-- ERROR 1064 (42000) at line 1: Type 'vsql_tvector.TVECTOR': duplicate parameter 'dimension' near 'vsql_tvector.TVECTOR('dimension=3,dimension=4'))' at line 1
```
...
A mutating `resolve_params` callback may add or change parameters, but
removing one the caller wrote is an error: `Type '<extension>.<type>':
resolve_params removed parameter '<key>'; parameters may be added or changed
but not removed`.
...
a buffer for the backing field. A variable-length type must not also call
`.persisted_length()`: its size is decided per value, so `build()` rejects the
...
- [Custom Types in C++](/mysql-8.4/dev/custom-types) — tutorial introduction to custom types
- [C++ API Reference](/mysql-8.4/dev/extension-api-reference) — VDF contracts, null handling, and buffer sizing
- [C++ Development](/mysql-8.4/dev/development) — VDF authoring depth, argument and result types, aggregates, varargs
