...
<Note>
  Custom types are exported in their string representation format. `SELECT ... INTO DUMPFILE` is the exception: it writes the value's stored binary form, so a `COMPLEX` value lands as the 16 bytes its `persisted_length` declares rather than as `(3,4)`.
</Note>
