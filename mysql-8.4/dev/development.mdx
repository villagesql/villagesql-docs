...
- `.accumulate<&fn>()` registers your `void(State&, TypedArgs...)` fold function. `TypedArgs` are deduced from the function signature (`IntArg`, `StringArg`, etc.). Call `.accumulate<>()` after all `.param(TYPE)` calls: the builder checks the typed-argument count against the declared SQL parameter count at compile time, so calling it earlier fails to build.
...
| `REGISTRATION_JSON` | `VARCHAR(65535)` | JSON serialization of the `vef_registration_t` struct, including `funcs` and `types` arrays and an `sdk_version` key reporting the SDK version the extension was built against. |
