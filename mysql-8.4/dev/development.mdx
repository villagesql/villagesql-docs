...
picking a CPU-specific implementation, reading its own configuration once, or
building a lookup table the VDFs then read. Register that work with
`.on_init<&Fn>()` on the extension builder, and the matching teardown with
`.on_deinit<&Fn>()`. Both take a function with the signature `void()`, and must
be registered together — declaring one without the other fails to compile. An
extension that reaches the server with a mismatched pair some other way is
rejected at load instead:

```text
ERROR 3219 (HY000): Failed to load VEF extension '<name>': extension registers
an on_init hook without an on_deinit
```
...
`on_init` runs once per load — at `INSTALL EXTENSION`, and again on every server
startup — after the server has validated the extension and populated its
required capabilities, so a registered system variable already holds its
configured value. At startup specifically, `on_init` is deferred until
persisted system-variable values have been applied, so it never sees a
compiled-in default in place of a value set with `SET PERSIST`. It never runs
for an extension the server rejected. `on_deinit` runs at unload —
`UNINSTALL EXTENSION` or server shutdown — before the server depopulates those
same capabilities, so it sees the same live state `on_init` did.
