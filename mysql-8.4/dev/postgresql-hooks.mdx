...
`on_init()` and `on_deinit()` run with the extension's declared capabilities
live: `on_init()` after the server has populated them, and `on_deinit()` before
it depopulates them, so a hook can read a registered system variable. They also
suit local setup such as choosing CPU-specific function pointers.
