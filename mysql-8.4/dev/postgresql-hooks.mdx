...
| Running SQL from an extension | Yes | Yes |
...
| Custom data types and operators | Register a new base type with its own storage and comparison behaviour | **Available** — see [Custom Types](/mysql-8.4/dev/custom-types) |
| Aggregate functions | Register a user-defined aggregate | **Available** — `make_aggregate_func`, see [Development Guide](/mysql-8.4/dev/development) |
...
`on_init()` and `on_deinit()` now run through the server rather than only
inside the extension process: `on_init()` runs once the extension's required
capabilities are populated, and `on_deinit()` runs before they are
depopulated, so both can reach a declared capability — reading a registered
system variable, for instance. They still suit local setup such as choosing
CPU-specific function pointers; setup that runs SQL belongs in
`vsql::preview::sql_query` instead.
...
plugin stage roles for the session once it has resolved an external identity,
and `roles_mode()` registers a callback, re-queried on every login, that
decides whether the server only activates the staged roles, also grants them,
or syncs the account's grants to match (an operator-pinned `DEFAULT ROLE`
survives a sync). Both are declared in
