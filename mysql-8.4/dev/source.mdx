...
These steps build VillageSQL on upstream MySQL 8.4. VillageSQL also builds on
Percona Server as an alternative codebase — see the
[Percona Build](/mysql-8.4/dev/percona) page.
...
of the branch: `-b publish/mysql-8.4_0.0.6`.
...
```
8.4.11-villagesql-0.0.6-dev-eac70667eb6
```
...
For GDB debugging, running individual tests, re-recording expected results, and contributing to the server codebase, see the [Server Development Guide](/mysql-8.4/dev/server-development).
