...
- **CMake** 3.14.6 or higher - Build system generator
- **C++ Compiler** - GCC 10+, Clang 14+, Xcode 10+ on macOS, or MSVC 2019
  Update 11 (16.11)+
...
<Note>
  **Linux users:** Use `$HOME` for absolute paths. **macOS users:** Use `~` (tilde), except in `mysqld` option values such as `--datadir=` and `--basedir=`, which need `$HOME`. The shell does not expand a tilde that follows `=`, and `mysqld` then treats the literal `~` as a directory name and aborts. `cmake` expands the tilde itself, so `-DCMAKE_INSTALL_PREFIX=~/mysql` above is fine. Replace the repository path with your actual clone location if different.
</Note>
...
[Step 6](#step-6-connect-with-mysql-client); `make -j10 mysqld` alone builds
...
**macOS:**
```bash
ls ~/build/villagesql/bin/mysqld ~/build/villagesql/bin/mysql
```
