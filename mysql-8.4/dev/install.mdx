...
A revoke takes effect on the next connection. An existing session keeps the
privileges it connected with until it reconnects, so an operator who just lost
`EXTENSION_ADMIN` can still run extension DDL until the current session ends.
