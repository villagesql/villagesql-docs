...
On a freshly initialized data directory, `root@localhost` receives
`EXTENSION_ADMIN` automatically, so a new server needs no manual grant. When an
existing data directory is upgraded in place, every user account holding `SUPER`
is granted `EXTENSION_ADMIN` — but only if no account holds it already, so a
server upgraded once is not backfilled again on later upgrades. The reserved
`mysql.*` system accounts are excluded.
