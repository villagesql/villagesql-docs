...
For development or custom builds, follow the [Clone and Build from Source Guide](/mysql-8.4/dev/source) to compile from the latest code.

VillageSQL also builds on Percona Server as an alternative to upstream MySQL —
see the [Percona Build](/mysql-8.4/dev/percona) page for what that involves.
...
- `vsql_complex` and `vsql_simple`, reference extensions used elsewhere in these docs (see [C++ Extension Examples](/mysql-8.4/dev/examples)) to show how the extension framework works.
...
For more details, see [Installing Extensions](/mysql-8.4/dev/install).
...
<CardGroup cols={2}>
  <Card title="Managing Extensions" icon="puzzle-piece" href="/mysql-8.4/dev/managing">
    Learn how to install and manage other extensions.
  </Card>
  <Card title="Create an Extension" icon="code" href="/mysql-8.4/dev/create">
