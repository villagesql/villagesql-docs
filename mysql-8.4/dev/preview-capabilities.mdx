...
description: "Preview capabilities give extensions access to server features that are still stabilizing. This page covers enabling the preview tier, the keyring, mysql_services, status_var, sys_var, thread_worker, sql_query, and statement_event capabilities, and registration patterns."
---
...
| `vsql::preview::mysql_services` | `<villagesql/preview/mysql_services.h>` | Preview |
...
extension consume MySQL registry services — the services a MySQL component
consumes, whether an installed component or the server core provides them. The
...
Reach for it when a server facility has no VEF capability of its own — the
error log, session attributes, or the keyring's own component services are all
available this way. Only consuming is supported: registering the extension's own
...
`VSQL_REQUIRE_SERVICE(services, name, var)` does two things in one line: it
declares `var`, the reference the server writes the acquired service into, and
it registers `name` on `services`. Both the `MysqlServices` object and every
reference must be `static` — the server writes through them at load, so they
have to outlive registration.

The macro spends one `name` on two roles: the C++ `SERVICE_TYPE(name)` and the
string the server looks up in the registry. When a service's registry name is
not its type name, declare the reference and require it explicitly instead:

```cpp
static preview_mysql_services::ServiceRef<SERVICE_TYPE(svc)> svc_ref;
static const int svc_req =
    (services.require<SERVICE_TYPE(svc)>("svc.impl", svc_ref), 0);
```
...
```cpp
if (!error_log.valid()) { out.set(1); return; }
const bool err = error_log->emit(/* ... */);
```

Check `valid()` before every `->` call. `->` dereferences the acquired pointer,
which is null when the service was not acquired.
...
Every declared service is acquired before any of the extension's functions can
run, so a service that is not registered is an install-time failure rather than
a runtime surprise. `INSTALL EXTENSION` fails and names the service:
...
The `vsql_mysql_services_errlog_test` extension that ships with the server. It
writes a line to the server error log through `mysql_simple_error_log`, a
server-core service that is registered on every server, so nothing has to be
installed first:
...
// log_warning(text) -> INT: 0 on success, 1 on error. WARNING severity is used
// so the line survives the default log_error_verbosity of 2, which drops
// informational lines.
...
Install it and call the function; a `0` means the service accepted the line:

```sql
INSTALL EXTENSION vsql_mysql_services_errlog_test;
SELECT vsql_mysql_services_errlog_test.log_warning('service-check') AS emit_status;
```
...
The line lands in the server error log, which `performance_schema` exposes:

```sql
SELECT COUNT(*) > 0 AS marker_logged FROM performance_schema.error_log
WHERE DATA LIKE '%service-check%';
```
