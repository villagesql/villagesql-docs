...
A minimal authenticator, condensed from the `vsql_auth_test` extension that
ships with the server. It accepts one fixed token, maps the connection to
`vsql_auth_test_user`, and pins `mysql_clear_password` so the token arrives
verbatim in the password slot. (The shipping extension adds extra token paths
and the `.auto_create()` opt-in described below to drive its test suite.)
...
The `vsql_auth_test` extension opts in, and provisions the connecting user with
the role `vsql_role_granted`. Create that role first, then connect as an account
that does not exist:
