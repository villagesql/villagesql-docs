...
that does either deadlocks on the system-variable lock. Keep an `on_change`
callback short and non-blocking, and move work that needs SQL, or that can
block, into a [thread worker](#thread-worker).
</Warning>
