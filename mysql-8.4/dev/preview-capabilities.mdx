...
| `vsql::preview::index_profile` | `<villagesql/preview/index_builder.h>` | Preview — see [Custom Indexes](/mysql-8.4/dev/custom-indexes) |
| `vsql::preview::index_type` | `<villagesql/preview/index_builder.h>` | Preview — see [Custom Indexes](/mysql-8.4/dev/custom-indexes) |
...
| `vsql::session_var` | `<villagesql/preview/session_var.h>` | Preview |
...
MySQL system variables backed by extension-owned storage. Five types are
supported: `BOOL` (`bool *`), `INT` (`long long *`), `DOUBLE` (`double *`),
`STR` (`char **`), and `ENUM` (`unsigned long *`, storing a zero-based index
into a fixed, ordered list of names). `INT` and `DOUBLE` descriptors also
carry `min_val` and `max_val` bounds; all descriptors carry a default value
and a comment.

Build the capability with `vsql::preview_sys_var::make_capability()` and the
matching factory functions `make_bool`, `make_int`, `make_double`, `make_str`,
and `make_enum`. The
...
| `sv::make_enum` | `unsigned long *` | `names`, `name_count`, `def_val` |

An enum variable is set by name (`SET GLOBAL my_ext.mode = 'sync'`) or by the
name's zero-based index; either way the server validates the value against
`names` before accepting it, rejecting an unrecognized name or an
out-of-range index rather than storing it:

```
ERROR 1231 (42000): Variable 'my_ext.mode' can't be set to the value of 'bogus'
```

That validation is what distinguishes it from a free-form `make_str`: the
value space is closed and enforced at `SET` time. `names` and its strings
must have static storage duration — the server keeps a `TYPELIB` pointing at
them for as long as the variable is registered. A convenience overload
deduces `name_count` from a `static const char *const NAMES[] = {...}` array,
so the count argument can be omitted.
...
static unsigned long g_mode  = 0;  // index into MODE_NAMES; default "off"
...
The session_var capability (`vsql::session_var`) lets an extension register
per-session (THD-local) system variables — the functional equivalent of a
plugin's `MYSQL_THDVAR_*`. It is the session-scoped sibling of
[`vsql::sys_var`](#system-variables): a sys_var has one value shared by every
connection, while a session_var gives each connection its own value, set with
`SET SESSION`, with the descriptor's default as the starting point for every
new session and as the value read by `SET GLOBAL`. A variable's scope is fixed
at registration — use this capability for per-session variables, `vsql::sys_var`
for global ones.

Only two types are supported: `INT` (`long long`) and `STR` (`std::string`).
There is no `BOOL` or `DOUBLE` session variable, and there is no `on_change`
callback mechanism — unlike `vsql::sys_var`, an extension observes a session
variable's value only by reading it back, not by being notified when it
changes.

### Declaring the Capability

Build the capability with `vsql::preview_session_var::make_capability()`,
passing a braced list of descriptors from `make_int(name, comment)` or
`make_str(name, comment)`, each configured with the fluent `.default_()`,
`.min()`, and `.max()` setters (`min`/`max` apply to `INT` only):
...
static auto SESSION_VARS = sv::make_capability({
    sv::make_int("session_int_var", "Search width")
        .default_(20)
        .min(1)
        .max(4096),
    sv::make_str("label", "Per-session label")
        .default_("default"),
});
...
`make_int` starts unconstrained (default `0`, full `long long` range);
`make_str` starts with an empty-string default. The capability object needs
static storage duration, matching every other preview capability on this page.

### Reading Session Values

There are two read paths, both resolved against the *calling* connection's
thread — so both **must be called on the connection thread (a VDF or callback
running on the connection), never from a background thread worker**:

- `SESSION_VARS.get_session_int(name, out)` / `get_session_str(name, out)` —
  name-keyed, re-resolves the variable on every call. Use this for occasional
  reads, and for `STR` variables it is the only path: there is no lock-free
  string fast path, because a `STR` slot holds a pointer to a separately
  allocated buffer that a concurrent `SET SESSION` or `UNINSTALL` can free.
- `SESSION_VARS.int_var(name)` returns an `IntVar` that resolves itself lazily
  on its first `read()` and caches the result, so it can be declared at
  file/static scope before the capability has loaded. Each subsequent `read()`
  is a lock-free per-thread offset load — no name lookup, no global lock — the
  equivalent of a plugin's `THDVAR(thd, var)`. This fast path is `INT`-only.
...
void read_session_int_var_impl(IntResult out) {
  long long val = 0;
  if (SESSION_INT.read(val)) { out.set_null(); return; }
  out.set(val);
}
```
...
After `INSTALL EXTENSION my_ext`, each variable takes the extension name as a
component prefix and is readable/settable with both `@@session.` and
`@@global.`:

```sql
SELECT @@global.my_ext.session_int_var;
SELECT @@session.my_ext.session_int_var;
SET SESSION my_ext.session_int_var = 100;
```

A second connection is unaffected by the first connection's `SET SESSION`, and
sees the compiled default until it sets its own value; `SET GLOBAL` changes the
default that new or reset sessions pick up, not any session's already-set
value.

Out-of-range `INT` assignments clamp to the bound and warn, and a wrong-typed
assignment is rejected, exactly as for an ordinary MySQL system variable:

```sql
SET SESSION my_ext.session_int_var = 999999;  -- max is 4096
```
```text
Warning	1292	Truncated incorrect my_ext.session_int_var value: '999999'
```
```sql
SET SESSION my_ext.session_int_var = "hello";
```
```text
ERROR 1232 (42000): Incorrect argument type to variable 'my_ext.session_int_var'
```

### Restart Behavior

Session variables are **not** persisted the way `SET PERSIST` makes an ordinary
sys_var durable — there is no `SET PERSIST` support for a session_var's own
value. Across a server restart with the extension still installed, each
variable reappears at its compiled default, not at whatever value a session
had set before the restart: a `SET SESSION` value is per-connection state, and
connections do not survive a restart in the first place, so there is nothing to
lose — the variable's *global default* was never changed by that session's
`SET SESSION` either, so restart simply reinstalls the variable at the same
...
| `client_pid()`, `client_name()`, `program_name()` | Client-supplied connection attributes sent once at handshake (`_pid`, `_client_name`, `program_name`): the client's OS process id, connector/driver library (e.g. `libmysql`, `mysql-connector-j`), and application name (e.g. `mysql`, `mysqldump`). Each is `NULL` when the client did not send that attribute. |
...
| `.roles_mode(callback)` | Optional. Chooses how the server reconciles the roles the handler stages with the account's granted roles — activate-only, also grant, or fully sync (see [Auto-Granting Roles](#auto-granting-roles)). |
...
it; one it does not hold is skipped, and the server logs why (`VEF auth: role
'<role>'@'<host>' requested for account '<user>'@'<host>' is not granted;
skipping`). `.roles_mode(&callback)` changes that: the callback returns a
`vef_auth_roles_mode_t` — `VEF_AUTH_ROLES_ACTIVATE` (the default),
`VEF_AUTH_ROLES_GRANT`, or `VEF_AUTH_ROLES_SYNC` — and the server reconciles
the staged roles with the account's granted roles accordingly on each login.

The callback matches `.auto_create()` in shape — no arguments — but returns the
mode enum instead of a `bool`, and the server calls it on each login, reading
whatever backs it live:

```cpp
unsigned long g_auto_grant = 0;  // 0=ACTIVATE, 1=GRANT, 2=SYNC
...
The two opt-ins run at different points in a login. `.auto_create()` decides
whether a login for an account that does not yet exist is allowed to provision
one; `.roles_mode()` runs afterward, against whatever account the login
resolved to — the one just provisioned or one that already existed — so the
same reconciliation applies either way. `VEF_AUTH_ROLES_ACTIVATE`, or no
callback at all, keeps the activate-only default.

`VEF_AUTH_ROLES_GRANT` is additive: it grants a staged role the account lacks,
and never runs the revoke step at all. `VEF_AUTH_ROLES_SYNC` runs that step:
it revokes every granted role the token did not claim, so a role dropped from
the token is dropped from the account too on the next login — except a role
set as the account's `DEFAULT ROLE` by an operator's `ALTER USER`, which SYNC
never revokes. Under either mode, a staged role that does not exist as a DB
role fails its `GRANT` and is logged and skipped rather than failing the
login.

`vsql_auth_test` exposes this as `vsql_auth_test.auto_grant`, an `OFF`/`ON`/`SYNC`
enum (also `OFF` to start) whose index order matches the mode order above. Its
`-token-roles` token stages `vsql_role_granted` and `vsql_role_denied`, and the
account below holds neither. With the setting off, the login leaves the
...
<Warning>
With `vsql_auth_test.auto_grant` set to `ON` or `SYNC`, a valid token is enough
to gain any role it names. A role that does not already exist as a DB role
fails its `GRANT` and is skipped, so a token still cannot invent a role that
was never created, but the DBA no longer decides which of the roles that do
exist an account may reach — the auth method does. `SYNC` goes further: it
also revokes a role a DBA granted out of band but the token never claims, when
the account next logs in — an operator can still pin one role past that by
setting it as the account's `DEFAULT ROLE`, which SYNC leaves alone. A role
revoked from `vsql_auth_test_user` does not disappear from a session that
already activated it: querying `CURRENT_ROLE()` again in that same session,
after the revoke, still lists the role.
</Warning>
