...
<Note>
If you prefer C++, see [Creating Extensions in C++](/mysql-8.4/dev/create) for the C++ SDK walkthrough.
</Note>
...
Before you begin, build VillageSQL from source — extensions link against the server's build tree. Follow the [Build from Source](/mysql-8.4/dev/source) guide first.
...
Create a new Rust library crate and add the SDK:
...
Then set the crate type in `Cargo.toml`:
...
Two worked examples in the [vsql-rust-sdk](https://github.com/villagesql/vsql-rust-sdk) repo cover the aggregate and varargs support mentioned in the Warning above:

- [`examples/vsql_agg_sum`](https://github.com/villagesql/vsql-rust-sdk/tree/main/examples/vsql_agg_sum) — an aggregate VDF that reimplements `SUM` with `agg_func!`, using separate `clear`, `accumulate`, and `result` callbacks over a per-group accumulator.
- [`examples/vsql_varargs`](https://github.com/villagesql/vsql-rust-sdk/tree/main/examples/vsql_varargs) — four varargs functions built with `varargs_func!`: `str_join` (validated, stateful), `arg_count` (no validation, no state), `describe` (validated, stateless, heterogeneous INT/REAL/STRING args), and `point_path` (varargs over a custom `point2d` type).

Each example ships a complete `Cargo.toml`, `manifest.json`, and MTR test.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="flask"
    href="/mysql-8.4/dev/rust-preview-capabilities"
  >
    Status variables, system variables, background thread workers, and
    keyring access via the requires: list.
  </Card>
  <Card
  ...
  <Card
    ...
    icon="network-wired"
    href="/mysql-8.4/dev/testing-network"
  >
