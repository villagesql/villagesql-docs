...
description: "Use VillageSQL preview capabilities from the Rust SDK — declare status variables, system variables, background thread workers, and keyring access with the requires: list in extension!."
---
    ...
    if reason == WakeupReason::Periodic {
        if let Some(session) = SQL_QUERY.open(handle) {
            if let Some(mut result) = session.execute("SELECT COUNT(*) FROM mysql.user") {
                if let Some(row) = result.next_row() {
                    if let Some(n) = row.get_int(0) {
