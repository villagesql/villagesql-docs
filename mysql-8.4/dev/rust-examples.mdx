...
This page walks through two of the reference extensions in the Rust SDK repository — a minimal function-only example and a complete custom type with arithmetic, ordering, and hashing. The repository's `examples/` directory also carries one example per supported [preview capability](/mysql-8.4/dev/rust-preview-capabilities).
