...
Versions with a pre-release suffix (e.g., `0.0.4-dev`) are development builds. The server blocks upgrades from a database initialized with a development build by default.
...
When an upgrade from a dev version is detected, the server logs a warning and proceeds:
