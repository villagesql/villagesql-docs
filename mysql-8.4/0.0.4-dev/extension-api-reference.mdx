...
To opt in, add the include to your extension source. For example, the column
storage API is currently in the preview tier:

```cpp
#include <villagesql/preview/storage_api.h>
```

The namespace matches the header tier (for example, `vsql::preview_storage`
for the include above).
