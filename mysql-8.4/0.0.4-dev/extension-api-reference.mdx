...
Some VEF capabilities are available as opt-in headers in the SDK include tree.
They live under two tiers, both unstable:

- `villagesql/experimental/` — early ideas we are still shaping. The API shape
  is not settled.
- `villagesql/preview/` — more developed APIs that are closer to stabilizing,
  but the ABI and API are still under active development and may change
  without notice.

To opt in, add the include to your extension source. Two capabilities are
currently in the preview tier:

```cpp
#include <villagesql/preview/storage_api.h>  // vsql::preview_storage
#include <villagesql/preview/keyring.h>      // vsql::preview::keyring
```

Neither header is pulled in by `<villagesql/vsql.h>`; you must include it
directly when you opt in.

Namespace layout under `vsql::preview` is per-capability — there is no single
universal pattern. The storage API uses a flat name (`vsql::preview_storage`),
while the keyring API uses a nested sub-namespace (`vsql::preview::keyring`,
with the registration traits type `vsql::preview::preview_keyring` used in
`.with<preview_keyring<g_keyring>>()`). Check each header for the exact
namespace it defines.

For the full keyring usage docs (read/write signatures, result codes, and
examples), see [Keyring Access](/mysql-8.4/0.0.4-dev/preview-capabilities#keyring-access).

<Warning>
Experimental and preview headers are not stable. An extension built against
them may break when the server is updated. When a feature stabilizes, its
headers move to a versioned stable SDK path.
</Warning>
