...
`villagesql/experimental/` in the SDK include tree. These headers reflect ideas
we are still working on and actively iterating on. The API shape is not settled
yet but we appreciate feedback.
...
<Warning>
Experimental headers are not stable. An extension built against them may break
