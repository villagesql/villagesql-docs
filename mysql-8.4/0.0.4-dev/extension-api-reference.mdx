...
The typed C++ wrappers used by extension authors are available via
`#include <villagesql/vsql.h>`. Underlying raw ABI types (`vef_context_t*`,
`vef_vdf_func_t`, etc.) are not exposed by that header.

### Part A: VDF Function Contracts

**1. VDF implementation functions are `void` -- they never return a value.**

Internally, the server invokes VDFs through this raw ABI typedef (not written
directly by extension authors):

```cpp
// Raw ABI — the SDK generates the bridge to this automatically.
typedef void (*vef_vdf_func_t)(vef_context_t *ctx, vef_vdf_args_t *args,
                                vef_vdf_result_t *result);
```

Extension authors write the per-argument typed form. The SDK generates the
bridge to the raw ABI automatically:
...
[Per-Statement State](/mysql-8.4/0.0.4-dev/development#per-statement-state-prerun-and-postrun)
in the Development guide.

<Note>
**Most extensions don't need prerun/postrun hooks.** The VEF SDK automatically
handles common cases like type checking and buffer allocation. Use
prerun/postrun for expensive per-statement setup or when you need per-statement
state that varargs or buffer-size tuning depends on.
...
[Aggregate VDFs](/mysql-8.4/0.0.4-dev/development#aggregate-vdfs) for the
...
`villagesql/preview/` in the SDK include tree. The ABI and API are still
...
For the full preview API documentation, see [Preview Capabilities](/mysql-8.4/0.0.4-dev/preview-capabilities).
