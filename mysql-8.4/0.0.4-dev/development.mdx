...
Register `.params<>()` on the type builder. Use `.int_to_params<&fn>()`
to handle `MYTYPE(N)` integer syntax and `.resolve_params<&fn>()` to
...
the VDF separately via `make_intrinsic_default<&fn>()`.
...
constexpr auto MYTYPE =
    vsql::make_type<kMyTypeName>()
        ...
        .int_to_params<&mytype_int_to_params>()
...
Extensions access the MySQL keyring through `villagesql::keyring::read()` and `villagesql::keyring::write()`. Both are available at Protocol 2 (`VEF_PROTOCOL_2`) and above.
...
```cpp
vef_keyring_result_t villagesql::keyring::read(
...
```cpp
vef_keyring_result_t villagesql::keyring::write(
    std::string_view data_id,
    std::string_view auth_id,
