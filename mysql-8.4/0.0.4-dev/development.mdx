...
The call site matters. The SDK encode wrapper pre-sets `VEF_RESULT_WARNING`
before invoking your function, so an encode that returns without calling
anything surfaces as a warning. The decode wrapper pre-sets
`VEF_RESULT_ERROR`, so a decode that returns silently aborts the statement.
At INSERT or UPDATE, `.warning()` from an encode function is promoted to a
statement error in strict mode (`STRICT_TRANS_TABLES`); in a SELECT,
`.warning()` returns NULL for that row while execution continues.
