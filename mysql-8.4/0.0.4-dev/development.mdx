...
using TypeDecodeFunc = bool (*)(vsql::Span<const unsigned char> data,
                                vsql::Span<char> out,
                                size_t *out_len);

// Compare: returns -1, 0, or 1 (used for ORDER BY and indexes).
using TypeCompareFunc = int (*)(vsql::Span<const unsigned char> a,
                                vsql::Span<const unsigned char> b);

// Hash: returns hash code (used for hash joins).
using TypeHashFunc = size_t (*)(vsql::Span<const unsigned char> data);
```
...
int mytype_compare(const MyTypeParams &params,
                   vsql::Span<const unsigned char> a,
                   vsql::Span<const unsigned char> b) {
  // Returns -1, 0, or 1.
}
...
The `vsql::make_type` template methods detect the params argument as the
first argument and route through the params cache automatically. Encode
functions take `vsql::MaybeParams<P> &` as the first argument; `is_known()`
is always true at runtime, and `value()` returns `const P&`. Decode,
compare, and hash variants continue to take `const P&` directly.
...
Status variables require Protocol 2 (`VEF_PROTOCOL_2`). The builder raises the
extension's minimum protocol automatically when `.status_var()` is called.

Chain `.status_var()` on the extension builder, passing a descriptor created
by `make_status_var_int` (for `long long *`) or `make_status_var_double` (for
`double *`). Each helper takes a name and a pointer to a global in the
extension's `.so`. The storage pointer must remain valid for the lifetime of
the extension.
...
VEF_GENERATE_ENTRY_POINTS(
    make_extension()
        ...
        .status_var(make_status_var_int("add_calls", &g_add_calls)))
```
