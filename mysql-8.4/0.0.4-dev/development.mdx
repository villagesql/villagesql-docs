...
Use the `TYPE::method` naming convention when registering these VDFs. The prefix
before `::` must match the type name (case-insensitive), and the suffix must be
one of: `from_string`, `to_string`, `compare`, or `hash`. This convention enables
`TYPE::method(args)` SQL call syntax — for example, `SELECT MYTYPE::from_string('value')`.
Extensions that don't need the `::` SQL syntax can still use plain names.
...
VEF_GENERATE_ENTRY_POINTS(
    make_extension("my_ext", "1.0.0")
        .type(make_type(MYTYPE)
                  ...
                  .encode("MYTYPE::from_string")    // Name of the encode VDF
                  .decode("MYTYPE::to_string")      // Name of the decode VDF
                  .compare("MYTYPE::compare")       // Name of the compare VDF
                  .hash("MYTYPE::hash")             // Optional: name of the hash VDF
                  .build())
        ...
        .func(make_type_hash<&my_hash>("MYTYPE::hash", MYTYPE)))
```
...
VEF_GENERATE_ENTRY_POINTS(
    make_extension("my_ext", "1.0.0")
        .type(make_type(MYTYPE)
                  ...
                  .build())
        .func(make_type_encode<&mytype_encode>("MYTYPE::from_string", MYTYPE))
        .func(make_type_decode<&mytype_decode>("MYTYPE::to_string", MYTYPE))
