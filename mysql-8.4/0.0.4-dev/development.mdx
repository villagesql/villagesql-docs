...
```cpp
// Encode: string -> custom binary. Report outcome via out.set_length(n),
...
void mytype_encode(vsql::MaybeParams<MyTypeParams> &params, std::string_view from,
                   vsql::CustomResult out) {
  size_t bytes = (size_t)params.value().dimension * 4;
  auto dst = out.buffer();
  if (dst.size() < bytes) { out.error("buffer too small"); return; }
  // ... parse from, write to dst ...
