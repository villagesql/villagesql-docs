...
server can publish inferred params (for example, from constant-string
inference at fix-fields time) in the same shape `parse` consumes.
...
uses this only on the fix-fields-time constant-string inference path, where it has not
yet inferred the params and so cannot consult `resolve_params` to size the encode buffer.
