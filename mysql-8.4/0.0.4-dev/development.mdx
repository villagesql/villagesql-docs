...
**Intrinsic default.** For a string-literal default value, use
`.intrinsic_default_str("literal")`. For a VDF-based default that needs
the params struct, use `.intrinsic_default_vdf("vdf_name")` and register
the VDF separately with `make_intrinsic_default<&fn>()` (see the
parameterized types example below).
...
VEF_GENERATE_ENTRY_POINTS(
    make_extension("my_ext", "1.0.0")
        ...
        .func(make_intrinsic_default<&mytype_default>(
            "mytype_intrinsic_default", MYTYPE)))
```
