...
An intrinsic default provides the encoded binary value that the server writes
when `INSERT IGNORE` or `UPDATE IGNORE` assigns `NULL` to a `NOT NULL`
custom-type column. Without one, the server has no fallback value and raises
an error.

The function receives a writable buffer and a length output pointer. It writes
the encoded default into the buffer and sets `*length` to the number of bytes
written. Returns `false` on success, `true` on error.
...
Register the function with `make_intrinsic_default<>()` and bind it by name
on the type builder with `.intrinsic_default()`:

```cpp
bool mytype_default(villagesql::Span<unsigned char> buffer, size_t *length,
                    char* /*error_msg*/) {
  if (buffer.size() < 8) { *length = 0; return true; }
...
VEF_GENERATE_ENTRY_POINTS(
    make_extension("my_ext", "1.0.0")
        .type(make_type(MYTYPE)
                  .persisted_length(8)
                  .max_decode_buffer_length(64)
                  .encode("MYTYPE::from_string")
                  .decode("MYTYPE::to_string")
                  .compare("MYTYPE::compare")
                  .intrinsic_default("mytype_intrinsic_default")
                  .build())
        .func(make_type_encode<&my_encode>("MYTYPE::from_string", MYTYPE))
        .func(make_type_decode<&my_decode>("MYTYPE::to_string", MYTYPE))
        .func(make_type_compare<&my_compare>("MYTYPE::compare", MYTYPE))
        .func(make_intrinsic_default<&mytype_default>(
            "mytype_intrinsic_default", MYTYPE)))
```

For parameterized types, the intrinsic default function can accept
`const P&` as its first parameter. The SDK detects this and routes through
the params cache automatically, using the same parse function bound via
`.params<P, &parse_fn>()`.
