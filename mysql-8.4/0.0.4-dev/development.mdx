...
<Note>
  If you are contributing to the VillageSQL server itself (not building an extension), see [Clone and Build from Source](/mysql-8.4/0.0.4-dev/source), which covers the full server developer workflow including running tests with `mysql-test-run.pl` directly.
</Note>
...
<Warning>
  ...
  and subject to change before the beta release. See
  [Protocol 2 (Unstable API)](/mysql-8.4/0.0.4-dev/protocol-2) for full
  C++ signatures.
</Warning>
