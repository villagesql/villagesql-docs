...
Keyring access is exposed as a preview capability. Extensions declare a
`vsql::preview::keyring::Capability` object on the extension builder; the
server populates the capability's vtable at load time when a MySQL keyring
component is installed. Both reads and writes go through the capability
object's methods.

The capability name is `vsql::keyring`
(`VEF_PREVIEW_KEYRING_NAME` in `villagesql/abi/preview/keyring.h`).

#### Result Codes

`Capability::read` and `Capability::write` both return
`vef_keyring_result_t` (defined in `villagesql/abi/preview/keyring.h`):

| Code | Value | Meaning |
|------|-------|---------|
| `VEF_KEYRING_OK` | 0 | Operation succeeded. |
| `VEF_KEYRING_NOT_FOUND` | 1 | The requested key does not exist (read only). |
| `VEF_KEYRING_UNAVAILABLE` | 2 | No keyring component is installed. |
| `VEF_KEYRING_ERROR` | 3 | Other failure. |

`Capability::available()` returns `false` when the server has not bound a
keyring vtable; `read` and `write` return `VEF_KEYRING_UNAVAILABLE` in that
state.

#### Declaring the Capability

Include the preview keyring header (the umbrella `<villagesql/vsql.h>` does
not include it):
...
Define a static capability object at file scope and register it on the
extension builder via `.with<preview_keyring<cap>>()`:
...
The `preview_keyring<cap>` traits type takes the capability object by
reference as a non-type template parameter; the same object is read by
extension code and written by the server.

#### Reading and Writing

Call `read` and `write` on the capability object. `auth_id` may be empty
to address internal keys.

```cpp
vef_keyring_result_t Capability::read(
    std::string_view data_id,
    std::string_view auth_id,
    std::string &value) const;

vef_keyring_result_t Capability::write(
    std::string_view data_id,
    std::string_view auth_id,
    std::string_view data) const;
```

On `VEF_KEYRING_OK`, `read` resizes `value` to the actual length of the
secret.

#### Complete Example

This extension exposes `keyring_read` and `keyring_store` VDFs. It is the
canonical reference implementation (`test-extensions/vsql-keyring-reader/`).
...
A keyring component must be installed on the server for calls to succeed,
for example:
...
<Warning>
  Keyring access is a preview capability — under active development and
  subject to change before the beta release. To install an extension that
  uses this capability, see [Preview Capabilities](/mysql-8.4/0.0.4-dev/preview-capabilities).
</Warning>
