...
<Warning>
  Custom types use VEF Protocol 3 (stable as of v0.0.4); the current SDK
  declares Protocol 4, which is under development. Extensions built against
  the old Protocol 2 are rejected by the server and must be rebuilt — see
  [Breaking Changes](/mysql-8.4/0.0.4-dev/breaking-changes).
</Warning>
...
Every custom type needs encode, decode, and compare operations, and optionally a hash operation. There are two ways to provide these: **VDF names** (VEF_PROTOCOL_3 or later, recommended) or **function pointers**. For each operation, provide one or the other — not both.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="sliders"
    href="/mysql-8.4/0.0.4-dev/development#parameterized-types"
  >
  ...
  <Card
    ...
    href="/mysql-8.4/0.0.4-dev/managing#replication"
  >
