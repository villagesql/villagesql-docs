...
Custom types replicate via binlog in ROW format. INSERT, UPDATE, DELETE, and
ALTER TABLE operations on custom type columns all replicate correctly.

`INSTALL EXTENSION` is not replicated — each server manages its own extensions.
Install the extension on every replica before replication starts. If a replica
encounters a custom type it doesn't recognize, replication stops with an error.
Install the missing extension, then resume:
