...
call `g_ping.available()`. It returns `true` once the server has populated
the capability.

```cpp
bool PingCapability::available() const noexcept;
```
...
struct Diag {
  ...
  std::string_view message;           // UTF-8, may be empty
};
```
...
| `sqlstate` | 5-character SQLSTATE, NUL-terminated source. |
