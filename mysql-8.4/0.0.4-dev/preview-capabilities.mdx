...
- `var_name` — optional. When non-null, the server registers this exact name
  ...
  pattern `{extension_name}.{suffix}_enabled`.

The control variable is a server-registered system variable scoped to the
extension. Enable the worker with `SET GLOBAL {extension_name}.{suffix}_enabled = ON`;
set it `OFF` to stop it.
...
server registers a `my_ext.heartbeat_enabled` system variable (where `my_ext`
is the extension name). Enable the worker with:
