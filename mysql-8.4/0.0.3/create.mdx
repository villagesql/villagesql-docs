...
Before you begin, build VillageSQL from source — extensions link against the server's SDK headers and build tree. Follow the [Clone and Build from Source](/mysql-8.4/0.0.3/source) guide first.
...
For the full naming convention across SQL, filenames, and repository names, see [Extension Naming Conventions](/mysql-8.4/0.0.3/install#extension-naming-conventions).
...
See the [development guide](/mysql-8.4/0.0.3/development#typed-wrappers-recommended)
...
**Function pointers** (`false` = success, `true` = error — see [Part B](#part-b-encode-and-decode-return-convention)):
...
VDF-name operations use the `bool` return convention (`false` = success, `true` = error), same as function-pointer encode/decode. See [development.mdx — Type Operation Builders](/mysql-8.4/0.0.3/development) for the full C++ signatures.
...
See [vsql_complex Example](/mysql-8.4/0.0.3/examples) for the complete implementation.
