...
<Note>
  If you are contributing to the VillageSQL server itself (not building an extension), see [Clone and Build from Source](/mysql-8.4/0.0.3/source), which covers the full server developer workflow including running tests with `mysql-test-run.pl` directly.
</Note>

## Setting Up Your Environment

To develop and test extensions, you need a built VillageSQL server. Follow the [Clone and Build from Source](/mysql-8.4/0.0.3/source) guide to compile the server binaries.
