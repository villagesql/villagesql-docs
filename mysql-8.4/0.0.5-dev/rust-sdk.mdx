...
<Note>
If you prefer C++, see [Creating Extensions](/mysql-8.4/0.0.5-dev/create) for the C++ SDK walkthrough.
</Note>

VillageSQL extensions can be written in Rust using the `vsql` crate. The SDK handles all FFI marshaling — you work in ordinary Rust types and the `extension!` macro generates the C entry points the server calls at load time.
...
Edit `Cargo.toml` to set the crate type and add the `vsql` dependency:
...
If you prefer a Git dependency over a path dependency, you can use:

```toml
vsql = { git = "https://github.com/villagesql/vsql-rust-sdk" }
```

This pins to the latest commit on the default branch. Use `rev = "..."` to lock to a specific commit.
