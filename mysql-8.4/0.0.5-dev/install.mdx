...
When `VERSION` is supplied, the server compares it against the extension's
manifest version and aborts if they differ. Omit the clause to install
whatever version the `.veb` ships.
...
<Info>
  Your server must have `veb_dir` configured before you can install external extensions. See [Configuring veb_dir](/mysql-8.4/0.0.5-dev/managing#configuring-veb_dir).
</Info>
...
```
ERROR 3219 (HY000): Cannot install extension 'my_extension': manifest
...
| `Cannot install extension 'name': manifest version is 'X' but VERSION 'Y' was specified` | The `VERSION` clause doesn't match the manifest version in the `.veb`. Re-run without `VERSION`, or use the manifest version. |

For more troubleshooting, see [Managing Extensions](/mysql-8.4/0.0.5-dev/managing).
