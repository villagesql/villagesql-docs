...
- `.max_result_length(size_t)` - Size the result column for a STRING return so a materialized result isn't truncated at the argument width. STRING only; call `.returns(STRING)` first. Requires Protocol 4 (dev ABI).
