...
This guide covers the end-to-end steps to build and install an extension. For writing VDF implementations in depth — typed wrappers, aggregates, system variables, parameterized types — see the [Development Guide](/mysql-8.4/0.0.5-dev/development).

<Note>
If you prefer Rust, see [Building extensions in Rust](/mysql-8.4/0.0.5-dev/rust-sdk).
</Note>
...
See the [development guide](/mysql-8.4/0.0.5-dev/development#typed-wrappers)
...
<Note>
Before implementing your functions, review the [Extension API Reference](/mysql-8.4/0.0.5-dev/extension-api-reference) for the complete VDF contracts: null checking, result types, buffer sizing, and error handling.
</Note>
...
See [Creating Custom Types](/mysql-8.4/0.0.5-dev/custom-types) for the full
implementation walkthrough. If your type takes parameters (e.g., `VECTOR(1536)`),
see [Parameterized Types](/mysql-8.4/0.0.5-dev/development#parameterized-types).
