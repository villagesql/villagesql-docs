...
For writing VDF implementations in depth — argument and result types, aggregates, system variables, parameterized types — see the [Development Guide](/mysql-8.4/0.0.5-dev/development).

<Note>
If you prefer Rust, see [Creating Extensions in Rust](/mysql-8.4/0.0.5-dev/rust-sdk).
</Note>
...
Before you begin, build VillageSQL from source — extensions link against the server's SDK headers and build tree. Follow the [Build from Source](/mysql-8.4/0.0.5-dev/source) guide first.
...
```json
{
  "$schema": "https://raw.githubusercontent.com/villagesql/villagesql-docs/main/mysql-8.4/0.0.5-dev/manifest.json.schema.json",
...
For the full naming convention across SQL, filenames, and repository names, see [Extension Naming Conventions](/mysql-8.4/0.0.5-dev/install#extension-naming-conventions).
...
- `.max_result_length(uint64_t)` - Size the result column for a STRING return so a materialized result isn't truncated at the argument width. STRING only; call `.returns(STRING)` first. Requires Protocol 4 (dev ABI).
...
See the [development guide](/mysql-8.4/0.0.5-dev/development#argument-and-result-types)
...
For STRING results, `out.set(sv)` follows a snprintf-style overflow contract: it
...
<Note>
Before implementing your functions, review the [C++ API Reference](/mysql-8.4/0.0.5-dev/extension-api-reference) for the complete VDF contracts: null checking, result types, buffer sizing, and error handling.
</Note>
...
See [Custom Types in C++](/mysql-8.4/0.0.5-dev/custom-types) for the full
implementation walkthrough. If your type takes parameters (e.g., `VECTOR(1536)`),
see [Parameterized Types](/mysql-8.4/0.0.5-dev/type-operations#parameterized-types).
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="code"
    href="/mysql-8.4/0.0.5-dev/development"
  >
  ...
  <Card
    ...
    icon="hammer"
    href="/mysql-8.4/0.0.5-dev/source"
  >
