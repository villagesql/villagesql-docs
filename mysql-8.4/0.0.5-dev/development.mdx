---
title: "C++ Development"
description: "VDF authoring depth for C++ extensions — typed wrappers, aggregates, prerun/postrun, varargs, and registration."
---
...
single header to access the full C++ SDK:
...
Under C++20 it is an alias for `std::span<T>`; under C++17 the C++ SDK provides
...
Raw ABI signatures are rejected at compile time. Use `PrerunResult::set_user_data(void*)` to stash state; use `PostrunArgs::delete_state<T>()` to release it. If prerun calls `set_user_data(new T{})`, postrun **must** call `delete_state<T>()` — the C++ SDK does not auto-free.
