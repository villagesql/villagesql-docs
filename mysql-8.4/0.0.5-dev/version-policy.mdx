...
Versions with a pre-release suffix (e.g., `0.0.5-dev`) are development builds. The server blocks upgrades from a database initialized with a development build by default. Dev builds are not tested for upgrade compatibility and may include breaking schema or protocol changes.
