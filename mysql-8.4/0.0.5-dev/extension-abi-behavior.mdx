...
After a crash and restart, extensions re-register automatically. No manual intervention is needed.

Row data survives through InnoDB's normal crash recovery. The extension's binary format is never special-cased — InnoDB handles a UUID column no differently from a `BINARY(16)` column.

After a SIGKILL and restart, UUID rows survived InnoDB crash recovery intact, and `INFORMATION_SCHEMA.EXTENSION_REGISTRATION` showed the same registration JSON as before the crash.
...
Those 16 raw bytes flow through the binary log. Any tool that reads those binlog events — replication consumers, CDC pipelines, dump utilities — needs the extension installed to decode the values.
...
Replacing the VEB file on disk is not a supported upgrade path — the server validates the VEB at install time and will reject a swapped file. Use `UNINSTALL EXTENSION` / `INSTALL EXTENSION` to update.

In-place extension upgrades without data loss are tracked in [#11](https://github.com/villagesql/villagesql-server/issues/11).
