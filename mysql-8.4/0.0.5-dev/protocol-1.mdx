...
Protocol 1 is the original VEF interface. It was stable as of v0.0.1 and is likely to be deprecated in a future release. New extensions should use the Protocol 3 API — the template-based type builders in the [Creating Extensions](/mysql-8.4/0.0.5-dev/create) and [Development](/mysql-8.4/0.0.5-dev/development) guides.
...
Protocol 1 VDF implementations can pass the raw C structs directly instead of using typed wrappers. This style is supported but [typed wrappers](/mysql-8.4/0.0.5-dev/development#typed-wrappers-recommended) are preferred for new code.
...
For aggregates, the raw ABI exposes `prerun`, `postrun`, `clear`, and `accumulate` with their full C signatures. Prefer the [typed aggregate approach](/mysql-8.4/0.0.5-dev/development#aggregate-vdfs) for new code.
