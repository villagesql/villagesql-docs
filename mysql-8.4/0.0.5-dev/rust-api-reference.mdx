...
This page is a reference for the `vsql` crate API. For the getting started tutorial, see [Building Extensions in Rust](/mysql-8.4/0.0.5-dev/rust-sdk). For custom types, see [Custom Types in Rust](/mysql-8.4/0.0.5-dev/rust-custom-types).
...
| `[param_types]` | Comma-separated list of `vsql::Type::*` or `vsql::custom!("name")` values. Use `[]` for zero-arity functions. |
| `return_type` | `vsql::Type::*` or `vsql::custom!("name")`. |
...
`vsql::custom!("type_name")` references a custom type by name in a `func!` declaration:
...
Use it anywhere a `vsql::Type::*` would appear in a parameter list or return type position. The string must match the `type_name` declared in the corresponding `custom_type!`.
