...
Both names must be identical, using underscores (see [Extension Naming Conventions](/mysql-8.4/0.0.5-dev/install#extension-naming-conventions)):
- VEB filename: `my_extension.veb`
...
The precheck refuses any change that would corrupt existing data — for example
a custom type whose stored length changed between versions:
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="trash"
    href="/mysql-8.4/0.0.5-dev/uninstall"
  >
