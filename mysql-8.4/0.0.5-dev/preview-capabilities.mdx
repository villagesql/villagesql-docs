...
`create` is the only place to install the extension's `MyCtx`. Construct it
through the server's arena so its lifetime is tied to the column:
...
bool MyStorage::create(Column::StorageCtx<MyCtx>* ctx,
                       ...
                       uint32_t col_len,
                       char* error_msg, uint32_t error_msg_len) {
    MyCtx* my = ctx->arena().construct<MyCtx>(/* args */);
    // ctx->user() returns my from this point on.
    return false;  // false = success
}
```

Do not allocate the context with `new` — the server owns the arena and frees
it on column teardown.
