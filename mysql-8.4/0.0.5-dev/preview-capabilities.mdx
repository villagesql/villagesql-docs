...
expose `int64_t` and `double` counters as MySQL status variables. The
...
`make_int` requires an `int64_t *`; `make_double` requires a `double *`.
...
supported: `BOOL` (`bool *`), `INT` (`int64_t *`), and `STR` (`char **`).
...
| `sv::make_int` | `int64_t *` | `def_val`, `min_val`, `max_val` |
