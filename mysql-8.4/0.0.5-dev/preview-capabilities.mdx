...
struct MyCtx { /* per-column state; SDK default-constructs before calling create or load */ };
...
Every function takes `Column::StorageCtx<MyCtx>*`, whose `user()` accessor
returns the extension's per-column state and whose `arena()` provides
server-managed allocation. Every function returns `false` on success and
`true` on error, writing a message into `error_msg` (capacity
`error_msg_len`) so failures surface to the SQL client.

```cpp
// CREATE TABLE / ALTER TABLE ADD COLUMN.
// col_len is the type's persisted length. Allocate per-column state in the
// server-provided arena and reserve segments here so root references
// survive restart.
bool create(Column::StorageCtx<MyCtx>*, Space::Ref, Segment::TrxRef,
...
bool drop(Column::StorageCtx<MyCtx>*, Segment::TrxRef,
          char* error_msg, uint32_t error_msg_len);

// Called when the server reattaches to existing storage (e.g. after restart).
// Use Column::StorageRef to recover root references previously persisted in
// the StorageCtx.
bool load(Column::StorageCtx<MyCtx>*, Column::StorageRef,
          char* error_msg, uint32_t error_msg_len);
...
bool insert(Column::StorageCtx<MyCtx>*, MtrCtx::Ref, Segment::TrxRef,
...
bool select(Column::StorageCtx<MyCtx>*, MtrCtx::Ref, Column::Ref,
            Column::Data* col_data, Column::Data* rowid_prefix,
            Segment::TrxRef*, bool* delete_marked,
            char* error_msg, uint32_t error_msg_len);

// DELETE (in-transaction). Set or clear the delete-mark flag. The actual
// bytes must remain readable until purge() runs.
bool mark_delete(Column::StorageCtx<MyCtx>*, MtrCtx::Ref, Segment::TrxRef,
                 Column::Ref, bool delete_mark,
                 char* error_msg, uint32_t error_msg_len);

// InnoDB purge. Reclaim storage for entries whose deleting transaction is
// no longer visible to any active snapshot.
bool purge(Column::StorageCtx<MyCtx>*, MtrCtx::Ref, Segment::TrxRef,
           Column::Ref, char* error_msg, uint32_t error_msg_len);
```
...
```cpp
bool MyStorage::create(Column::StorageCtx<MyCtx>* ctx,
                       Space::Ref space, Segment::TrxRef trx,
                       uint32_t col_len,
                       char* error_msg, uint32_t error_msg_len) {
  Segment::PageRef root;
  if (Segment::create(space, 1, trx, root) != Error::SUCCESS) {
    snprintf(error_msg, error_msg_len, "%s", last_error().data());
    return true;
  }

  // Storage reference is persisted across restarts; load() receives this
  // value as storage_ref.
  ctx->set_ref(static_cast<Column::StorageRef>(root));
  ctx->user()->col_len = col_len;
  return false;
}
```

`load` follows the same pattern — `ctx->user()` is pre-populated and
`storage_ref` carries the value stored by `ctx->set_ref()` in `create`:

```cpp
bool MyStorage::load(Column::StorageCtx<MyCtx>* ctx,
                     Column::StorageRef storage_ref,
                     char* error_msg, uint32_t error_msg_len) {
  ctx->user()->root = static_cast<Segment::PageRef>(storage_ref);
  return false;
}
```
...
**Segments** are reserved at `create` time. Persist the root page reference
into the `StorageCtx` so `load` can recover it after restart:

```cpp
Segment::PageRef root;
Segment::create(space, /* num_segments */ 1, trx, root);
ctx->set_ref(static_cast<Column::StorageRef>(root));
```
...
// Read
page.load(space, page_num, Page::Latch::SHARED, mtr_ref);
uint32_t v = page.read_integer_4(Page::HEADER_SIZE + offset);

// Write
page.load(space, page_num, Page::Latch::EXCLUSIVE, mtr_ref);
page.write_integer_4(Page::HEADER_SIZE + offset, v, mtr_ref);
```
...
|---|---|---|
| `Page::HEADER_SIZE` | `38` | Extension data begins at this offset. |
| `Page::TRAILER_SIZE` | `8` | Do not write past `page_size - TRAILER_SIZE`. |
| `Page::get_size(space)` | runtime | Use instead of hard-coding 16384. |
