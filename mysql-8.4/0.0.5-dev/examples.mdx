...
The C++ SDK provides typed argument and result wrappers, so implementations work with C++ types instead of raw protocol structs:
...
| **NULL handling** | Call `in.is_null()` on the typed argument wrapper |
| **Error handling** | Call `out.error("message")` on the result wrapper |
| **Result output** | Call `out.set(value)` / `out.set_length(n)` on the result wrapper |
