...
**Memory overhead:** each committed entry is a `std::map<std::string, std::shared_ptr<EntryType>>` node plus the descriptor payload — no fixed per-entry figure is tracked in source.
...
**Installation security:** `INSTALL EXTENSION` and `UNINSTALL EXTENSION` require the `EXTENSION_ADMIN` privilege (or `SUPER`), enforced by the ordinary MySQL grant system.
...
<CardGroup cols={2}>
  ...
  <Card
    title="Examples"
    icon="lightbulb"
    href="/mysql-8.4/stable/examples"
  >
