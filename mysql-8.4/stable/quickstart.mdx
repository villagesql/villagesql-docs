...
and which version. Choose **MySQL 8.4** at the first question to get the server
...
`VSQL_CODEBASE` takes `mysql-8.4`, `percona-8.4`, or `mysql-9.7`.
...
For development or custom builds, follow the [Clone and Build from Source Guide](/mysql-8.4/stable/source) to compile from the latest code.

VillageSQL also builds on Percona Server as an alternative to upstream MySQL —
see the [Percona Build](/mysql-8.4/stable/percona) page for what that involves.
...
- `vsql_complex` and `vsql_simple`, reference extensions used elsewhere in these docs (see [C++ Extension Examples](/mysql-8.4/stable/examples)) to show how the extension framework works.

If you built VillageSQL from source (Option C), `vsql_complex` and `vsql_simple` are already in `veb_dir` too — `make install` builds them unconditionally. `vsql_uuid` and the other extensions are not: they live in separate repositories and need to be built and installed on their own. If you used Option C, clone and build [vsql-uuid](https://github.com/villagesql/vsql-uuid) before continuing, or skip ahead using `vsql_complex` in its place — see [Installing Extensions](/mysql-8.4/stable/install).
...
For more details, see [Installing Extensions](/mysql-8.4/stable/install).
...
-- Insert rows with generated v7 UUIDs. Each UUID_V7() embeds a millisecond
-- timestamp, so rows inserted in the same statement can land in the same
-- millisecond and tie-break on their random bits instead of insertion
-- order — insert separately, with a gap, to see the ordering clearly.
INSERT INTO events VALUES (UUID_V7(), 'signup');
SELECT SLEEP(0.01);
INSERT INTO events VALUES (UUID_V7(), 'login');
SELECT SLEEP(0.01);
INSERT INTO events VALUES (UUID_V7(), 'purchase');
...
<CardGroup cols={2}>
  <Card title="Managing Extensions" icon="puzzle-piece" href="/mysql-8.4/stable/managing">
    Learn how to install and manage other extensions.
  </Card>
  <Card title="Create an Extension" icon="code" href="/mysql-8.4/stable/create">
