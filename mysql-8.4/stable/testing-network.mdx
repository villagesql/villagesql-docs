...
`vsql-http/mysql-test/t/vsql_http_requests.test` demonstrates the same goal — a background helper with a clean, race-free teardown — through a different concrete mechanism: a single Python file that re-execs itself with a `--serve` flag to become the detached child, rather than the separate `launcher.py`/`echo_server.py` split shown above. It writes no PID file; the child writes its own `stop_echo.sh` script (`kill <pid>`) for the test to run at cleanup.

## See also

- [C++ Testing](/mysql-8.4/stable/testing) — MTR setup, running suites, recording results, and debugging failures
- [Creating Extensions in C++](/mysql-8.4/stable/create) — end-to-end build steps and CMake setup
- [C++ Development](/mysql-8.4/stable/development) — VDF authoring, argument types, and result handling
