...
title: "Creating Extensions in C++"
description: "Learn how to create custom VillageSQL extensions in C++ using the extension template and VEF."
---
...
For writing VDF implementations in depth — argument and result types, aggregates, system variables, parameterized types — see the [Development Guide](/mysql-8.4/stable/development).

<Note>
If you prefer Rust, see [Creating Extensions in Rust](/mysql-8.4/stable/rust-sdk).
</Note>
...
Before you begin, build VillageSQL from source — extensions link against the server's SDK headers and build tree. Follow the [Build from Source](/mysql-8.4/stable/source) guide first.
...
For the full naming convention across SQL, filenames, and repository names, see [Extension Naming Conventions](/mysql-8.4/stable/install#extension-naming-conventions).
...
See the [development guide](/mysql-8.4/stable/development#argument-and-result-types)
...
When you build the value yourself with `buffer()` and `set_length()`, the buffer
...
For STRING results, `out.set(sv)` truncates silently: it copies as many bytes as
fit in `out.buffer()` and reports that truncated length. It does not report the
value's full size, and the server does not grow the buffer and re-invoke the
function. To avoid truncation, size the buffer correctly with `.buffer_size()`,
or write into `out.buffer()` and call `out.set_length()` yourself as shown above.
...
<Note>
Before implementing your functions, review the [C++ API Reference](/mysql-8.4/stable/extension-api-reference) for the complete VDF contracts: null checking, result types, buffer sizing, and error handling.
</Note>
...
See [Custom Types in C++](/mysql-8.4/stable/custom-types) for the full
implementation walkthrough. If your type takes parameters (e.g., `VECTOR(1536)`),
see [Parameterized Types](/mysql-8.4/stable/type-operations#parameterized-types).
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="code"
    href="/mysql-8.4/stable/development"
  >
  ...
  <Card
    ...
    icon="sitemap"
    href="/mysql-8.4/stable/architecture"
  >
