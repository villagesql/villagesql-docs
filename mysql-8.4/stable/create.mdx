...
Before you begin, you need the VillageSQL Extension SDK, and there are two ways to point CMake at one. If you have a VillageSQL build tree, pass `-DVillageSQL_BUILD_DIR=/path/to/villagesql/build` — the [Build from Source](/mysql-8.4/stable/source) guide produces that tree. If you have an unpacked SDK directory — one containing `include/villagesql/vsql.h` — that is enough on its own: pass `-DVillageSQL_SDK_DIR=/path/to/sdk`. The [extension template README](https://github.com/villagesql/vsql-extension-template) covers how to get an SDK without building the server. Only the build-tree form sets a VEB install directory, so on the SDK-only path copy the built VEB file yourself into the directory `SHOW VARIABLES LIKE 'veb_dir'` reports.
