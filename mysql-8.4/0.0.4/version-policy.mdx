...
**Protocol 2 extensions must be rebuilt.** VEF Protocol 3 is stable as of v0.0.4, and the server now rejects Protocol 2 extensions at load time. Rebuild against the Protocol 3 API using the typed wrappers (`StringArg`, `IntArg`, `StringResult`, etc.).
...
populated by the SDK from your `manifest.json` version field. If yours
