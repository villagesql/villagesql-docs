...
<Warning>
  Rust SDK 处于 alpha 阶段——各版本之间可能出现破坏性的 API 变更。支持纯函数扩展和自定义类型（encode、decode、compare、hash）。聚合、`prerun()`、`VarArgs`、系统变量和状态变量、keyring 访问以及列存储 ABI 目前仅在 C++ SDK 中提供——如果您需要其中任何一项，请使用 [C++ SDK](/zh/mysql-8.4/0.0.5/create)。
</Warning>

本页是 `villagesql` crate API 的参考。有关入门教程，请参阅 [在 Rust 中构建扩展](/zh/mysql-8.4/0.0.5/rust-sdk)。有关自定义类型，请参阅 [Rust 中的自定义类型](/zh/mysql-8.4/0.0.5/rust-custom-types)。
