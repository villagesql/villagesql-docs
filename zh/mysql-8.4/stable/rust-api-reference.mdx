...
<Warning>
  Rust SDK 处于 alpha 阶段——各版本之间可能出现破坏性的 API 变更。支持纯函数扩展、聚合函数、变长参数函数以及自定义类型（encode、decode、compare、hash），同时也支持 `sys_var`、`status_var`、`thread_worker` 和 `keyring` 预览功能。列存储 ABI 目前仅在 C++ SDK 中提供——如果您需要它，请使用 [C++ SDK](/zh/mysql-8.4/stable/create)。
</Warning>

本页是 `villagesql` crate API 的参考。有关入门教程，请参阅 [使用 Rust 构建扩展](/zh/mysql-8.4/stable/rust-sdk)。有关自定义类型，请参阅 [Rust 中的自定义类型](/zh/mysql-8.4/stable/rust-custom-types)。
...
| `CustomWithParams { bytes, params }` | 原始字节 + `TypeParams` | 通过 [`parameterized_type!`](/zh/mysql-8.4/stable/rust-custom-types#parameterized-types) 注册的参数化自定义类型 |
...
`requires:` 声明扩展所使用的 [Rust 中的预览功能](/zh/mysql-8.4/stable/rust-preview-capabilities)，以对 `static` 功能对象的引用的形式给出。它必须放在最后，位于 `funcs:` 部分之后——如果扩展不注册任何函数，请包含 `funcs: []`。
...
您不能自行释放该状态：`func!` 会生成在语句结束时丢弃它的 postrun。这与 C++ SDK 相反，在 C++ SDK 中，您的 postrun 必须调用 `delete_state<T>()`——请参阅[每个语句的状态](/zh/mysql-8.4/stable/development#per-statement-state-prerun-and-postrun)。
...
按照[使用 Rust 构建扩展](/zh/mysql-8.4/stable/rust-sdk)中的说明构建并安装它，然后：
...
<Warning>
  对于变长参数 VDF，服务器不执行任何参数数量和参数类型的验证。没有已声明的参数列表可用来检查调用，因此每次调用都会把 SQL 文本传入的任何内容原样送达您的函数，包括零个参数和您从未预期的类型。验证完全是 `prerun` 钩子的职责。变长参数注册还需要 VEF Protocol 3——较旧的服务器会在安装时拒绝该扩展。这与 C++ SDK 一致，在 C++ SDK 中，[框架同样无法为变长参数 VDF 验证参数数量或类型](/zh/mysql-8.4/stable/development#varargs-vdfs)。
</Warning>
