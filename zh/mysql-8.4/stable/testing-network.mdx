...
`vsql-http/mysql-test/t/vsql_http_requests.test` 展示了同样的目标——一个具有干净、无竞态拆除过程的后台辅助进程——但采用了不同的具体机制：使用单个 Python 文件，它通过 `--serve` 标志重新执行自身以成为分离的子进程，而不是上文所示的 `launcher.py`/`echo_server.py` 拆分方式。它不写入 PID 文件；子进程会自行写出 `stop_echo.sh` 脚本（`kill <pid>`），供测试在清理时运行。

## 另请参阅

- [C++ 测试](/zh/mysql-8.4/stable/testing) — MTR 设置、运行测试套件、记录结果以及调试故障
- [使用 C++ 创建扩展](/zh/mysql-8.4/stable/create) — 端到端的构建步骤与 CMake 设置
- [C++ 开发](/zh/mysql-8.4/stable/development) — VDF 编写、参数类型与结果处理
