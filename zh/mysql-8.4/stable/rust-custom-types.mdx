...
<Warning>
  ...
  仅含函数的扩展和自定义类型（编码、解码、比较、哈希），同时也支持
  `sys_var`、`status_var`、`thread_worker` 和 `keyring` 预览功能。聚合、
  `VarArgs` 以及列存储 ABI 目前仅在 C++
  中可用——如果您需要其中任何功能，请使用 [C++ SDK](/zh/mysql-8.4/stable/create)。
</Warning>
