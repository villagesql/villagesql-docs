...
**错误：** `ERROR 3219 (HY000): VEB file not found: my_extension.veb`
...
### 扩展加载错误 {#extension-load-errors}

**错误：** `Failed to load VEF extension 'extension_name'`

**原因：** `.veb` 文件在入口点阶段加载失败——例如缺少或无效的
`vef_register` 符号，或者 `.veb` 文件已损坏。
...
两个名称必须完全相同，使用下划线（请参阅[扩展命名约定](/zh/mysql-8.4/stable/install#extension-naming-conventions)）：
...
<Note>
  `ALTER EXTENSION` 需要 `EXTENSION_ADMIN` 权限（或以 `SUPER` 作为
  替代），与 `INSTALL EXTENSION` 和 `UNINSTALL EXTENSION` 的限制条件相同。
  请参阅 [所需权限](/zh/mysql-8.4/stable/install#required-privilege)。
</Note>
...
对于自定义类型，推荐使用 ROW 格式的 binlog。对自定义类型列执行的 INSERT、UPDATE、DELETE 和 ALTER TABLE 操作在 ROW 格式中都能正确复制。
...
<CardGroup cols={2}>
  ...
  <Card
    title="卸载扩展"
    icon="trash"
    href="/zh/mysql-8.4/stable/uninstall"
  >
    安全地删除扩展
  </Card>
  <Card
