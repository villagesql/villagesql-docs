...
title: "C++ 中的自定义类型"
description: "在 C++ 中为 VillageSQL 扩展定义新的列类型——类型操作、ALTER TABLE 规则、转换函数，以及一个完整的 COMPLEX 类型示例。"
---

<Warning>
  自定义类型使用 VEF 协议 3，该协议自 v0.0.4 版本起已稳定。协议 4 正在
  开发中，并且仅通过选择加入的开发 ABI 头文件提供
...
页是 [在 C++ 中创建扩展](/zh/mysql-8.4/stable/create)
...
`vsql::Span<unsigned char>`。使用 C++20 或更高版本，`vsql::Span<T>` 是 `std::span<T>` 的别名；在 C++17 下，C++ SDK 提供了一个最小的源代码兼容回退，具有相同的 `data()`、`size()`、`empty()`、索引和迭代器接口。它通过 `#include <villagesql/vsql.h>` 提供。
...
有关完整的 C++ 签名，请参阅 [类型操作](/zh/mysql-8.4/stable/type-operations)。
...
| 从 | 到 | 结果 |
|------|----|--------|
| 非自定义 | 自定义 | 错误：`Cannot convert column 'col' to custom type 'MYTYPE'` |
| 自定义 | 字符串类型 | 允许 |
| 自定义 | 非字符串类型 | 错误：`Cannot convert custom type column 'col' to non-string type` |
| 自定义 | 不同的自定义类型 | 如果不兼容，则出错：`Cannot convert between incompatible custom types 'A' and 'B'` |
...
**何时需要显式转换。** 在直接进行列赋值时，VillageSQL 会隐式将字符串字面量转换为自定义类型，因此
...
解码使用 `std::to_chars` 而不是 `snprintf`：诸如 `"%.6f"` 这样的固定精度格式会丢失 `double` 所需的数位，因此 `0.1234567891` 读回时变为 `0.123457`。编码会在调用 `strtod` 之前复制输入，因为 `std::string_view` 不带终止空字符。`std::from_chars` 的浮点重载可以避免该复制，但 libc++ 将它们标记为仅从 macOS 26 起可用，因此使用它们的扩展在为更早的目标平台构建时会失败。
...
有关完整的实现，请参阅 [vsql_complex 示例](/zh/mysql-8.4/stable/examples)。

## 函数索引中的 VDF

VDF 可用于函数索引表达式。与生成列相同的 `.deterministic()` 要求也适用于此处，因为 MySQL 将函数索引实现为隐藏的生成列。
...
<CardGroup cols={2}>
  <Card
    title="继续：构建您的扩展"
    icon="hammer"
    href="/zh/mysql-8.4/stable/create#step-5-update-build-configuration"
  >
    返回教程，以构建和安装您的扩展。
  </Card>
  <Card
    title="参数化类型"
    icon="sliders"
    href="/zh/mysql-8.4/stable/type-operations#parameterized-types"
  >
  ...
  <Card
    title="扩展 API 参考"
    icon="book"
    href="/zh/mysql-8.4/stable/extension-api-reference"
  >
  ...
  <Card
    ...
    href="/zh/mysql-8.4/stable/managing#replication"
  >
