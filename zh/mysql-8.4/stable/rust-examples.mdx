...
本页介绍 Rust SDK 仓库中的四个参考扩展——一个仅包含函数的最小示例，一个包含算术运算、排序和哈希的完整自定义类型，一个聚合函数，以及一个变长参数函数。该仓库的 `examples/` 目录还为每一项受支持的[预览功能](/zh/mysql-8.4/stable/rust-preview-capabilities)提供了一个示例。
