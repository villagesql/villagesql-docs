...
本页介绍 Rust SDK 仓库中的两个参考扩展——一个仅包含函数的最小示例，以及一个包含算术运算、排序和哈希的完整自定义类型。该仓库的 `examples/` 目录还为每一项受支持的[预览功能](/zh/mysql-8.4/stable/rust-preview-capabilities)提供了一个示例。
...
（注意：manifest 中的描述文本已过时——该函数注册为 `rot13`，而不是 `vsql_rot13`；上游修复另行跟踪）
