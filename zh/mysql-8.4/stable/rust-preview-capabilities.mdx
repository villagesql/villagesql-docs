...
title: "Rust 中的预览功能"
description: "从 Rust SDK 使用 VillageSQL 预览功能——通过 extension! 中的 requires: 列表声明状态变量、系统变量、后台线程工作器、从工作线程发起的 SQL 查询和密钥环访问。"
---
...
预览功能是服务器提供的功能，在最终确定其 API 之前，会将其暴露给扩展。Rust SDK 封装了其中五项：状态变量、系统变量、后台线程工作器、从工作线程发起的 SQL 查询和密钥环访问。本页介绍如何从 Rust 声明和使用其中的每一项。有关该概念、预览层以及仅在 C++ 中存在的功能，请参阅[预览功能](/zh/mysql-8.4/stable/preview-capabilities)。
...
有关详细信息，包括为什么此变量会拒绝 `SET GLOBAL`，请参阅[启用预览层](/zh/mysql-8.4/stable/preview-capabilities#启用预览层)。

您还需要一套可用的 Rust 扩展设置——[使用 Rust 构建扩展](/zh/mysql-8.4/stable/rust-sdk)演练涵盖了工具链、`cargo-vsql` 以及您的第一个 `extension!` 块。
...
其余的预览功能——`auth`、`mysql_services`、`statement_event` 以及列存储 ABI——目前仅支持 C++。如果您需要其中之一，请使用 [C++ SDK](/zh/mysql-8.4/stable/preview-capabilities)。
...
`ThreadHandle` 参数以引用方式传入；将它传给 `villagesql::preview::sql_query` 中的 `SqlQueryCapability::open`，即可在工作器内部执行 SQL。
...
<CardGroup cols={2}>
  <Card
    title="预览功能 (C++)"
    icon="flask"
    href="/zh/mysql-8.4/stable/preview-capabilities"
  >
    完整的功能索引、预览层，以及仅支持 C++ 的功能：auth、mysql_services、statement_event 和列存储。
  </Card>
  <Card
    title="Rust API 参考"
    icon="book"
