...
noindex: true
title: "预览功能"
description: "预览功能使扩展程序能够访问仍在稳定中的服务器功能。本页涵盖启用预览层、密钥环、status_var、sys_var、thread_worker 和 sql_query 功能，以及注册模式。"
---
...
功能对象必须具有静态存储持续时间。当用户设置变量时，MySQL 会直接写入存储指针。
...
线程工作器功能 (`vsql::preview::thread_worker`) 允许扩展在服务器驱动的后台线程中运行。线程通过服务器注册的控制系统变量在扩展加载时启动和停止；服务器在定期计时器上、在文件描述符准备就绪时，或响应启用/禁用事件时调用扩展的工作函数。
...
`column_str()` 返回一个 `string_view`，它仅在下一次 `next()` 调用或 `Result` 销毁时有效。如果需要更长的生命周期，请复制它。`string_view` 具有 `data() == nullptr` 表示 SQL NULL。
...
static constexpr auto kMyStorage =
    make_column_store</Warning>(MY_TYPE)
...
SDK 在调用 `create` 或 `load` 之前，默认构造 `MyCtx`——在进入您的函数时，`ctx->user()` 已经填充。`MyCtx` 必须是默认可构造的；SDK 使用不带参数的 `T()` 调用。
...
| `storage::Page::get_size(space)` | 运行时 | 而是使用硬编码的 16384。 |
