...
理解 VillageSQL 的扩展架构有助于在构建扩展时调试问题和优化性能。
...
**性能：** 对于热点代码路径，使用限定调用（`extension_name.function_name()`）跳过解析链，直接调用扩展函数。

---

## VEB 文件格式

VillageSQL 扩展以 `.veb`（VillageSQL 扩展包）文件分发 - 包含以下内容的 tar 归档文件：
...
**符号隔离：** 扩展以 `RTLD_LOCAL` 标志加载，确保一个扩展的符号不会与其他扩展的符号冲突。这防止了多个扩展使用常见库名称或函数名称时的命名冲突。

<Note>
`veb_dir` 系统变量指向 `.veb` 扩展文件存储的目录。
</Note>
...
VillageSQL 将 `.veb` 文件展开到 MySQL 数据目录中以支持多版本：
