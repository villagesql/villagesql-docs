---
title: 扩展示例
description: 使用 VEF SDK 学习 vsql_complex 参考实现
---

`vsql_complex` 扩展是 VillageSQL 使用 VEF SDK 的自定义类型的参考实现，展示了生产就绪扩展模式。
...
vsql_complex 使用 VEF SDK，并使用 `VEF_GENERATE_ENTRY_POINTS()`：
...
- 聚合注册使用 `make_aggregate_func<State, &result_fn>("name")`，并使用 `.clear<&fn>()` 和 `.accumulate<&fn>()`；结果函数具有签名 `void(const State&, ResultWrapper)`
...
VEF SDK 提供了类型化的参数和结果包装器，因此实现可以使用 C++ 类型而不是原始协议结构：
...
| **NULL 处理** | 在类型化的参数包装器上调用 `in.is_null()` |
...
VillageSQL 支持使用自定义类型进行 `SELECT INTO OUTFILE` 和 `LOAD DATA INFILE` 操作，允许您导出和导入数据，同时保留自定义类型的值。
