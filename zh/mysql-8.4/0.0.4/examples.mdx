---
title: 扩展示例
description: 从 vsql_complex 参考实现学习，使用 VEF SDK
---

`vsql_complex` 扩展是 VillageSQL 使用 VEF SDK 实现自定义类型的参考实现——展示了生产就绪的扩展模式。

**源码位置：** `villagesql/examples/vsql-complex/` 在 [VillageSQL 仓库](https://github.com/villagesql/villagesql-server/tree/main/villagesql/examples/vsql-complex) 中

---

## vsql_complex 提供的功能

COMPLEX 类型用于复数（a + bi），支持算术运算、工具函数和聚合操作。
...
vsql_complex 使用 VEF SDK 配合 `VEF_GENERATE_ENTRY_POINTS()`：
...
**关键模式：**
- 单个宏调用注册所有内容——无需手动 SQL
- 类型对象是 `constexpr` 变量，通过 `vsql::make_type<kName>()` 构建，其中 `kName` 是用作非类型模板参数的 `static constexpr const char[]`
- 类型操作（`.from_string<>()`, `.to_string<>()`, `.compare<>()`, `.hash<>()`）嵌入类型对象中——无需单独的 `.func()` 调用
- `.compare()` 支持 ORDER BY 和索引；`.hash()` 为可选
- `.intrinsic_default_str("(0,0)")` 设置当 INSERT IGNORE 或 UPDATE IGNORE 将 NULL 赋值给此类型的 NOT NULL 列时的默认值
- 函数注册使用 `make_func<&impl>("name")` 配合 `.build()`
- 聚合注册使用 `make_aggregate_func<State, &result_fn>("name")` 配合 `.clear<&fn>()` 和 `.accumulate<&fn>()`；结果函数签名需为 `void(const State&, ResultWrapper)`

---

## 二进制存储格式

COMPLEX 存储 **16 字节**（小端序）：
- 字节 0-7：实部（double）
- 字节 8-15：虚部（double）
...
使用平台无关的字节序函数确保跨平台兼容性。

---

## 包装函数

VEF SDK 提供类型化参数和结果包装器，使实现可直接使用 C++ 类型而非原始协议结构体：
...
| **平台无关** | 对 double 使用自定义字节序函数 |
| **NULL 处理** | 在类型化参数包装器上调用 `in.is_null()` |
| **错误处理** | 在结果包装器上调用 `out.error("message")` |
| **结果输出** | 在结果包装器上调用 `out.set(value)` / `out.set_length(n)` |
...
VillageSQL 支持 `SELECT INTO OUTFILE` 和 `LOAD DATA INFILE` 用于自定义类型，允许在导出和导入数据时保留自定义类型值。
...
<Note>
  自定义类型以字符串表示格式导出。目前不支持使用 `SELECT INTO DUMPFILE` 进行二进制导出。
</Note>
