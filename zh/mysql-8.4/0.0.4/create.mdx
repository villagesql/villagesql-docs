...
本指南涵盖构建和安装扩展的端到端步骤。如需深入了解编写 VDF 实现（类型包装器、聚合、系统变量、参数化类型)，请参阅 [开发指南](/zh/mysql-8.4/0.0.4/development)。


<Note>
如果您更偏好 Rust，请参阅 [使用 Rust 构建扩展](/zh/mysql-8.4/0.0.4/rust-sdk)。
</Note>
...
在开始之前，请先从源代码构建 VillageSQL——扩展需要链接服务器 SDK 的头文件和构建树。请先遵循 [从源代码构建](/zh/mysql-8.4/0.0.4/source) 指南。
...
有关 SQL、文件名和仓库名称的完整命名约定，请参阅 [扩展命名约定](/zh/mysql-8.4/0.0.4/install#extension-naming-conventions)。
...
有关完整的 `CustomArg`/`CustomResult` API（包括用于参数化类型的 `CustomArgWith<P>` 和 `CustomResultWith<P>`)，请参阅 [开发指南](/zh/mysql-8.4/0.0.4/development#typed-wrappers-recommended)。
...
<Note>
在实现函数之前，请查阅 [扩展 API 参考](/zh/mysql-8.4/0.0.4/extension-api-reference) 以了解完整的 VDF 契约：空值检查、结果类型、缓冲区大小和错误处理。
</Note>
...
请参阅 [创建自定义类型](/zh/mysql-8.4/0.0.4/custom-types) 获取完整的实现指南。如果您的类型需要参数（例如 VECTOR(1536))，
请参阅 [参数化类型](/zh/mysql-8.4/0.0.4/development#parameterized-types)。
