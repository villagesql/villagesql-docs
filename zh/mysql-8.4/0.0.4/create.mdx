---
title: "创建扩展"
description: "了解如何使用扩展模板和 VEF 创建自定义 VillageSQL 扩展。"
---
...
本指南涵盖构建和安装扩展的端到端步骤。如需深入了解编写 VDF 实现（类型包装器、聚合、系统变量、参数化类型)，请参阅 [开发指南](/mysql-8.4/0.0.4/development)。


<Note>
如果您更偏好 Rust，请参阅 [使用 Rust 构建扩展](/mysql-8.4/0.0.4/rust-sdk)。
</Note>
...
在开始之前，请先从源代码构建 VillageSQL——扩展需要链接服务器 SDK 的头文件和构建树。请先遵循 [从源代码构建](/mysql-8.4/0.0.4/source) 指南。
...
有关完整的 `CustomArg`/`CustomResult` API（包括用于参数化类型的 `CustomArgWith<P>` 和 `CustomResultWith<P>`)，请参阅 [开发指南](/mysql-8.4/0.0.4/development#typed-wrappers-recommended)。
...
优化器可能会利用此信息，在每条语句中仅评估一次该函数，并在行之间重用该值，而不是逐行调用。因此，错误地将非确定性函数标记为确定性函数可能导致服务器对应该产生不同输出的输入返回相同的结果。仅当您的函数确实不依赖外部状态、随机性或时间时，才添加 `.deterministic()`。有关为何及何时需要确定性的更多详细信息，请参阅 [MySQL UDF 特性文档](https://dev.mysql.com/doc/refman/8.4/en/udf-characteristics.html)。
...
<Note>
请根据函数的最大输出大小，通过 .buffer_size() 请求足够的缓冲区大小。
</Note>

<Note>
在实现函数之前，请查阅 [扩展 API 参考](/mysql-8.4/0.0.4/extension-api-reference) 以了解完整的 VDF 契约：空值检查、结果类型、缓冲区大小和错误处理。
</Note>
...
请参阅 [创建自定义类型](/mysql-8.4/0.0.4/custom-types) 获取完整的实现指南。如果您的类型需要参数（例如 VECTOR(1536))，
请参阅 [参数化类型](/mysql-8.4/0.0.4/development#parameterized-types)。
...
<CardGroup cols={2}>
  ...
  <Card
    title="开发指南"
    icon="code"
    href="/zh/mysql-8.4/0.0.4/development"
  >
    类型包装器、聚合函数、系统变量和测试
  </Card>
  <Card
  ...
  <Card
    title="从源码构建"
    icon="hammer"
    href="/zh/mysql-8.4/0.0.4/source"
  >
    从源代码构建 VillageSQL
  </Card>
</CardGroup>
