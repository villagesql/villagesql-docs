...
协议 1 是原始 VEF 接口。自 v0.0.1 版本起稳定，将在 0.0.5 版本后弃用。新的扩展应使用协议 3 API——基于模板的类型构建器，请参阅 [创建扩展](/mysql-8.4/0.0.4/create) 和 [开发](/mysql-8.4/0.0.4/development) 指南。
...
协议 1 VDF 实现可以直接传递原始 C 结构体，而不是使用类型化包装器。这种样式将在 0.0.5 版本后弃用——对于所有新代码，请使用 [类型化包装器](/mysql-8.4/0.0.4/development#typed-wrappers-recommended)。
...
对于聚合，`.clear<>()` 和 `.accumulate<>()` 接受原始 ABI 函数指针签名。`.prerun<>()` 和 `.postrun<>()` 不接受——构建器需要类型化签名（`void(PrerunArgs, PrerunResult)` 和 `void(PostrunArgs)`)，即使对于协议 1 扩展也是如此。对于所有新代码，请使用 [类型化聚合方法](/mysql-8.4/0.0.4/development#aggregate-vdfs)。
