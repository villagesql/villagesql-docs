...
<Warning>
  Rust SDK 处于 alpha 阶段——各版本之间可能出现破坏性的 API 变更。仅函数扩展和自定义类型（encode、decode、compare、hash）受支持。聚合、`prerun()`、`VarArgs`、系统变量和状态变量、keyring 访问以及列存储 ABI 目前仅支持 C++——如果您需要其中任何一项，请使用 [C++ SDK](/zh/mysql-8.4/0.0.5/create)。
</Warning>
...
`func!` 声明一个可从 SQL 调用的函数。有四种形式（无参数、仅 `buffer_size`、仅 `deterministic`、两者都有）：
