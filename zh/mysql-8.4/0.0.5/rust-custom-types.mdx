...
<Warning>
  ...
  中可用——如果您需要其中任何功能，请使用 [C++ SDK](/zh/mysql-8.4/0.0.5/create)。
</Warning>

自定义类型允许您定义新的列类型——例如 `RATIONAL`、`VECTOR` 或 `INET`——这些类型可以与 `ORDER BY`、索引和聚合函数一起使用。Rust SDK 通过 `custom_type!` 宏支持此功能。

本页假设您已经完成了 [在 Rust 中创建扩展](/zh/mysql-8.4/0.0.5/rust-sdk) 的相关步骤。设置（Cargo.toml、manifest.json、cargo-vsql）与之前相同。
...
<CardGroup cols={2}>
  ...
  <Card
    title="C++ 自定义类型"
    icon="shapes"
    href="/zh/mysql-8.4/0.0.5/custom-types"
  >
