...
如需深入了解编写 VDF 实现——参数和结果类型、聚合、系统变量、参数化类型——请参阅 [开发指南](/zh/mysql-9.7/dev/development)。

<Note>
如果您更偏好 Rust，请参阅 [使用 Rust 创建扩展](/zh/mysql-9.7/dev/rust-sdk)。
</Note>
...
有关 SQL、文件名和仓库名称的完整命名约定，请参阅 [扩展命名约定](/zh/mysql-9.7/dev/install#extension-naming-conventions)。
...
有关完整的 `CustomArg`/`CustomResult` API（包括用于参数化类型的 `CustomArgWith<P>` 和 `CustomResultWith<P>`），请参阅 [开发指南](/zh/mysql-9.7/dev/development#argument-and-result-types)。
...
<Note>
在实现函数之前，请查阅 [C++ API 参考](/zh/mysql-9.7/dev/extension-api-reference) 以了解完整的 VDF 契约：空值检查、结果类型、缓冲区大小和错误处理。
</Note>
...
请参阅 [C++ 中的自定义类型](/zh/mysql-9.7/dev/custom-types) 获取完整的实现指南。如果您的类型需要参数（例如 `VECTOR(1536)`），
请参阅 [参数化类型](/zh/mysql-9.7/dev/type-operations#parameterized-types)。
...
<CardGroup cols={2}>
  ...
  <Card
    title="开发指南"
    icon="code"
    href="/zh/mysql-9.7/dev/development"
  >
  ...
  <Card
    title="扩展架构"
    icon="sitemap"
    href="/zh/mysql-9.7/dev/architecture"
  >
    生命周期、缓存、性能和安全模型
  </Card>
  <Card
