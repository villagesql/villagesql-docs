...
本指南是编写 C++ VDF 实现的深度参考。它是 [使用 C++ 创建扩展](/zh/mysql-9.7/dev/create) 的补充，后者涵盖端到端的构建步骤；也是 [C++ 测试](/zh/mysql-9.7/dev/testing) 的补充，后者涵盖测试与迭代循环。
...
**参数类型：** `IntArg`、`RealArg`、`StringArg`、`CustomArg`——每个都提供 `is_null()` 和 `value()`。对于参数化的自定义类型，`CustomArgWith<P>` 添加了一个 `params()` 访问器，该访问器返回缓存的解析参数结构（请参阅 [参数化类型](/zh/mysql-9.7/dev/type-operations#parameterized-types)）。
...
有关完整的类型操作生成器参考——编码、解码、比较、哈希、内建默认值和参数化类型——请参阅 [类型操作](/zh/mysql-9.7/dev/type-operations)。

## 预览功能

以下 VEF 功能作为选择加入的预览标头提供。ABI 和 API 仍在积极开发中；有关完整参考，请参阅 [预览功能](/zh/mysql-9.7/dev/preview-capabilities)。

- **扩展系统变量**——[预览功能 → 系统变量](/zh/mysql-9.7/dev/preview-capabilities#system-variables)
- **扩展状态变量**——[预览功能 → 状态变量](/zh/mysql-9.7/dev/preview-capabilities#status-variables)
- **密钥环访问**——[预览功能 → 密钥环访问](/zh/mysql-9.7/dev/preview-capabilities#keyring-access)
- **列存储**——[预览功能 → 列存储](/zh/mysql-9.7/dev/preview-capabilities#column-storage)
...
- [使用 C++ 创建扩展](/zh/mysql-9.7/dev/create) — 端到端构建步骤、CMake 设置和安装
- [C++ 测试](/zh/mysql-9.7/dev/testing) — 本地开发服务器、MTR 以及调试失败
- [类型操作](/zh/mysql-9.7/dev/type-operations) — 编码、解码、比较、哈希、参数化类型
- [C++ API 参考](/zh/mysql-9.7/dev/extension-api-reference) — VDF 合约、空值处理和缓冲区大小
- [扩展架构](/zh/mysql-9.7/dev/architecture) — 生命周期、Victionary 缓存、性能模式和安全模型
