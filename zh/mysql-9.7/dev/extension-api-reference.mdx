...
此页面是 C++ 扩展作者的参考。有关分步教程，请参阅[使用 C++ 创建扩展](/zh/mysql-9.7/dev/create)。对于自定义列类型，请参阅[C++ 中的自定义类型](/zh/mysql-9.7/dev/custom-types)。
...
有关 `PrerunArgs` 和 `PostrunArgs` 方法的详细信息，请参阅开发指南中的[每语句状态（预执行和后执行）](/zh/mysql-9.7/dev/development#per-statement-state-prerun-and-postrun)。
...
还支持自定义聚合 VDF。使用 `make_aggregate_func<State, &result_fn>("name")` 注册一个，然后链接 `.returns()`、`.param()`、`.clear<>()` 和 `.accumulate<>()`，然后再调用 `.build()`。`.clear<>()` 和 `.accumulate<>()` 都是必需的。有关构建器 API 和回调签名，请参阅[聚合 VDF](/zh/mysql-9.7/dev/development#aggregate-vdfs)。
...
有关完整的预览 API 文档，请参阅[预览功能](/zh/mysql-9.7/dev/preview-capabilities)。
