...
协议 1 是原始 VEF 接口。自 v0.0.1 版本起稳定，并仍受支持，但可能在未来版本中被弃用。新的扩展应使用协议 3 API——基于模板的类型构建器，请参阅 [使用 C++ 创建扩展](/zh/mysql-9.7/dev/create) 和 [C++ 开发](/zh/mysql-9.7/dev/development) 指南。
...
协议 1 VDF 实现可以直接传递原始 C 结构体，而不是使用类型化包装器。这种样式仍受支持，但可能在未来版本中被弃用——对于所有新代码，请使用 [类型化参数/结果 API](/zh/mysql-9.7/dev/development#argument-and-result-types)。
...
不存在原始 ABI 的聚合注册路径。`make_func<>()` 没有 `.clear<>()` 或 `.accumulate<>()` 成员——这些方法只存在于来自 `<villagesql/vsql.h>` 的独立构建器 `make_aggregate_func<State, &result_fn>()` 上，该构建器使用基于 `State&` 的类型化签名，而不是原始的 `vef_context_t*`/`vef_vdf_args_t*` 指针。即使是协议 1 扩展也必须以这种方式注册聚合。对于所有新代码，请使用 [类型化聚合方法](/zh/mysql-9.7/dev/development#aggregate-vdfs)。
