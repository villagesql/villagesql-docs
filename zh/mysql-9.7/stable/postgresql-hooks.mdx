...
任何通过预览功能标记为**可用**的项都需要 `vsql_allow_preview_extensions = ON`——请参阅[预览功能](/zh/mysql-9.7/stable/preview-capabilities)。
...
| 自定义数据类型和运算符 | 注册一个具有自身存储和比较行为的新基础类型 | **可用**——请参阅 [C++ 中的自定义类型](/zh/mysql-9.7/stable/custom-types) |
| 聚合函数 | 注册一个用户定义的聚合 | **可用**——`make_aggregate_func`，请参阅 [C++ 开发](/zh/mysql-9.7/stable/development) |
