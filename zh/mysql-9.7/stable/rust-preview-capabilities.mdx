...
预览功能是服务器提供的功能，在最终确定其 API 之前，会将其暴露给扩展。Rust SDK 封装了其中五项：状态变量、系统变量、后台线程工作器、从工作线程发起的 SQL 查询和密钥环访问。本页介绍如何从 Rust 声明和使用其中的每一项。有关该概念、预览层以及仅在 C++ 中存在的功能，请参阅[预览功能](/zh/mysql-9.7/stable/preview-capabilities)。
...
其余的预览功能——`auth`、`mysql_services`、`statement_event` 以及列存储 ABI——目前仅支持 C++。如果您需要其中之一，请使用 [C++ SDK](/zh/mysql-9.7/stable/preview-capabilities)。
