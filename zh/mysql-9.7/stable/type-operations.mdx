...
title: "类型操作"
description: "C++ 类型操作构建器的深度参考——编码、解码、比较、哈希、内建默认值和参数化类型。"
---

本页是 C++ 类型操作构建器的参考。有关自定义类型的入门级介绍，请参阅 [C++ 中的自定义类型](/zh/mysql-9.7/stable/custom-types)。
...
省略自定义哈希函数会回退为对该类型存储的原始字节进行哈希。对于编码函数可能为同一个逻辑上相等的值生成多种二进制表示的类型，这样做并不安全——常见情形是浮点字段，其中 `-0.0` 和 `+0.0` 比较相等，但逐位不同，因此 `GROUP BY` 和 `COUNT(DISTINCT)` 会把它们视为不同的值。
...
一个自定义类型每个值最多可以声明 **65532 字节** 的存储空间。自定义列由一个
`VARBINARY` 字段支撑，而 MySQL 每行 65535 字节的预算还要为该字段的两个长度字节
以及该行的空值字节买单；因为列是否可为空是在 `CREATE TABLE` 时按列决定的，
而且默认可为空，所以该上限必须为空值字节留出空间。`persisted_length` 和
`max_persisted_length` 都会依据它进行检查，并且该检查在 `INSTALL EXTENSION` 时
运行，而不是在 `CREATE TABLE` 时运行，因此过宽的类型会连同整个扩展一起被拒绝：
...
不超过该上限并不能保证列一定放得下。表中的其他每一列都要占用同一份 65535 字节的
行预算，而只有 `CREATE TABLE` 才能衡量它，因此达到或接近上限的类型仍可能在那里
以 `ER_TOO_BIG_ROWSIZE` 失败。同等宽度的普通 `VARBINARY` 的行为也是如此。

## 内建默认值 {#intrinsic-default}
...
可以用两种方式之一提供内建默认值。两者互斥：同时声明
`.intrinsic_default_str()` 和 `.intrinsic_default_vdf()` 是编译时错误。
...
只要注册了 `.params<>()` 或 `.int_to_params<>()`，`build()` 就要求
`.resolve_params<>()`：`int_to_params` 只是把 `MYTYPE(N)` 简写改写为参数字符串，
而 `resolve_params` 才是验证参数并解析存储大小的关口。省略它是编译时错误。

<Warning>
`.max_persisted_length()` 需要 VEF 协议 3 或更高版本。使用它的类型无法被协议 3 之前的服务器加载。它仅对可变长度类型或参数化类型有效；设置了它的固定长度非参数化类型将无法编译。
</Warning>

`resolve_params` 报告的大小还会在 `CREATE TABLE` 时进行验证，此时列的参数已经确定。
对于固定长度类型，解析出的 `persisted_length` 不得超过声明的
`max_persisted_length`：如果某个参数化计算出的占用空间（例如按 `dimension * 4`
计算大小）超出该上界，它会被拒绝并返回一个语法错误。无论长度种类如何，
`resolve_params` 都必须解析出一个大于零的 `max_decode_buffer_length`，
因为该值决定解码暂存缓冲区的大小；非正的结果会被以同一类错误拒绝。
...
仅注册了 `.resolve_params<>()` 的类型接受字符串形式并拒绝 `MYTYPE(N)`。

`SHOW CREATE TABLE` 不会重复作者写下的内容。服务器保留解析后的参数映射，并据此
重建一种显示形式：当 `int_to_params(N)` 能够重现整个映射时，优先使用
`MYTYPE(N)`，否则回退到键已排序的规范形式 `MYTYPE('key=value,...')`。因此，
写作 `MYTYPE('dimension=8')` 的列读回时显示为 `MYTYPE(8)`。
...
可变长度类型还必须调用 `.max_persisted_length(N)`。如果省略它，`build()` 将在编译时失败——服务器需要这个上界来为底层字段分配缓冲区。可变长度类型不得同时调用
`.persisted_length()`：它的大小是按每个值决定的，因此 `build()` 会在编译时拒绝这种组合。
...
- [C++ 中的自定义类型](/zh/mysql-9.7/stable/custom-types) — 自定义类型的入门介绍
- [C++ API 参考](/zh/mysql-9.7/stable/extension-api-reference) — VDF 契约、空值处理和缓冲区大小调整
- [C++ 开发](/zh/mysql-9.7/stable/development) — VDF 编写深度、参数和结果类型、聚合、可变参数
