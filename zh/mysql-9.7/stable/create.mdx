...
如需深入了解编写 VDF 实现——参数和结果类型、聚合、系统变量、参数化类型——请参阅 [开发指南](/zh/mysql-9.7/stable/development)。

<Note>
如果您更偏好 Rust，请参阅 [使用 Rust 创建扩展](/zh/mysql-9.7/stable/rust-sdk)。
</Note>
...
在开始之前，您需要 VillageSQL Extension SDK，有两种方式可以让 CMake 指向它。如果您有 VillageSQL 构建树，请传入 `-DVillageSQL_BUILD_DIR=/path/to/villagesql/build`——[从源代码构建](/zh/mysql-9.7/stable/source) 指南会生成该构建树。如果您有一个已解包的 SDK 目录——其中包含 `include/villagesql/vsql.h`——那么仅此就已足够：传入 `-DVillageSQL_SDK_DIR=/path/to/sdk`。[扩展模板 README](https://github.com/villagesql/vsql-extension-template) 介绍了如何在不构建服务器的情况下获取 SDK。只有构建树方式会设置 VEB 安装目录，因此在仅使用 SDK 的方式下，请自行将构建好的 VEB 文件复制到 `SHOW VARIABLES LIKE 'veb_dir'` 所报告的目录中。

您还需要：

- **Git** - 用于克隆和版本控制
- **CMake** 3.18 或更高版本 - 构建系统
- **C++ 编译器** - 需支持 C++17：模板设置了 `CMAKE_CXX_STANDARD 17`，并且 SDK 的 CMake 包会拒绝更低的标准
- **基础 C++ 知识** - 理解 C++ 和函数指针
...
有关 SQL、文件名和仓库名称的完整命名约定，请参阅 [扩展命名约定](/zh/mysql-9.7/stable/install#extension-naming-conventions)。
...
有关完整的 `CustomArg`/`CustomResult` API（包括用于参数化类型的 `CustomArgWith<P>` 和 `CustomResultWith<P>`），请参阅 [开发指南](/zh/mysql-9.7/stable/development#argument-and-result-types)。
...
当您使用 `buffer()` 和 `set_length()` 自行构建值时，切勿写入超出 `buffer().size()` 的部分——那会导致内存溢出。但也不要因为值过大而失败：请报告您本应写入的大小，并让服务器扩大缓冲区并再次调用您的函数。
...
对于 STRING 结果，`out.set(sv)` 遵循 snprintf 风格的溢出契约：它复制能容纳的尽可能多的字节，但报告值的*完整*大小。服务器检测到所报告的长度超过缓冲区后，会扩大缓冲区并第二次调用该函数——因此大于 `.buffer_size()` 的值会被完整返回，而不是被截断。一次重试总是足够的，因为所报告的长度准确说明了缓冲区必须有多大。

扩容受 `max_allowed_packet` 限制。大于该限制的值将返回 `NULL`，而不是被截断。

您自行写入 `out.buffer()` 并调用 `out.set_length()` 时，同样遵循该契约：请报告您本应写入的完整大小，而不是能容纳的字节数。报告被截断的长度会使重试失效，并静默丢弃剩余部分。
...
<Note>
在实现函数之前，请查阅 [C++ API 参考](/zh/mysql-9.7/stable/extension-api-reference) 以了解完整的 VDF 契约：空值检查、结果类型、缓冲区大小和错误处理。
</Note>
...
请参阅 [C++ 中的自定义类型](/zh/mysql-9.7/stable/custom-types) 获取完整的实现指南。如果您的类型需要参数（例如 `VECTOR(1536)`），
请参阅 [参数化类型](/zh/mysql-9.7/stable/type-operations#parameterized-types)。
...
<CardGroup cols={2}>
  <Card
    title="使用扩展"
    icon="puzzle-piece"
    href="/zh/mysql-9.7/stable/install"
  >
    了解如何安装和管理扩展
  </Card>
  <Card
    title="开发指南"
    icon="code"
    href="/zh/mysql-9.7/stable/development"
  >
  ...
  <Card
    title="从源码构建"
    icon="hammer"
    href="/zh/mysql-9.7/stable/source"
  >
    从源代码构建 VillageSQL
  </Card>
</CardGroup>
