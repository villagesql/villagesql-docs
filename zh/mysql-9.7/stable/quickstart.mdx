...
`VSQL_CODEBASE` 接受 `mysql-8.4`、`percona-8.4` 或 `mysql-9.7`。Percona 构建仅面向 MySQL 8.4。`INSTALL_METHOD` 接受 `docker`、`prebuilt` 或 `source`。源码构建还接受 `VSQL_VERSION=stable|nightly|latest`；Docker 和预编译安装始终使用稳定版本。
