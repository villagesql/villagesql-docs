...
本页介绍 Rust SDK 仓库中的四个参考扩展——一个仅包含函数的最小示例，一个包含算术运算、排序和哈希的完整自定义类型，一个聚合函数，以及一个变长参数函数。该仓库的 `examples/` 目录还为每一项[预览功能](/zh/mysql-9.7/stable/rust-preview-capabilities)提供了一个示例，此外还包含 `vsql_call_index` 和 `vsql_postrun_order`，它们演示的是按语句的状态生命周期，而不是某项预览功能。
...
<CardGroup cols={2}>
  <Card
    title="在 Rust 中创建扩展"
    icon="rust"
    href="/zh/mysql-9.7/stable/rust-sdk"
  >
  ...
  <Card
    title="Rust 自定义类型"
    icon="cube"
    href="/zh/mysql-9.7/stable/rust-custom-types"
  >
