...
Real boolean type. A proper `STRICTBOOL` column type with standard truth values (`true`/`false`, `yes`/`no`, `on`/`off`, `1`/`0`), 1-byte storage, correct dump/restore semantics, and aggregate functions `boolean_sum()` and `boolean_avg()` for boolean columns.
